        ...
        .then(function (response) {
            ...
            if(error.request) {
              ...
              let factureListContainer = []
                ...
                getData.SE_ID,
                getData.Me_Name,
                new Date(getData.SE_Date).toLocaleDateString('fr-FR'),
                new Date(getData.SE_Date).toLocaleDateString('fr-FR'),
                _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/ft/info/${getData.SE_ID}`)}><span className='d-none d-lg-inline'> Info </span><Icon  name='angle right' /></Button>)
                ],))
                setFactureList(factureListContainer)
            }
          });
