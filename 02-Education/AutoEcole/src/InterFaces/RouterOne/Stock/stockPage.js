    ...
    const FetchByGenreReserv = (genre) =>{  
      ...
      let commandeContainer = []
          found.map( (commandeDate) => commandeContainer.push([          
            ...
            _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/C/L/rdv/info/${commandeDate.R_ID}`)}><span className='d-none d-lg-inline'> Info </span><Icon  name='angle right' /></Button>)
        ],))
