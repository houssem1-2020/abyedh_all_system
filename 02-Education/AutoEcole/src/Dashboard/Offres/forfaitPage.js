...
function StockPage() {
    ...
    /*#########################[Function]##################################*/
      ...
      const SaveForfait = (event) => {
               if (!forfaitD.F_Name) {toast.error("Nom Invalide !", GConf.TostErrorGonf)}
               else if (!forfaitD.NB_Seance) {toast.error("Nombre de Seance Invalide !", GConf.TostErrorGonf)}
               else if (!forfaitD.Tarif) {toast.error("Prix Invalide !", GConf.TostErrorGonf)}
               else{
                  setLS(true)
