...
import { View, ScrollView, Platform, Text, FlatList, TouchableOpacity, Image, StyleSheet, Animated } from 'react-native';
...
const CountryPage = () => {
  ...
  const ItemListCard = (props) => {
    ...
    const renderItem = ({ item, index }) => (
        ...
        item.seeAll  ?
        <TouchableOpacity onPress={() => navigation.navigate('SeeAllList' , { Tag: props.slectedTag })}>
          <View style={{ padding: 10, marginBottom: 8, backgroundColor:'#ffffff', margin: 5,   borderRadius: 8,     }}>
            <View style={{ flexDirection: 'column', alignContent:'center' }}>
              <View style={{ marginRight: 10, width: 40,  height: 40, borderColor:'white', borderWidth :1, borderRadius: 50,  justifyContent: 'center', alignItems: 'center' }}>
                  <Ionicons name={isRTL ? 'arrow-back-outline' : 'arrow-forward-outline' } size={20} color="grey" />
        ...
        :
        <TouchableOpacity onPress={() => navigation.navigate('LandingPage', { Tag: item.link })}>
          <View style={{ padding: 10, marginBottom: 8, backgroundColor:'#ffffff', margin: 5,   borderRadius: 8,     }}>
            <View style={{ flexDirection: 'column', alignContent:'center' }}>
  ...
  const GoToToolsMainPage = () => {
    return(<>
      <View style={{ flex: 1 , marginBottom : 20, marginTop : 20}}>
        <TouchableOpacity onPress={() => navigation.navigate('ToolsPage')}>
          <View
            style={{
              width : '100%',
              padding: 13,
    ...
    <SafeAreaProvider style={{backgroundColor : 'white'}}>
        ...
        >
                <View style={{ paddingHorizontal:10,  }}  >
                    <StackBarCard backColor='' leftComLogo dropDwon={true}  backLink='LandingPage' rigthImage='05.gif' clickOption='' bottomSearchs />
                    <SearchBox />
