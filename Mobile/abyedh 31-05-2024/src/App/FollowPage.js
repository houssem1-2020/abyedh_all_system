...
const UserPage = () => {
  ...
  const UserBottomNavCard = (props) => {
    const TabsList =  [
      {id:1, iconName:'view-grid-plus', indexNum : 0 },
      {id:1, iconName:'star-circle', indexNum : 1 },
      {id:1, iconName:'animation', indexNum : 2 },
      // {id:1, iconName:'folder-eye', indexNum : 3 },
      {id:1, iconName:'shield-account', indexNum : 4 },
    ...
    const TabsCard = (props) => {
      return(
        ...
        >
          <Text >
            <Icon name={props.data.indexNum == selectedTab ? props.data.iconName : `${props.data.iconName}-outline`} size={27} color={props.data.indexNum == selectedTab ? '#0e8a71': 'grey'} />
          </Text>
        </Ripple>
      )
    }
