...
function MenuPage() {
        ...
        .then(function (response) {
          if (!response.data) {
              ...
              response.data.map( (getData) => articleListContainer.push([
                ...
                getData.RA_Time,
                _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/rp/info/${getData.RA_ID}`)}><span className='d-none d-lg-inline'> {t('menuTabs.rapportPage.infoBtn')} </span><Icon  name='angle right' /></Button>),
              ],))
    ...
    return (<>
              ...
              <SubNav dataForNav={GConf.SubNavs.Menu} snTag='Menu' />
              <br />
              <Fade> 
                <TableGrid tableData={articleList} columns={t(`TableHead.menu`).split(/,[\s]*'/).map(item => item.replace(/'/g, '')).filter(Boolean)}  />
              </Fade> 
