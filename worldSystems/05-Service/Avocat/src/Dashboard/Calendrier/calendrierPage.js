...
function CalendarCommandes() {
 ...
 console.log(response.data)
            setTodayListe(response.data.filter((data) =>   data.SE_ID ==  new Date().toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' )))
            let calendarData = []
            response.data.map( (getData) => calendarData.push( { title: getData.SJ_Title , date: new Date(getData.SE_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ), url: `/S/sa/info/${getData.SE_ID}` }))
           ...
           <div className='col-12 col-lg-9'>  
            ...
            <div className='col-12 col-lg-3'>
                <div className="sticky-top" style={{top:'90px', zIndex:'999'}}>
                    <h5><span className='bi bi-calendar-week'></span> Seances D'aujourd'hui </h5>
