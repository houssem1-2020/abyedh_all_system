...
function ClientInfo() {
    ...
    const panes = [
        ...
        {
            menuItem: { key: 'home', icon: 'home', content: 'Sujets' }, 
    ...
    useEffect(() => {
        ...
        axios.post(`${GConf.ApiLink}/client/info`, {
            ...
            if(!response.data.Data.PK) {
                ...
                let factureTable = []
                response.data.demmandes.map( (getData, index) => factureTable.push([ 
                getData.T_ID,
                getData.CA_Name,
                ...
                let commandeTable = []
                response.data.sujets.map( (getData, index) => commandeTable.push([ 
                    getData.R_ID,
                ...
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className="img-card-container text-center">
                        <div className="card-container">
                            <img src="https://cdn.abyedh.com/images/system/avocat/client.png" className="rounded-circle" width="80" />                    
                        </div>
                    </div>
                    <div className="mt-5 text-center">
                            <h4 className='mt-2'>{loading ? clientD.CL_Name : SKLT.BarreSkl } </h4> 
                            <h6 className="text-secondary">  {loading ? <><span className="bi bi-geo-alt-fill"></span> { clientD.CL_Adress } </>: SKLT.BarreSkl} </h6>
                            <h6 className="text-secondary"> {loading ? <><span className="bi bi-geo-fill"></span> { clientD.CL_Gouv } , ({ clientD.CL_Deleg })</>: SKLT.BarreSkl } </h6>
