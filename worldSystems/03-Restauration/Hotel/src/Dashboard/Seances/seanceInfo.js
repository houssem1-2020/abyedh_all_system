  ...
  return (
    <>
      <div className="p-1  mb-2">
        ...
        <datalist id="clientList">
          {abonnementListe.map((test) => (
            <option key={test.AB_ID} value={test.AB_ID}>
              {test.ME_Name} : {test.F_Name}
            </option>
          ))}
        </datalist>
        <Input
          ...
          list="clientList"
          placeholder={seanceD.Membre_ID}
          onBlur={(e) => SelectForfaitFunc(e.target.value)}
          size="small"
          iconPosition="left"
          fluid
      ...
      <div className="card card-body shadow-sm rounded">
        <div>
          <b>Nom </b> : {selectedForfait ? selectedForfait.ME_Name : "..."}{" "}
        </div>
        <div>
          <b>Forfait </b> : {selectedForfait ? selectedForfait.F_Name : "..."}{" "}
        </div>
        <div>
          <b>Etat </b> : {selectedForfait ? selectedForfait.State : "..."}{" "}
        </div>
      </div>
      ...
      <div className="row">
        <div className="col-6">
          ...
          <Input
            ...
            fluid
            className="mb-1 shadow-sm"
            value={new Date(seanceD.SE_Date).toISOString().split("T")[0]}
            onChange={(e) =>
              setSeanceD({ ...seanceD, SE_Date: e.target.value })
            }
          />
        </div>
        <div className="col-6">
          ...
          <Input
            ...
            fluid
            className="mb-1 shadow-sm"
            value={seanceD.SE_Time}
            onChange={(e) =>
              setSeanceD({ ...seanceD, SE_Time: e.target.value })
            }
          />
        </div>
      </div>
...
function CamionInfo() {
  ...
  useEffect(() => {
    axios
      .post(`${GConf.ApiLink}/seances/info`, {
        PID: GConf.PID,
        SID: CID,
      })
      ...
      .catch((error) => {
        if (error.request) {
          toast.error(
            <>
              <div>
                <h5>Probleme de Connextion</h5> Impossible de charger les
  ...
  const EditCamion = () => {
    if (!seanceD.Abonnement_ID) {
      toast.error("Matricule Invalide !", GConf.TostErrorGonf);
    } else if (!seanceD.Membre_ID) {
      toast.error("Fond Invalide !", GConf.TostErrorGonf);
    } else if (!seanceD.SE_Date) {
      toast.error("Identifiant Invalide !", GConf.TostErrorGonf);
    } else if (!seanceD.SE_Time) {
      toast.error("Mot De Passe Invalide !", GConf.TostErrorGonf);
    } else {
      setLS(true);
      axios
        .post(`${GConf.ApiLink}/seances/modifier`, {
          PID: GConf.PID,
          seanceData: seanceD,
          SE_ID: CID,
        })
  ...
  const SelectForfaitFunc = (abID) => {
    ...
    if (selectedAB) {
      ...
      setSeanceD({
        ...seanceD,
        Abonnement_ID: selectedAB.AB_ID,
        Membre_ID: selectedAB.ME_ID,
      });
    } else {
  ...
  const DeleteSeanceFunc = () => {
    setLS(true);
    axios
      .post(`${GConf.ApiLink}/seances/supprimer`, {
        PID: GConf.PID,
        SE_ID: CID,
      })
      .then(function (response) {
  ...
  const StateCard = ({ status }) => {
    ...
    const statusCard = React.useCallback(() => {
      switch (status) {
        ...
        case "P":
          return <StateCard color="secondary" text="Pas Encore" />;
        case "T":
          return <StateCard color="success" text="Termineé" />;
        case "PA":
          return <StateCard color="info" text="Aujourd hui" />;
        default:
          return <StateCard color="primary" text="Indefinie" />;
      }
    }, [status]);
  ...
  const FindSeanceState = (date, time) => {
    ...
    if (
      Today.toLocaleDateString("fr-FR").split("/").reverse().join("-") <
      new Date(date).toLocaleDateString("fr-FR").split("/").reverse().join("-")
    ) {
      return <StateCard status={"P"} />;
    } else if (
      Today.toLocaleDateString("fr-FR").split("/").reverse().join("-") >
      new Date(date).toLocaleDateString("fr-FR").split("/").reverse().join("-")
    ) {
      return <StateCard status={"T"} />;
    } else {
      if (new Date().toLocaleTimeString([], { hourCycle: "h23" }) > time) {
        return <StateCard status={"PA"} />;
      } else if (
          ...
          <div className="card card-body shadow-sm mb-2 border-div">
            ...
            <div className="img-card-container text-center">
              <div className="card-container">
                <img
                  src="https://cdn.abyedh.com/images/system/gym/seance.jpg"
            ...
            <div className="mt-5 text-center">
              <h4 className="mb-2 ">
                {loading ? props.dataC.ME_Name : SKLT.BarreSkl}{" "}
              </h4>
              <h4 className="mb-2 mt-0">
                {loading ? props.dataC.F_Name : SKLT.BarreSkl}{" "}
              </h4>
              <h6 className="text-secondary mt-0">
                {" "}
                {loading ? (
                  <>
                    <span className="bi bi-calendar-check-fill"></span>{" "}
                    {new Date(props.dataC.SE_Date).toISOString().split("T")[0]}{" "}
                  </>
                ) : (
                  SKLT.BarreSkl
                )}{" "}
              ...
              <h6 className="text-secondary mt-0">
                ...
                {loading ? (
                  <>
                    <span className="bi bi-alarm-fill"></span>{" "}
                    {props.dataC.SE_Time}{" "}
                  </>
                ) : (
                  SKLT.BarreSkl
    ...
    return (
      <>
        <h3 className="text-secondary">
          Voulez-Vous Vraimment Supprimer Cette Seance ?
        </h3>
        <h5 className="text-danger text-left">
          <b>Lorsque Vous Supprimer La Seance : </b>
        </h5>
