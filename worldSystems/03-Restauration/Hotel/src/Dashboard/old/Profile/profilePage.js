...
const EditProfile = ({
  generalData,
  setGeneralData,
  UpdateGeneralDataFunc,
  delegList,
  GetDelegList,
  loaderState,
}) => {
  const genreOptions = [
    { key: 1, value: "GYM", text: "GYM" },
    { key: 2, value: "INDIV", text: "رياضات فردية" },
    { key: 3, value: "COLLECTIV", text: "رياضات جماعية" },
  ];
  return (
    <>
      ...
      <div className="mb-2">
        <Select
          ...
          className="mb-2"
          options={TunMap.Gouv}
          value={generalData.Gouv}
          onChange={(e, { value }) => GetDelegList(value)}
        />
        <Select
          placeholder="Selectionnez Delegation "
          fluid
          value={generalData.Deleg}
          options={delegList}
          onChange={(e, { value }) =>
            setGeneralData({ ...generalData, Deleg: value })
          }
        />
      </div>
      ...
      <Form>
        <TextArea
          ...
          onChange={(e) =>
            setGeneralData({ ...generalData, Adress: e.target.value })
          }
        />
      </Form>
      ...
      <Dropdown
        ...
        className="mb-1"
        onChange={(e, { value }) =>
          setGeneralData({ ...generalData, Genre: value })
        }
      ...
      <div className="text-end">
        <Button
          ...
          onClick={() => UpdateGeneralDataFunc()}
        >
          <Icon name="save" /> Modifier{" "}
          <Loader
            active={loaderState}
            inverted
            inline
  ...
  return (
    <>
      ...
      <Input
        ...
        onChange={(e) =>
          setPasswordData({ ...passwordData, Identification: e.target.value })
        }
        fluid
      />
      ...
      <Input
        ...
        onChange={(e) =>
          setPasswordData({ ...passwordData, PasswordSalt: e.target.value })
        }
        fluid
      />
      <br />
...
function ProfilePage() {
  ...
  /*Position */
  const [myPosition, setMyPosition] = useState([36.1772, 9.12337]);
  ...
  const panes = [
        ...
        <>
          <Tab.Pane className="border-div" attached={false}>
            <EditProfile
              ...
              UpdateGeneralDataFunc={UpdateGeneralDataFunc}
              delegList={delegList}
    ...
    },
    {
      ...
      render: () => (
        <>
          <Tab.Pane className="border-div p-4" attached={false}>
            <Tab
              menu={{ secondary: true, className: "tab-right" }}
              panes={RatingPanes}
            />
          </Tab.Pane>
          <br />
  ...
  const UpdateGeneralDataFunc = () => {
    if (!generalData.Name) {
      toast.error("Nom de la ste est  Invalide !", GConf.TostErrorGonf);
    } else if (!generalData.Gouv) {
      toast.error("Gouv est  Invalide !", GConf.TostErrorGonf);
    } else if (!generalData.Deleg) {
      toast.error("Delegation est  Invalide !", GConf.TostErrorGonf);
    } else if (!generalData.Phone) {
      toast.error("Phone est  Invalide !", GConf.TostErrorGonf);
    } else if (!generalData.Adress) {
      toast.error("Adress est  Invalide !", GConf.TostErrorGonf);
    } else if (!generalData.Genre) {
      toast.error("Genre est  Invalide !", GConf.TostErrorGonf);
    } else {
      setLS(true);
      axios
        .post(`${GConf.ApiLink}/profile/update/general`, {
          PID: GConf.PID,
  ...
  const UpdatePasswordFunc = () => {
    if (!passwordData.Identification) {
      toast.error("Identifiant est Invalide !", GConf.TostErrorGonf);
    } else if (!passwordData.PasswordSalt) {
      toast.error("Mot de passe est  Invalide !", GConf.TostErrorGonf);
    } else {
      setLS(true);
      axios
        .post(`${GConf.ApiLink}/profile/update/password`, {
          PID: GConf.PID,
  ...
  const CalculateLikes = (table) => {
    ...
    if (WantedValue / 1000 > 1 && WantedValue / 1000000 < 1) {
      return parseFloat(WantedValue / 1000).toFixed(1) + "K";
    } else if (WantedValue / 1000000 > 1) {
      return parseFloat(WantedValue / 1000000).toFixed(1) + "M";
    } else {
      return parseInt(WantedValue);
    }
  };
  ...
  const ReturnAvatarGroupList = (list) => {
    ...
    list.map((data, index) =>
      FinalList.push({
        key: index,
        name: data.Name,
        src: `https://cdn.abyedh.com/images/p_pic/${data.PictureId}.gif`,
      })
    );
    return FinalList;
  };
  ...
  const UpdateImageFunc = (genre) => {
    if (!uploadImage) {
      toast.error("Image est Invalide !", GConf.TostErrorGonf);
    } else {
      ...
      axios({
        ...
        .catch((error) => {
          toast.error(
            <>
              <div>
                <h5>Probleme de Connextion</h5> La commande sera enregisstrer
                sur votre appareil{" "}
              </div>
            </>,
            GConf.TostInternetGonf
  ...
  const UpdateImageFuncMultiple = (formData) => {
    axios
      .post(`${GConf.ApiLink}/profile/images/ajouter`, formData)
  ...
  const RemoveImageFunc = (imgName) => {
    ...
    axios
      .post(`${GConf.ApiLink}/profile/images/deletefile`, {
        fileName: imgName,
      })
      .then(function (response) {
        console.log(response.data);
      ...
      function (position) {
        if (!position.coords.latitude) {
          toast.error(
            <>
              <div>
                <h5>ENTRE VOTRE POSITION Lat</h5>
              </div>
            </>,
            GConf.TostInternetGonf
          );
          ...
          toast.error(
            <>
              <div>
                <h5>ENTRE VOTRE POSITION Lng</h5>
              </div>
            </>,
            GConf.TostInternetGonf
          );
  ...
  const UpdatePositionFunc = () => {
    if (!passwordData.Identification) {
      ...
      setLS(true);
      axios
        .post(`${GConf.ApiLink}/profile/update/position`, {
          PID: GConf.PID,
      ...
      <>
        <div className="d-flex">
          ...
          <div className="flex-grow-1 ms-3">
            <h1> {GConf.PID} </h1>
            Cet identifiant vous distinguera des autres inscrits sur la
            plateforme. Cela peut aussi être un court moyen de faire de la
            publicité pour vous
            <div>
              <Button
                size="mini"
                ...
                onClick={() =>
                  navigator.clipboard.writeText(localStorage.getItem("PID"))
                }
              >
                <Icon name="copy" /> Copier PID
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="d-flex">
          ...
          <div className="flex-grow-1 ms-3">
            Imprimez ce lien qui peut être accroché à la porte de votre magasin
            afin que vos clients puissent vous joindre facilement, et il peut
            également être partagé directement sur les réseaux sociaux
            <div className="mt-2">
              <Button
                size="mini"
                positive
                onClick={(e) => PrintFunction("printPID")}
              >
                {" "}
                <Icon name="print" />
                Imprimer
              ...
              <Button
                ...
                primary
                target="_blank"
                href={`https://www.facebook.com/sharer/sharer.php?u=https://abyedh.com/S/P/${GConf.systemTag}/${GConf.PID}`}
              >
                {" "}
                <Icon name="facebook f" /> Partager{" "}
              </Button>
            </div>
          </div>
        </div>
  ...
  const ProfileCard = () => {
    return (
      <>
        <div
          className="card card-body shadow-sm mb-4 sticky-top border-div"
          style={{ top: "70px" }}
        >
          <div className="text-center ">
            ...
            <div>
              <small className="text-secondary">
                Salle de Sport (
                {loading ? profileData.general[0].Genre : SKLT.BarreSkl})
              </small>
            </div>
            <div>
              <small className="text-secondary">
                ...
                {loading ? (
                  <>
                    {" "}
                    {profileData.general[0].Adress} ,{" "}
                    {profileData.general[0].Gouv}{" "}
                  </>
                ) : (
                  SKLT.BarreSkl
                )}{" "}
            ...
            <div>
              <small className="text-secondary">
                <span className="bi bi-telephone"></span> +216{" "}
                {loading ? profileData.general[0].Phone : SKLT.BarreSkl}{" "}
              </small>
            </div>
            <h5>
            ...
            <div className="row mt-2">
              <div className="col-6">
                <Statistic color="red" size="small">
                  <Statistic.Value>
                    {loading
                      ? CalculateRating(profileData.review)
                      : SKLT.BarreSkl}
                  </Statistic.Value>
              ...
              <div className="col-6">
                <Statistic color="red" size="small">
                  <Statistic.Value>
                    {loading
                      ? CalculateLikes(profileData.likes)
                      : SKLT.BarreSkl}
                  </Statistic.Value>
            ...
            <div className="row">
              <div className="col-3">
                <Icon circular inverted color="red" name="youtube" />
                youtube
              </div>
              <div className="col-3">
                <Icon circular inverted color="blue" name="facebook square" />{" "}
                facebook
              </div>
              <div className="col-3">
                <Icon circular inverted color="pink" name="instagram" />{" "}
                instagram
              </div>
              <div className="col-3">
                <Icon circular inverted color="yellow" name="snapchat ghost" />{" "}
                snapchat
              </div>
            </div>
            <br />
            ...
            <div className="d-grid gap-2">
              <a
                className="btn btn-danger btn btn-lg bnt-block rounded-pill text-white "
                target="_blank"
                href={`https://abyedh.com/S/P/${GConf.systemTag}/${GConf.PID}`}
              >
    ...
    return (
      <>
        <div className="card card-body shaows-sm mb-2">
          <h5>J'aimes</h5>
          {loading
            ? profileData.likes.map((data, index) => (
      ...
      <>
        <div className="p-1">
          ...
          <div className="row mb-3">
            <div className="col-8">
              Lorsque vous cliquer ici la position enregistré sera 'Mon
              position'
            </div>
            <div className="col-4 text-end">
              <Button
                className="rounded-pill bg-system-btn"
                disabled={CheckPositions()}
                size="mini"
                onClick={() => UpdatePositionFunc()}
              >
                <Icon name="save" /> Modifier Position{" "}
                <Loader
                  inverted
                  active={loaderState}
                  inline
              ...
              <div className="col-3">
                <img
                  ...
                  src={
                    ...
                    `${GConf.AssetsLink}/${props.data.ImageLink}`
                  }
                  alt={props.data.Descreption}
                  width="100%"
                  height="80px"
                />
              </div>
              <div className="col-4">{props.data.ImageTag}</div>
        ...
        <>
          <div className="card p-2 mb-2 rounded">
            ...
            <div className="row">
              ...
              <div className="col-3">
                {displayedImage ? (
                  <img src={displayedImage} width="100%" height="50px" />
                ) : (
                  ""
                )}
              </div>
              ...
              <div className="col-3 align-self-center text-end">
                <Button
                  className="rounded-pill bg-system-btn"
                  size="tiny"
                  onClick={UpdateImageFunc}
                >
                  <Icon name="save" /> Modifier{" "}
                  <Loader inverted inline size="tiny" className="ms-2" />
                </Button>
              </div>
            </div>
            {/* </form> */}
    ...
    return (
      <>
        <div className="p-2 mb-2">
          {profileData.images.map((data, index) => (
            ...
            <div className="row">
              ...
              <div className="col-6">
                <Button
                  className="rounded-pill bg-system-btn"
                  size="tiny"
                  onClick={() => UpdateImageFuncMultiple(formaDataArr)}
                >
                  <Icon name="save" /> Enregistreé{" "}
                  <Loader
                    inverted
                    active={loaderState}
                    inline
        ...
        <>
          <div className="card card-body shadow-m mb-2 border-div">
            <div className="row">
              <div className="col-4">
                <img
                  src={`https://cdn.abyedh.com/images/Directory/${props.imageLink}`}
                  className="border border-2 rounded shadow-sm"
                  width="200px"
                  height="90px"
                />
              </div>
              ...
              <div className="col-4">
                <Button onClick={() => RemoveImageFunc(props.imageLink)}>
                  Delete Btn
                </Button>
              </div>
            </div>
        ...
        <>
          <div className="text-center">
            ...
            >
              ...
              <img
                src="https://assets.ansl.tn/Images/usful/uploadImage.jpg"
                width="100%"
                height="150px"
              />
            </label>
                ...
                <>
                  {todisplayedImage.map((data, index) => (
                    <div className="col-4 mb-3" key={index}>
                      <img
                        src={URL.createObjectURL(todisplayedImage[index])}
                        className="border border-2 rounded shadow-sm"
                        width="200px"
                        height="90px"
                      />
          ...
          <>
            <Carousel>
              {imagesListe.map((data, index) => (
                <div key={index}>
                  <img
                    src={`https://cdn.abyedh.com/images/Directory/${data.ImageLink}`}
                  />
                  <p className="legend">
                    <Button onClick={() => RemoveImageFunc(data.ImageLink)}>
                      Supprimer
                    </Button>
                  </p>
                </div>
    ...
    return (
      <>
        ...
        <div className="row text-center">
          <div className="col-4 align-self-center">
            <h1 className="text-warning">
              {loading ? <> {CalculateRating(profileData.review)}</> : 0}
            </h1>
            <Rating
              className="d-inline"
              maxRating={5}
              defaultRating={loading ? CalculateRating(profileData.review) : 0}
              icon="star"
              disabled
              size="massive"
            />
          ...
          <div className="col-8">
            <RatingBar
              name={1}
              value={loading ? CalculateReview(profileData.review, 1) : 0}
            />
            <RatingBar
              name={2}
              value={loading ? CalculateReview(profileData.review, 2) : 0}
            />
            <RatingBar
              name={3}
              value={loading ? CalculateReview(profileData.review, 3) : 0}
            />
            <RatingBar
              name={4}
              value={loading ? CalculateReview(profileData.review, 4) : 0}
            />
            <RatingBar
              name={5}
              value={loading ? CalculateReview(profileData.review, 5) : 0}
            />
          </div>
        </div>
        <hr />
        ...
        <AvatarGroup
          ...
          size="large"
          maxCount={12}
          data={ReturnAvatarGroupList(profileData.likes)}
          borderColor="#cfcecc"
        />
        {/* { loading ?
      ...
      return (
        <>
          <div className="d-flex mb-2 border-bottom">
            <div className="flex-shrink-0">
              <img
                src={`https://cdn.abyedh.com/images/p_pic/${props.data.PictureId}.gif`}
                className="rounded-circle"
                width="30px"
                alt="..."
              />
            ...
            <div className="flex-grow-1 ms-3 w-100">
              <div className="row  mb-0">
                ...
                <div className="col-6 text-end">
                  <small className="text-secondary">
                    {" "}
                    {new Date(props.data.R_Date).toLocaleDateString()}
                  </small>
                </div>
              </div>
              <div>
                <b>{props.data.Comment} </b>
              </div>
            </div>
          </div>
        </>
        ...
        <div className="row ">
          <div className="col-12 pe-1">
            ...
            >
              {loading ? (
                <Comment.Group>
                  {profileData.review.map((data, index) => (
                    <CommentsCard key={index} data={data} />
                  ))}
                </Comment.Group>
              ) : (
                "..."
            ...
            <div className="row">
              <div className="col-6">
                <Input
                  ...
                  size="mini"
                  iconPosition="left"
                  disabled={JSON.parse(props.data.dayOff)}
                  fluid
                  className="mb-1"
                  value={props.data.matin.start}
                />
              </div>
              <div className="col-6">
                <Input
          ...
          <div className="col-1 align-self-center">
            <div className="form-check form-switch">
              <input
                ...
                type="checkbox"
                onChange={() => setAlwaysState(!props.data.dayOff)}
                checked={JSON.parse(props.data.dayOff)}
              />
            </div>
          </div>
        </div>
      ...
      <>
        <div className="card-body ">
            ...
            <div className="col-12 col-lg-1 align-self-center">
              <div className="form-check form-switch">
                <input
                  className="form-check-input form-check-input-lg"
                  type="checkbox"
                  onChange={() => setAlwaysState(!alwaysState)}
                  checked={alwaysState}
                />
              </div>
            </div>
          ...
          <div className="row mb-2 mt-4">
            ...
            <div className="col-4 text-end">
              <Button className="rounded-pill bg-system-btn" size="mini">
                <Icon name="save" /> Modifier Horaire{" "}
                <Loader inverted inline size="tiny" className="ms-2" />
              </Button>
            </div>
          </div>
      ...
      <>
        <FullCalendar
          ...
          events={[
            {
              title: "S1",
              start: "2022-08-18T08:00:00",
              end: "2022-08-18T12:00:00",
              display: "background",
              backgroundColor: "red",
            },
            {
              title: "S2",
              start: "2022-08-18T14:00:00",
              end: "2022-08-18T18:00:00",
              display: "background",
            },
            {
              title: "S1",
