    ...
    <>
      ...
      >
        ...
        <Modal.Content>
          ...
          <div className="p-1 mb-2">
            ...
            <Input
              ...
              placeholder="Numero"
              className="w-100 border-0 shadow-sm rounded mb-1"
              value={editGroupData.Num}
              onChange={(e) =>
                setEditGroupData({ ...editGroupData, Num: e.target.value })
              }
            />
          </div>
        </Modal.Content>
        <Modal.Actions>
          ...
          <Button className=" bg-system-btn rounded-pill" onClick={EditTable}>
            {" "}
      ...
      >
        ...
        <Modal.Content>
          ...
          <div className="mb-0 p-0">
            <h5> Table : {editGroupData.Name}</h5>
          </div>
          <div>
            <h5> Numero : {editGroupData.Num} </h5>
          </div>
        </Modal.Content>
        <Modal.Actions>
          {/* <Button className='rounded-pill' negative onClick={ () => setDeleteModalS(false)}> <span className='bi bi-x' ></span> </Button> */}
          <Button negative className="rounded-pill" onClick={DeleteTable}>
            {" "}
...
function CaissePage() {
  ...
  const [groupData, setGroupData] = useState({
    GP_Temps: new Date().toLocaleTimeString([], { hourCycle: "h23" }),
  });
      ...
      .catch((error) => {
        if (error.request) {
          toast.error(
            <>
              <div>
                <h5>Probleme de Connextion</h5> Chargemment des ancien Camion{" "}
              </div>
            </>,
  ...
  const SaveGroupFunction = () => {
    if (!groupData.GP_Name) {
      toast.error("Nom Invalide !", GConf.TostErrorGonf);
    } else if (!groupData.GP_Genre) {
      toast.error("Genre Invalide !", GConf.TostErrorGonf);
    } else if (!groupData.GP_Activite) {
      toast.error("Activite Invalide !", GConf.TostErrorGonf);
    } else if (!groupData.GP_Temps) {
      toast.error("Temps Invalide !", GConf.TostErrorGonf);
    } else if (!groupData.GP_Cotch) {
      toast.error("Cotch Invalide !", GConf.TostErrorGonf);
    } else {
      setLS(true);
      axios
        .post(`${GConf.ApiLink}/groupe/ajouter`, {
          PID: GConf.PID,
          groupData: groupData,
        })
        .then(function (response) {
        ...
        .catch((error) => {
          if (error.request) {
            toast.error(
              <>
                <div>
                  <h5>Probleme de Connextion</h5> Le Camion sera enregistrer sur
                  votre ordinateur{" "}
                </div>
              </>,
              GConf.TostInternetGonf
  ...
  const EditTable = () => {
    setLS(true);
    axios
      .post(`${GConf.ApiLink}/groupe/modifier`, {
        PID: GConf.PID,
      ...
      .catch((error) => {
        if (error.request) {
          toast.error(
            <>
              <div>
                <h5>Probleme de Connextion</h5> Vous Pouvez pas modifier cette
                famille{" "}
              </div>
            </>,
            GConf.TostInternetGonf
          ...
          <div className="card card-body border-div  shadow-sm h-100 bg-hover-card-st">
            <div className="row">
                ...
                <div className="text-center">
                  <div className="row mb-0">
                    <div className="col-4 align-self-center text-center">
                      <img
                        src="https://cdn.abyedh.com/images/system/gym/group.jpg"
                        className="rounded"
                        width="50px"
                      />
                    </div>
                    <div className="col-8 align-self-center text-start">
                      ...
                      <h5 className="mt-2 mb-0">
                        {" "}
                        <span className="bi bi-bookmarks-fill "></span>{" "}
                        <b>
                          {" "}
                          {loadingPage
                            ? props.data.GP_Genre
              ...
              <div className="col-6">
                <h5 className="mt-0 mb-0">
                  ...
                  <b>
                    {" "}
                    {loadingPage ? props.data.GP_Activite : SKLT.BarreSkl}{" "}
                  </b>{" "}
                </h5>
                <h5 className="mt-0 mb-0">
                  {" "}
                  <span className="bi bi-calendar "></span>{" "}
                  <b> {loadingPage ? props.data.GP_Temps : SKLT.BarreSkl} </b>{" "}
                </h5>
                <h5 className="mt-0 mb-0">
                  {" "}
                  <span className="bi bi-person "></span>{" "}
                  <b> {loadingPage ? props.data.GP_Cotch : SKLT.BarreSkl} </b>{" "}
                </h5>
              </div>
            </div>
            ...
            <div className="row mb-0">
              <div className="col-8 align-self-center text-center">
                <Button
                  className="rounded-pill bg-primary text-white mt-2"
                  size="mini"
                  icon
                  fluid
                  onClick={() => openEditModal(props.data, true)}
                >
              ...
              <div className="col-4 align-self-center text-center">
                <Button
                  className="rounded-pill bg-danger text-white mt-2"
                  size="mini"
                  icon
                  fluid
                  onClick={() => openEditModal(props.data, false)}
                >
                  <span className="d-none d-lg-inline"> </span>
                  <Icon name="trash" /> Supp
                </Button>
              </div>
            </div>
          </div>
          ...
          {loadingPage ? (
            <>
              {GrouListe.length == 0 ? (
                ...
                <div className="row">
                  {GrouListe.map((data, index) => (
                    <GrouCard key={index} data={data} />
                  ))}
                </div>
              )}
            </>
            ...
            <div className="card card-body border-div shadow-sm mb-4">
              <div className="p-1  mb-2">
                ...
                <Input
                  ...
                  className="w-100 border-0 shadow-sm rounded mb-1"
                  onChange={(e) =>
                    setGroupData({ ...groupData, GP_Name: e.target.value })
                  }
                />
              </div>
              <div className="p-1 mb-2">
                ...
                <Select
                  ...
                  value={groupData.GP_Genre}
                  onChange={(e, data) =>
                    setGroupData({ ...groupData, GP_Genre: data.value })
                  }
                />
              </div>
              <div className="p-1 mb-2">
                ...
                <Form>
                  <TextArea
                    ...
                    className="w-100 shadow-sm rounded mb-3"
                    value={groupData.GP_Activite}
                    onChange={(e) =>
                      setGroupData({
                        ...groupData,
                        GP_Activite: e.target.value,
                      })
                    }
                  />
                </Form>
              ...
              <div className="p-1 mb-2">
                ...
                <Input
                  ...
                  value={groupData.GP_Temps}
                  onChange={(e) =>
                    setGroupData({ ...groupData, GP_Temps: e.target.value })
                  }
                />
              </div>
              <div className="p-1 mb-2">
                ...
                <Input
                  ...
                  className="w-100 border-0 shadow-sm rounded mb-1"
                  onChange={(e) =>
                    setGroupData({ ...groupData, GP_Cotch: e.target.value })
                  }
                />
              </div>
              <div className="text-end mb-2">
                <Button
                  onClick={SaveGroupFunction}
                  className="text-end rounded-pill bg-system-btn "
                  disabled={saveBtnState}
                  positive
                >
                  {" "}
                  <Icon name="save outline" /> Enregistrer{" "}
                  <Loader
                    inverted
                    active={loaderState}
                    inline
