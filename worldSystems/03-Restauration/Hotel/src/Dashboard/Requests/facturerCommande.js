    ...
    <>
      <div className="card p-4 shadow-sm mb-2 border-div">
        ...
        <Input
          ...
          fluid
          className="mb-1"
          value={abonnemmentData.jour}
          onChange={(e) =>
            setAbonnemmentData({ ...abonnemmentData, jour: e.target.value })
          }
        />
        ...
        <Dropdown
          ...
          className="mb-1"
          onChange={(e, { value }) =>
            setAbonnemmentData({ ...abonnemmentData, Fournisseurs: value })
          }
        ...
        <Input
          ...
          fluid
          className="mb-1"
          value={abonnemmentData.Chauffeur}
          onChange={(e) =>
            setAbonnemmentData({
              ...abonnemmentData,
        ...
        <div className="row">
          <div className="col-6">
            ...
            <Input
              ...
              fluid
              className="mb-1"
              value={abonnemmentData.de}
              onChange={(e) =>
                setAbonnemmentData({ ...abonnemmentData, de: e.target.value })
              }
            />
          </div>
          <div className="col-6">
            ...
            <Input
              ...
              fluid
              className="mb-1"
              value={abonnemmentData.vers}
              onChange={(e) =>
                setAbonnemmentData({ ...abonnemmentData, vers: e.target.value })
              }
            />
          </div>
        </div>
        <div className="text-end mt-4">
          <Button
            className="rounded-pill text-secondary bg-system-btn"
            onClick={(e) => SaveAbonnementFunc()}
          >
            <Icon name="edit outline" /> Enregistrer Client
          </Button>
        </div>
      </div>
    </>
...
function FacturerCommande() {
  ...
  const panesRigth = [
    {
      ...
      render: () => (
        <MainDataCard
          ...
          SaveAbonnementFunc={SaveAbonnementFunc}
          forfaitListe={[]}
  ...
  useEffect(() => {
    axios
      .post(`${GConf.ApiLink}/reservation/info`, {
        PID: GConf.PID,
        CID: CID,
      })
      .then(function (response) {
        ...
        if (!response.data[0]) {
          ...
          setLoading(true);
          if (response.data[0].State != "W" && response.data[0].State != "S") {
            setBtnState(true);
          }
          if (response.data[0].State == "W") {
            UpdateState("S");
          }
        }
      })
  ...
  const SaveClientFunction = () => {
    ...
    setLS(true);
    axios
      .post(`${GConf.ApiLink}/membres/ajouter`, {
        PID: GConf.PID,
        clientD: {
          CIN: "",
          Name: commandeData.Name,
          Phone: commandeData.PhoneNum,
          Gouv: commandeData.BirthGouv,
          Deleg: commandeData.BirthDeleg,
          Adress: "",
          Releted_UID: commandeData.UID,
        },
      })
      .then(function (response) {
      ...
      .catch((error) => {
        if (error.request) {
          toast.error(
            <>
              <div>
                <h5>Probleme de Connextion</h5> Le client sera enregistrer sur
                votre ordinateur{" "}
              </div>
            </>,
            GConf.TostInternetGonf
  ...
  const SaveAbonnementFunc = () => {
    if (!abonnemmentData.CommandeID) {
      toast.error("Commande ID est Invalide !", GConf.TostErrorGonf);
    } else if (!abonnemmentData.totale) {
      toast.error("totale est Invalide !", GConf.TostErrorGonf);
    } else if (
      !abonnemmentData.articles ||
      abonnemmentData.articles.length == 0
    ) {
      ...
      setLS(true);
      axios
        .post(`${GConf.ApiLink}/commande/facturer`, {
          PID: GConf.PID,
          abonnemmentData: abonnemmentData,
        })
        .then(function (response) {
        ...
        .catch((error) => {
          if (error.request) {
            toast.error(
              <>
                <div>
                  <h5>Probleme de Connextion</h5> La Facture sera enregistrer
  ...
  const UpdateState = (stateBtn) => {
    axios
      .post(`${GConf.ApiLink}/commande/controle`, {
        PID: GConf.PID,
        RID: CID,
        state: stateBtn,
      })
      ...
      .catch((error) => {
        if (error.request) {
          toast.error(
            <>
              <div>
                <h5>Probleme de Connextion</h5> Impossible de modifier L'etat du
                commande{" "}
              </div>
            </>,
            GConf.TostInternetGonf
            ...
            <table class="table table-striped">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {loading
                      ? new Date(commandeData.Start_At)
                ...
                <tr>
                  <th scope="row">Age</th>
                  <td>{loading ? commandeData.User_Age : ""}</td>
                </tr>
                <tr>
                  <th scope="row">Genre</th>
                  <td>{loading ? commandeData.Ab_Genre : ""}</td>
                </tr>
                <tr>
        ...
        <div className="card card-body shadow-sm mb-2 mt-3 border-div">
          ...
          <div className="row mb-2">
            <div className="text-center mb-3">
              <img
                src={`https://cdn.abyedh.com/images/p_pic/05.gif`}
            ...
            <div className="col-12 col-lg-6 mb-3">
              {" "}
              Nom : {loading ? commandeData.Name : ""}
              <Input
                icon="add user"
                onKeyPress={(event) => OnKeyPressFunc(event)}
                list="clientList"
                placeholder={loading ? commandeData.Name : ""}
                value={abonnemmentData.Name}
                onBlur={(e) =>
                  setAbonnemmentData({
                    ...abonnemmentData,
                    Name: e.target.value,
            ...
            <div className="col-12 col-lg-6 mb-3">
              {" "}
              Phone : {loading ? commandeData.PhoneNum : ""}
              <Input
                icon="add user"
                onKeyPress={(event) => OnKeyPressFunc(event)}
                list="clientList"
                placeholder={loading ? commandeData.PhoneNum : ""}
                value={abonnemmentData.PhoneNum}
                onBlur={(e) =>
                  setAbonnemmentData({
                    ...abonnemmentData,
            ...
            <div className="col-12 col-lg-6 mb-3">
              {" "}
              Gouv : {loading ? commandeData.BirthGouv : ""}
              <Select
                placeholder=" Gouvernorat"
                fluid
                className="mb-2"
                options={TunMap.Gouv}
                value={abonnemmentData.Gouv}
            ...
            <div className="col-12 col-lg-6 mb-3">
              ...
              <Select
                ...
                fluid
                value={abonnemmentData.Deleg}
                options={delegList}
                onChange={(e, { value }) =>
                  setAbonnemmentData({ ...abonnemmentData, Deleg: value })
                }
              />
            </div>
          ...
          <div className="text-end">
            <Button
              className="rounded-pill text-secondary btn-imprimer"
              size="mini"
              disabled={commandeData.Releted_UID}
              onClick={(e) => SaveClientFunction()}
            >
              <Icon name="edit outline" /> Enregistrer Client
            </Button>
          </div>
