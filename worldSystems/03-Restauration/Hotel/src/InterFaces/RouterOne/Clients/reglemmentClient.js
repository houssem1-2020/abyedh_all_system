        ...
        <div className="card p-3 mb-2 shadow-sm ">
          <div className="row">
            ...
            <div className="col-1">
              <Button
                icon
                className="rounded-circle p-2 bg-system-btn"
                onClick={() => GetArticleDataByAID(props.data.P_Code)}
              >
                {" "}
                <Icon name="plus" />
              </Button>
  ...
  return (
    ...
    >
        ...
        <div className="row">
          <div className="col-9">
            <Input
              ...
              type="text"
              icon="braille"
              onBlur={(e) => setRechercheKey(e.target.value)}
              iconPosition="left"
              placeholder=" "
              fluid
              className="mb-1"
          ...
          <div className="col-3 align-self-center">
            <Button
              ...
              onClick={() => RechercheFunction()}
            >
              {" "}
              <Icon name="save" /> Recherche{" "}
            </Button>
          </div>
        </div>
      </Modal.Header>
      <Modal.Content scrolling className="spesific-commp">
        <div className="p-1">
          {rechercheList.map((data, index) => (
            <RechercheListeCard key={index} data={data} />
          ))}
        </div>
      </Modal.Content>
      <Modal.Actions>
        ...
        >
          {" "}
          <span className="bi bi-x"></span> Fermer
        </Button>
      </Modal.Actions>
    </Modal>
  );
...
const SelectModalCard = ({
  ...
  return (
    ...
    >
      ...
      <Modal.Content scrolling className="spesific-commp">
        <div className="row">
          {fastArticleList.map((data, index) => (
            <SelectedListeCard key={index} data={data} />
          ))}
        </div>
      </Modal.Content>
      <Modal.Actions>
...
const SelectClientFideliteCard = ({
  clientFidele,
  setClientFidelle,
  clientList,
}) => {
    ...
    <>
      <div className="ccard-body mb-2">
        ...
        <Input
          ...
          list="clientList"
          size="large"
          onKeyPress={(e) => SetClientFidelite(e)}
          iconPosition="left"
          placeholder="Client Fidelite"
          fluid
          className="mb-1"
        />
...
function ReglemmentClient() {
  ...
  let Offline = JSON.parse(
    localStorage.getItem(`${OneGConf.routerTagName}_Offline`)
  );
  ...
  const [factureD, setFactureD] = useState({
    client: "PASSAGER",
    Caisse: CaisseID,
  ...
  const panes = [
    {
      menuItem: {
        ...
        content: "Plats ",
        className: `p-4 ${
          OneGConf.themeMode == "dark"
            ? "bg-dark-theme-1 text-white"
            : "bg-ligth-theme-1"
        } `,
      },
      render: () => (
    ...
    },
    {
      ...
      render: () => (
        <>
          <TotaleCard />
          <div
            className="spesific-commp"
            style={{ height: "70vh", overflow: "scroll", overflowX: "hidden" }}
          >
            {factureD.articles.map((val) => (
              <ArticleListCard key={val.id} dataA={val} />
            ))}
            <br />
          </div>
        </>
  ...
  useEffect(() => {
    axios
      .post(`${GConf.ApiRouterOneLink}/nv/stock`, {
        forPID: GConf.PID,
        factureD: factureD,
      })
      .then(function (response) {
        ...
        setSelectedArticleList(
          response.data.filter(
            (article) => article.Genre == response.data[0].Genre
          )
        );
        setLoadingP(false);
      })
    ...
    axios
      .post(`${GConf.ApiLink}/commande/info`, {
        PID: GConf.PID,
        CID: CID,
      })
      .then(function (response) {
      ...
      .catch((error) => {
        if (error.request) {
          toast.error(
            <>
              <div>
                <h5>Probleme de Connextion</h5> Impossible de charger la
                commande{" "}
              </div>
            </>,
            GConf.TostInternetGonf
  ...
  const SearchForCommande = (commandeID) => {
    if (!commandeID) {
      toast.error("Commande ID Invalide !", GConf.TostErrorGonf);
    } else {
      setLoadingP(true);
      axios
        .post(`${GConf.ApiRouterOneLink}/commande/search`, {
        ...
        .catch((error) => {
          if (error.request) {
            toast.error(
              <>
                <div>
                  <h5>Probleme de Connextion</h5> La facture sera enregisstrer
                  sur votre appareil{" "}
                </div>
              </>,
              GConf.TostInternetGonf
  ...
  const SetSelectedListeFunc = (selectedGenre) => {
    const selectedListe = articleList.filter(
      (article) => article.Genre == selectedGenre
    );
    setSelectedArticleList(selectedListe);
  };
  const AddArticleToList = () => {
    if (!articleNow.P_Code) {
      toast.error("Code à barre Invalide !", GConf.TostErrorGonf);
    } else if (!articleNow.Name || articleNow.Name == "") {
      toast.error("Name Invalide !", GConf.TostErrorGonf);
    } else if (!articleNow.Qte || articleNow.Qte == "") {
      toast.error("Quantite Invalide !", GConf.TostErrorGonf);
    } else {
      const searchObject = factureD.articles.find(
        (article) => article.P_Code == articleNow.P_Code
      );
      if (searchObject) {
        let IndexOfArticle = factureD.articles.findIndex(
          (article) => article.P_Code == articleNow.P_Code
        );
        factureD.articles[IndexOfArticle].Qte =
          factureD.articles[IndexOfArticle].Qte + parseInt(articleNow.Qte);
        factureD.articles[IndexOfArticle].PU = (
          factureD.articles[IndexOfArticle].Qte *
          factureD.articles[IndexOfArticle].Prix
        ).toFixed(3);
        setArticleNow([{}]);
        ...
        let arrayToAdd = {
          id: factureD.articles.length + 1,
          P_Code: articleNow.P_Code,
          Name: articleNow.Name,
          Prix: articleNow.Prix_piece,
          Qte: parseInt(articleNow.Qte),
          PU: prix_u,
        };
  ...
  const DeleteFromUpdateList = (value) => {
    const searchObject = factureD.articles.findIndex(
      (article) => article.P_Code == value
    );
  ...
  const SaveFacture = () => {
    if (!factureD.client) {
      toast.error("Client est Invalide !", GConf.TostErrorGonf);
    } else if (!factureD.jour) {
      toast.error("Date est Invalide !", GConf.TostErrorGonf);
    } else if (!factureD.totale) {
      toast.error("totale est Invalide !", GConf.TostErrorGonf);
    } else if (!factureD.articles || factureD.articles.length == 0) {
      toast.error("article list est Invalide !", GConf.TostErrorGonf);
    } else {
      setLoadingP(true);
      setLS(true);
      setSaveBtnState(true);
      axios
        .post(`${GConf.ApiRouterOneLink}/nv/ajouter`, {
          forPID: OneGConf.forPID.PID,
          factureD: factureD,
        })
        .then(function (response) {
        ...
        .catch((error) => {
          if (error.request) {
            ...
            localStorage.setItem(
              `${OneGConf.routerTagName}_Offline`,
  ...
  const GetArticleDataByAID = (barCodeValue) => {
    if (peseState) {
      ...
      const searchObjectFirst = articleList.find(
        (article) => article.P_Code == barreCode
      );
      if (searchObjectFirst) {
        ...
        if (searchObject) {
          ...
          factureD.articles[IndexOfArticle].Qte = (
            parseFloat(factureD.articles[IndexOfArticle].Qte) + parseFloat(qte)
          ).toFixed(3);
          factureD.articles[IndexOfArticle].PU = (
            factureD.articles[IndexOfArticle].Qte *
            factureD.articles[IndexOfArticle].Prix
          ...
          let arrayToAdd = {
            id: factureD.articles.length + 1,
            P_Code: barreCode,
            Name: searchObjectFirst.Name,
            Prix: searchObjectFirst.Prix_vente,
            Qte: qte,
            PU: searchObjectFirst.Prix_vente * qte,
          };
          factureD.articles.push(arrayToAdd);
          setArticleNow([]);
      ...
      const searchObjectFirst = articleList.find(
        (article) => article.P_Code == barCodeValue
      );
      if (searchObjectFirst) {
        const searchObject = factureD.articles.find(
          (article) => article.P_Code == barCodeValue
        ...
        if (searchObject) {
          ...
          factureD.articles[IndexOfArticle].Qte =
            factureD.articles[IndexOfArticle].Qte + 1;
          factureD.articles[IndexOfArticle].PU = (
            factureD.articles[IndexOfArticle].Qte *
            factureD.articles[IndexOfArticle].Prix
          ).toFixed(3);
          ...
          let arrayToAdd = {
            ...
            Prix: searchObjectFirst.Prix_vente,
            Qte: 1,
            PU: searchObjectFirst.Prix_vente,
          };
          factureD.articles.push(arrayToAdd);
          setArticleNow([]);
  ...
  const AddMoreQte = (valueCode, qte) => {
    let IndexOfArticle = factureD.articles.findIndex(
      (article) => article.P_Code == valueCode
    );
  ...
  const RechercheFunction = () => {
    let values = articleList.filter((element) =>
      element.Name.toLowerCase().includes(rechercheKey.toLowerCase())
    );
  ...
  const SavePaymmentCache = () => {
    if (!gettedFID || gettedFID == "") {
      toast.error("Une Erreur s'est Produit !", GConf.TostErrorGonf);
    } else if (keyaPaymment == "") {
      toast.error("Solde Insufisante !", GConf.TostErrorGonf);
    } else if (JSON.parse(factureD.totale) > JSON.parse(keyaPaymment)) {
      toast.error("Solde Insufisante !", GConf.TostErrorGonf);
    } else {
      setLoadingP(true);
      axios
        .post(`${GConf.ApiRouterOneLink}/nv/payee/espece`, {
          forPID: CaisseID,
          espece: keyaPaymment,
          fid: gettedFID,
        })
        ...
        .catch((error) => {
          if (error.request) {
            toast.error(
              <>
                <div>
                  <h5>Probleme de Connextion</h5> Paymment à été enregistre
                  localmment{" "}
                </div>
              </>,
              GConf.TostInternetGonf
  ...
  const CalculateBons = (genre) => {
    if (genre == "Plus") {
      return {
        qte: Math.trunc(factureD.totale / selectedBon) + 1,
        valeur: selectedBon,
      };
    } else {
      return {
        qte: Math.trunc(factureD.totale / selectedBon),
        valeur: selectedBon,
      };
    }
  };
  const SavePaymmentBons = (genre) => {
    ...
    // else if (JSON.parse(factureD.totale) > JSON.parse(keyaPaymment)) {toast.error("Solde Insufisante !", GConf.TostErrorGonf)}
      ...
      setLoadingP(true);
      axios
        .post(`${GConf.ApiRouterOneLink}/nv/payee/bons`, {
          forPID: CaisseID,
        ...
        .catch((error) => {
          if (error.request) {
            ...
            Offline.factureToSave[gettedFID].Paye_Bons = JSON.stringify(
              CalculateBons(genre)
            );
            Offline.factureToSave[gettedFID].State = "Payee";
            localStorage.setItem(
  ...
  const SaveClientCredit = (genre) => {
    if (!gettedFID || gettedFID == "") {
      toast.error("Une Erreur s'est Produit !", GConf.TostErrorGonf);
    } else if (clientCredit == "") {
      toast.error("Solde Insufisante !", GConf.TostErrorGonf);
    } else {
      setLoadingP(true);
      axios
        .post(`${GConf.ApiRouterOneLink}/nv/client/credit`, {
          forPID: CaisseID,
  ...
  const SaveClientFidelle = () => {
    if (!gettedFID || gettedFID == "") {
      toast.error("Une Erreur s'est Produit !", GConf.TostErrorGonf);
    } else if (clientFidele == "") {
      toast.error("Solde Insufisante !", GConf.TostErrorGonf);
    } else {
      setLoadingP(true);
      axios
        .post(`${GConf.ApiRouterOneLink}/nv/client/fidelite`, {
          forPID: CaisseID,
      ...
      <>
        <div className="row mb-0">
          <div className="col-4">
            <Button
              ...
              fluid
              style={{ height: "50px" }}
              className="border-div bg-system-btn"
              onClick={() => SearchForCommande(keyBordI)}
            >
              {" "}
              <Icon name="check" /> Rechercher{" "}
            </Button>
          </div>
          <div className="col-8">
            <Button
            ...
            >
              {" "}
              <Icon name="save" /> Valideé Pour PASSAGER{" "}
              <Loader
                inverted
                active={loaderState}
                inline
          ...
          <div className="col-3 d-none">
            <Button
              ...
              fluid
              style={{ height: "40px" }}
              className="rounded-pill bg-system-btn mb-3"
              onClick={() => setModaT(true)}
            >
              {" "}
              <Icon name="search" /> Trouver{" "}
            </Button>
          </div>
        </div>
        <div className="row mt-0">
              ...
              {scanResultCommande ? (
                <QrReader
                  ...
                  onResult={(result, error) => {
                    if (!!result) {
                      SearchForCommande(result.text);
                      setScanResultCommande(false);
                    }
                    if (!!error) {
                      console.log(error);
                    }
                ...
                <div className="text-center mt-4">
                  <Button onClick={() => setScanResultCommande(true)}>
          ...
          <div className="col-12 col-md-7 align-self-center">
            <h2>
              <span className="bi bi-keyboard"></span> : {keyBordI}
            </h2>
            <ClavierCard />
          </div>
        </div>
  ...
  const ArticleListCard = (props) => {
    return (
      <>
        <div
          className={`card shadow-sm p-2 mb-1 me-1 rounded-pill  ${
            OneGConf.themeMode == "dark"
              ? "bg-dark-theme-4 text-white"
              : "bg-ligth-theme-4"
          } `}
        >
          <div className="row p-0">
            <div className="col-1 align-self-center text-start">
              <Button
                disabled={saveBtnState}
                onClick={() => DeleteFromUpdateList(props.dataA.P_Code)}
                icon="times"
                className="rounded-circle p-2 text-white bg-danger"
              ></Button>
            </div>
            <div className="col-9 text-start align-self-center ps-4">
              <div>{props.dataA.Name}</div>
              <b className="text-blod">{props.dataA.Qte}</b> x{" "}
              {props.dataA.Prix.toFixed(3)} ={" "}
              <span className="text-warning">
                {parseFloat(props.dataA.PU).toFixed(3)}
              </span>
            </div>
            <div className="col-2 align-self-center m-0">
              <Button
                disabled={saveBtnState}
                onClick={() => AddMoreQte(props.dataA.P_Code, keyBordI)}
                icon="plus cart"
                className="rounded-circle p-2 text-white bg-info"
              ></Button>
            </div>
          </div>
        </div>
  ...
  const TotaleCard = () => {
    return (
      <>
        <div
          className={`card card-body shadow-sm mb-2 sticky-top border-div ${
            OneGConf.themeMode == "dark"
              ? "bg-dark-theme-3 text-white"
              : "bg-ligth-theme-3"
          } `}
          style={{ top: "70px" }}
        >
          <div className="row">
            <div className="col-9 align-self-center text-start">
              <h1>Totale : {factureD.totale}</h1>
            </div>
            <div className="col-3 align-self-center text-center">
              <h5 className="mb-0">{factureD.articles.length}</h5> articles
            </div>
          </div>
        </div>
      </>
          ...
          <Button
            className={`shadow-sm ${
              props.bg == true
                ...
                : OneGConf.themeMode == "dark"
                ? "bg-dark-theme-3 text-white border-dark"
                : "bg-white"
            } border mb-1 border-div `}
            style={{ width: "100%", height: "60px", backgroundColor: "red" }}
            onClick={(e) => BtnClicked(props.value)}
          >
            <h2>{props.value}</h2>
          </Button>
        </>
  ...
  const PlatsListe = () => {
    const GenreListeCard = (props) => {
      return (
        <>
          <div
            className={`card card-body mb-1  text-danger text-center btn ${
              OneGConf.themeMode == "dark"
                ? "bg-dark-theme-3 text-white"
                : "bg-ligth-theme-3"
            }`}
            onClick={() => SetSelectedListeFunc(props.text)}
          >
            {" "}
            <h3> {props.text} </h3>
          </div>
      ...
      return (
        <>
          <div className="col-6 col-lg-3 mb-1">
            <div
              className={`card p-3 text-secondary h-100 btn ${
                OneGConf.themeMode == "dark"
                  ? "bg-dark-theme-4 text-white"
                  : " "
              }`}
            ...
            >
              <div className="row">
                <div className="col-12 text-end">
                  <b>{props.data.Prix_vente.toFixed(3)}</b>
                </div>
                <div className="col-12">
                  <b>
                    <img
                      src={`https://cdn.abyedh.com/images/system/Resto/${props.data.Photo_Path}`}
                      width="60px"
                      height="60px"
                    />
                  </b>
    ...
    return (
      <>
        <div className="row">
          <div className="col-4">
            <Button
              disabled={saveBtnState && !payBtnState}
              fluid
              style={{ height: "40px" }}
              className="rounded-pill bg-system-btn mb-3"
              onClick={() => NewTicket()}
          ...
          <div className="col-2">
            <Button
              ...
              style={{ height: "40px" }}
              className="rounded-pill bg-system-btn mb-3"
              onClick={() => PrintFunction("openCaisse")}
            >
              {" "}
              <Icon name="lock open" />{" "}
            </Button>
          ...
          <div className="col-1">
            <Button
              ...
              onClick={() => sethowKeyBord(!showKeyBoerd)}
            >
              {" "}
              <Icon name="keyboard" />{" "}
            </Button>
        ...
        <div className="row">
          <div
            className="col-12 col-lg-3 spesific-commp-2"
            style={{ height: "58vh", overflow: "scroll", overflowX: "hidden" }}
          >
            {genreListe.map((data, index) => (
              <GenreListeCard key={index} text={data} />
            ))}
          </div>
          <div className="col-12 col-lg-9">
            <div className="row">
              {showKeyBoerd ? (
                ...
                <>
                  {selectedArticleList.map((data, index) => (
                    <ArticleItemCard key={index} data={data} />
                  ))}
                </>
              )}
            </div>
      ...
      return (
        <>
          <Button
            ...
            onClick={(e) => PaymmentBtnClicked(props.value)}
          >
            <h2>{props.value}</h2>
          </Button>
        </>
          ...
          <div className="col-12 col-md-5">
            <div className="mb-2">
              <Button
                ...
                style={{ height: "40px" }}
                className="rounded-pill bg-system-btn mb-3"
                onClick={() => setModaPS(true)}
              >
                {" "}
                <Icon name="search" /> Sahcet & Carte de Recharge{" "}
              </Button>

              <h3 className="mb-2">Totale : {factureD.totale}</h3>
              <h3 className="mb-2 mt-1">
                Reste : {(keyaPaymment - factureD.totale).toFixed(3)}
              </h3>
              <div className="row">
                <div className="col-9">
                  <Input
                    ...
                    icon="money"
                    value={keyaPaymment}
                    onKeyPress={(e) => EnterKyPressed(e)}
                    autoFocus={true}
                    onChange={(e) => {
                      setKeyPaymment(Number(e.target.value));
                    }}
                    iconPosition="left"
                    placeholder=" "
                    fluid
                ...
                <div className="col-3 ms-0 me-0">
                  {" "}
                  <Button
                    disabled={false}
                    fluid
                    className="border-div   text-white"
                    style={{
                      height: "50px",
                      backgroundColor:
                        OneGConf.themeMode == "dark" ? "#96999e" : "#343536",
                    }}
                    onClick={() => setKeyPaymment("")}
                  >
                    {" "}
                    <h2>C</h2>{" "}
                  </Button>{" "}
              ...
              <div className="row">
                <div className="col-12">
                  <Button
                    disabled={payBtnState || !saveBtnState}
                    fluid
                    style={{ height: "50px" }}
                    className="border-div bg-system-btn"
                    onClick={() => SavePaymmentCache()}
                  >
                    {" "}
                    <Icon name="check" /> Payeé
                  </Button>
                </div>
              </div>
              <h5>Imprimer</h5>
              <div className="row">
                <div className="col-9">
                  <Button
                    ...
                    style={{ height: "40px" }}
                    className="rounded-pill bg-system-btn mb-3"
                    onClick={() => PrintFunction("printFacture")}
                  >
                    {" "}
      ...
      return (
        <>
          <Button
            ...
            style={{ width: "100%", height: "60px" }}
            onClick={(e) => BonsBtnClicked(props.value)}
          >
            <h2>{props.value}</h2>
          </Button>
        </>
      ...
      <>
        <div className="row">
          <div className="col">
            <Button
              fluid
              className={`shadow-sm  border mb-1 border-div ${
                selectedPoucentage == 5 ? "bg-warning border-0 text-danger" : ""
              }`}
              style={{ height: "60px" }}
              onClick={(e) => BonSlected(0)}
            >
              <h5>
          ...
          <div className="col">
            <Button
              fluid
              className={`shadow-sm  border mb-1 border-div ${
                selectedPoucentage == 8 ? "bg-warning border-0 text-danger" : ""
              } `}
              style={{ height: "60px" }}
              onClick={(e) => BonSlected(8)}
            >
              <h5>8%</h5>
            </Button>
          </div>
          <div className="col">
            <Button
              fluid
              className={`shadow-sm  border mb-1 border-div ${
                selectedPoucentage == 10
                  ? "bg-warning border-0 text-danger"
                  : ""
              } `}
              style={{ height: "60px" }}
              onClick={(e) => BonSlected(10)}
            >
              <h5>10%</h5>
            </Button>
          </div>
          <div className="col">
            <Button
              fluid
              className={`shadow-sm  border mb-1 border-div ${
                selectedPoucentage == 12
                  ? "bg-warning border-0 text-danger"
                  : ""
              } `}
              style={{ height: "60px" }}
              onClick={(e) => BonSlected(12)}
            >
              <h5>12%</h5>
            </Button>
          </div>
          <div className="col">
            <Button
              fluid
              className={`shadow-sm  border mb-1 border-div ${
                selectedPoucentage == 15
                  ? "bg-warning border-0 text-danger"
                  : ""
              } `}
              style={{ height: "60px" }}
              onClick={(e) => BonSlected(15)}
            >
              <h5>15%</h5>
            </Button>
          </div>
  ...
  const ListeDesBonsCard = () => {
      ...
      return (
        <>
          <Button
            ...
            style={{ width: "100%", height: "60px" }}
            onClick={(e) => BonSlected(props.value)}
          >
            <h2 className="mb-0">{props.text}</h2>{" "}
            <small className="mt-0">{props.value.toFixed(3)}</small>
          </Button>
        </>
      );
    };
    return (
      <>
        <div
          className="row  spesific-commp-2"
          style={{ height: "70vh", overflow: "scroll", overflowX: "hidden" }}
        >
          {OneGConf.listeDesBons.map((data, index) => (
            <div key={index} className="col-4 p-2">
              <BtnCard
                value={data.value * (1 - data.pourcentage)}
                text={data.text}
              />
            </div>
          ))}
          ...
          <div className="col-12 col-md-5">
            <div className="row mb-3">
              <div className="col-9 align-self-center">
                <h2>
                  <span className="bi bi-keyboard"></span> :{" "}
                  {keyBonsValue == ""
                    ? "0"
                    : parseFloat(keyBonsValue).toFixed(3)}{" "}
                  <span className="bi bi-arrow-left-right"></span>{" "}
                  {selectedBon == 1 ? "0" : selectedBon.toFixed(3)}{" "}
                </h2>{" "}
              </div>
              <div className="col-3 ms-0 me-0">
                {" "}
            ...
            <div className="row">
              <div className="col-6">
                <div className="mb-2">
                  ...
                  <h3 className="mb-2">
                    Nombre : {Math.trunc(factureD.totale / selectedBon) + 1}
                  </h3>
                  <h3 className="mb-2 mt-1">
                    Tot:{" "}
                    {(
                      (Math.trunc(factureD.totale / selectedBon) + 1) *
                      selectedBon
                    ).toFixed(3)}
                  </h3>
                  <h3 className="mb-2 mt-1">
                  ...
                  <div className="row">
                    <div className="col-12">
                      <Button
                        ...
                        fluid
                        style={{ height: "50px" }}
                        className="border-div bg-success text-white"
                        onClick={() => SavePaymmentBons("Plus")}
                      >
                        {" "}
                        <Icon name="check" /> Payeé
                      </Button>
              ...
              <div className="col-6">
                <div className="mb-2">
                  ...
                  <h3 className="mb-2">
                    Nombre : {Math.trunc(factureD.totale / selectedBon)}
                  </h3>
                  <h3 className="mb-2 mt-1">
                    Tot:{" "}
                    {(
                      Math.trunc(factureD.totale / selectedBon) * selectedBon
                    ).toFixed(3)}
                  </h3>
                  <h3 className="mb-2 mt-1">
                    T :{" "}
                  ...
                  <div className="row">
                    <div className="col-12">
                      <Button
                        ...
                        fluid
                        style={{ height: "50px" }}
                        className="border-div bg-danger text-white"
                        onClick={() => SavePaymmentBons("Moins")}
                      >
                        {" "}
                        <Icon name="check" /> Payeé
                      </Button>
  ...
  const SelectClientCard = () => {
    const ClientCardList = (props) => {
      return (
        <>
          <div
            className="card p-3 mb-2 shadow-sm rounded"
            onClick={() => setClientFidelle(props.data)}
          >
            <div className="row">
              <div className="col-4">{props.data.CL_ID}</div>
              <div className="col-6">{props.data.CL_Name}</div>
              <div className="col-2 align-self-center">
                <Button icon className="p-2 btn-system-bg">
                  <Icon name="check" />
                </Button>
              </div>
            </div>
        ...
        <div className="row">
          <div className="col-3">
            <Modal
              ...
              dimmer="blurring"
              trigger={
                <Button className=" bg-white border" size="large" fluid>
                  {" "}
            ...
            >
              <Modal.Content
                scrolling
                className="d-block spesific-commp"
                style={{ height: "100vh" }}
              >
                <div className="p-2 text-start">
                  {clientList.map((data, index) => (
                    <ClientCardList key={index} data={data} />
                  ))}
                </div>
              </Modal.Content>
            </Modal>
          ...
          <div className="col-9">
            <SelectClientFideliteCard
              clientFidele={clientFidele}
              setClientFidelle={setClientFidelle}
      ...
      <>
        <div className="row mb-2">
          <div className="col-12 col-lg-6 mb-3 d-none">
            ...
            <h5>Enregistrer</h5>
            <Button
              disabled={payBtnState || !saveBtnState || !clientCredit}
              className="rounded-pill bg-system-btn mb-3"
              fluid
              style={{ height: "50px" }}
              onClick={() => SaveClientCredit()}
            >
              <Icon name="save" /> Enregistrer Credit
              <Loader
                inverted
                active={loaderState}
                inline
            ...
            {scanResult ? (
              <QrReader
                ...
                onResult={(result, error) => {
                  if (!!result) {
                    setClientFidelle(
                      clientList.find((obj) => obj.CL_ID == result.text)
                    );
                    setScanResult(false);
                  }
                  if (!!error) {
              ...
              <div className="text-center mt-4">
                <Button onClick={() => setScanResult(true)}>
                  Cliquer Pour Scanner
                </Button>
              </div>
            )}
          ...
          <div className="col-12 col-lg-6">
            ...
            <Button
              disabled={
                !payBtnState || !saveBtnState || !clientFidele || fideliteState
              }
              className="rounded-pill bg-system-btn mb-3"
              fluid
              style={{ height: "50px" }}
              onClick={() => SaveClientFidelle()}
            >
              <Icon name="save" /> Enregistrer Fidelité
              <Loader
                inverted
                active={loaderState}
                inline
                  ...
                  <>
                    {articleL.map((artData, index) => (
                      <tr
                        key={index}
                        className={
                          artData.selected ? "text-decoration-line-through" : ""
                        }
                      >
                        ...
                        <td>
                          <Button
                            size="mini"
                            disabled={artData.selected}
                            className="p-1"
                            onClick={() => ThisSelected(index)}
                            icon="check"
                          />{" "}
                        </td>
                      </tr>
  ...
  return (
    <>
      <div
        className={`${
          OneGConf.themeMode == "dark" ? "bg-dark-theme-2" : "bg-ligth-theme-2"
        }`}
      ...
      <SearchModalCard
        modalS={modalS}
        setModalS={setModalS}
        rechercheList={rechercheList}
        saveBtnState={saveBtnState}
        rechercheKey={rechercheKey}
        setRechercheKey={setRechercheKey}
        RechercheFunction={RechercheFunction}
        GetArticleDataByAID={GetArticleDataByAID}
      />
      <SelectModalCard
        modalT={modalT}
        setModaT={setModaT}
        fastArticleList={fastArticleList}
        saveBtnState={saveBtnState}
        rechercheKey={rechercheKey}
        setRechercheKey={setRechercheKey}
        RechercheFunction={RechercheFunction}
      ...
      <FrameForPrint
        frameId="printFacture"
        src={`/Pr/caisse/facture/${toPrintFID}`}
      />
