...
function RequestInfo() {
    ...
    const UpdateRequestState = (stateBtn,dataGenre,selectedData,saveNotif,actionName) =>{
        axios.post(`${GConf.SharedApi}/request/controle`, {
            ...
            TAG : 'restaurant',
            RID: CID,
            genreTag : 'restaurant_commande',
            state: stateBtn,
            data: selectedData,
            dataGenre: dataGenre,
            saveNotif : saveNotif,
            actionName : `restaurant_${actionName}`,
          })
          .then(function (response) { 
    ...
    const StateCard = ({ status }) => {
        ...
        const statusCard = React.useCallback(() => {
          switch(status) {
            ...
            case 'EC': return <StateCard color='retarder' text={t(`menuTabs.rdvPage.tabsTtems.retarde`)} />;
            case 'LV': return <StateCard color='rederecter' text={t(`menuTabs.rdvPage.tabsTtems.redirecte`)} />;
        ...
        return(<>
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className='row '>
                        ...
                        <div className='col-6 mb-2'>
                            <Button  className={`rounded-pill ${reqState == 'EC' ? 'bg-retarder text-white' : 'bg-gray text-retarder'}`}  fluid onClick={ () => UpdateRequestState('EC',false,false,true,false) }><Icon name='hourglass half' /> {t('menuTabs.rdvPage.rdvInfoCardData.controlBtnCard.encours')} </Button>
                        </div>
                        <div className='col-6 mb-2'>
                            <Button  className={`rounded-pill ${reqState == 'LV' ? 'bg-rederecter text-white' : 'bg-gray text-rederecter'}`} fluid onClick={ () => UpdateRequestState('LV',false,false,true,false) }><Icon name='motorcycle' /> {t('menuTabs.rdvPage.rdvInfoCardData.controlBtnCard.Livree')} </Button>
                        </div>
                    ...
                    <div className="table-responsive">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{loading ? <> {requestData.Comm_Genre} {requestData.Comm_Genre != 'Importee' ? requestData.Table_Num  : ''} </>: ''} </td>
                                </tr>
                                <tr>
