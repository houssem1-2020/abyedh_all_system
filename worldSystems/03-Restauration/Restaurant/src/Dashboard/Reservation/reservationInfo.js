...
function RequestInfo() {
    ...
    const UpdateRequestState = (stateBtn,dataGenre,selectedData,saveNotif,actionName) =>{
        axios.post(`${GConf.SharedApi}/request/controle`, {
            ...
            TAG : 'restaurant',
            RID: CID,
            genreTag : 'restaurant_reservation',
            state: stateBtn,
            data: selectedData,
            dataGenre: dataGenre,
            saveNotif : saveNotif,
    ...
    const StateCard = ({ status }) => {
        ...
        const statusCard = React.useCallback(() => {
          switch(status) {
            ...
            case 'PR': return <StateCard color='rederecter' text={t(`menuTabs.rdvPage.tabsTtems.redirecte`)} />;
        ...
        return(<>
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className='row '>
                        ...
                        <div className='col-6 mb-2'>
                            <Button  className={`rounded-pill ${reqState == 'PR' ? 'bg-rederecter text-white' : 'bg-gray text-rederecter'}`} fluid onClick={ () => UpdateRequestState('PR',false,false,true,false) }><Icon name='refresh' /> {t('menuTabs.rdvPage.rdvInfoCardData.controlBtnCard.pretBtn')} </Button>
                        </div>
                    ...
                    <div className="table-responsive">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{loading ? new Date(requestData.Wanted_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ) : ''}</td>
                                </tr>
                                <tr>
                                    <td className='col-5 text-secondary'><span className='bi bi-clock me-2'></span> Temps </td>
                                    <td>{loading ? requestData.Wanted_Time : ''}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    <td className='col-5 text-secondary'><span className='bi bi-calendar-check me-2'></span> Tables </td>
