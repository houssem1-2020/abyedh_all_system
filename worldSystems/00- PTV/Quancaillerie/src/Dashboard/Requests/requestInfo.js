...
function RequestInfo() {
    ...
    const UpdateRequestState = (stateBtn,dataGenre,selectedData,saveNotif,actionName) =>{
        axios.post(`${GConf.SharedApi}/request/controle`, {
            ...
            TAG : 'magazin',
            RID: CID,
            genreTag : 'magazin_commande',
            state: stateBtn,
            data: selectedData,
            dataGenre: dataGenre,
            saveNotif : saveNotif,
            actionName : `magazin_${actionName}`,
          })
          .then(function (response) { 
    ...
    const StateCard = ({ status }) => {
        ...
        const statusCard = React.useCallback(() => {
          switch(status) {
            ...
            case 'LV': return <StateCard color='retarder' text={t(`menuTabs.rdvPage.tabsTtems.livree`)} />;
                ...
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className='row '>
                        ...
                        <div className='col-12 mb-2'>
                            <Button  className={`rounded-pill ${reqState == 'LV' ? 'bg-retarder text-white' : 'bg-gray text-retarder'}`}  fluid onClick={ () => UpdateRequestState('LV',false,false,true,false) }><Icon name='truck' /> {t('menuTabs.rdvPage.rdvInfoCardData.controlBtnCard.livreeBtn')} </Button>
                        </div>
                        {/* <div className='col-6 mb-2'>
