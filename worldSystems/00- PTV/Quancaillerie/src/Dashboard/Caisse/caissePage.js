...
import { Button , Divider, Icon, Modal, Placeholder, Statistic} from 'semantic-ui-react';
...
function CaissePage() {
      ...
      return(<>
          <div className='col-12 col-md-12 mb-3'>
            <div className='card card-body border-div  shadow-sm h-100 bg-hover-card-st'>
                  <div className='row'>
                    <div className='col-1 align-self-center'><img src="https://cdn.abyedh.com/images/system/Magazin/caisse.png" className="rounded-circle" width="50px" /></div>
                    <div className='col-3 align-self-center'><h4  ><a  className='data-link-modal'  onClick={() => openEditModal(props.data,true)} ><b> {loadingPage ? props.data.CA_Name : SKLT.BarreSkl } </b></a> </h4> </div>
                    <div className='col-3 align-self-center'><h4  >{loadingPage ? props.data.T_Name : SKLT.BarreSkl }</h4> </div>
                    <div className='col-3 align-self-center'>{loadingPage ? <><span className="bi bi-box2 "></span> :  {props.data.Recette}  | <span className="bi bi-coin"></span> : {parseInt(props.data.Caisse_Fond).toFixed(3)}  </>: SKLT.BarreSkl}</div>
                    <div className='col-2 align-self-center text-end'><Button className='rounded-pill bg-system-btn mt-2' size='mini' onClick={ (e) => NavigateFunction(`/S/ca/info/${props.data.C_ID}`)}><span className='d-none d-lg-inline'> Info </span><Icon  name='angle right' /></Button></div>
                  </div>        
            </div>
          </div>
      </>)
  ...
  const LoadingCard = () =>{
        const ProfilePlacholder = () => {
          return(<>
              <div className='card p-2 shadow-sm border-div'>
                  <div className='row'>
                    ...
                    <div className='col-5 align-self-center'><Placeholder style={{ width: '100%' }}><Placeholder.Line  /></Placeholder></div>
                    <div className='col-4 align-self-center'><Placeholder style={{ width: '100%' }}><Placeholder.Line  /></Placeholder></div>
                    <div className='col-2 align-self-center text-center'><Placeholder className='rounded-pill  ' style={{ width: 50, height : 20 }}> </Placeholder></div>
                  </div>   
    ...
    return(<>
          <div className='row '>
              <div className='col-12  mb-3'> <ProfilePlacholder /></div>
              <div className='col-12  mb-3'> <ProfilePlacholder /></div>
              <div className='col-12  mb-3'> <ProfilePlacholder /></div>
              <div className='col-12  mb-3'> <ProfilePlacholder /></div>
          </div>
