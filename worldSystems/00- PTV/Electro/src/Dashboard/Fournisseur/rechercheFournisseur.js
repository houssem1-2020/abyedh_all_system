...
const InputDatCard = ({deleg , setDeleg , gouv ,  delegList ,  FetchTargetList, loaderState, gouvList, GetDelegList}) => {
    return(<>
        <div className='card card-body shadow-sm mb-2  border-div'>
            <h5>Entrer Une Location   </h5>
            <div className='p-1 mb-2'>
                <Select placeholder='Selectionnez Gouvernorat' fluid className='mb-2 shadow-sm' options={gouvList} value={gouv} onChange={(e, { value }) => GetDelegList(value)} />
                <Select placeholder='Selectionnez Gouvernorat ' fluid className='shadow-sm' value={deleg} options={delegList} onChange={(e, { value }) => setDeleg(value)} />
            </div>
            <div className='mt-3'>
                <Button  className='rounded-pill bg-system-btn' onClick={FetchTargetList} fluid><Icon name='search' /> Rechercher <Loader inverted active={loaderState} inline size='tiny' className='ms-2 text-danger'/> </Button>
            </div>
        </div>
    </>)
}
...
function RechercheFournisseur() {
    ...
    const FetchTargetList = () =>{
        setLS(true)
        axios.post(`https://api.abyedh.com/api/application/Search/search`, {
            tag: 'depot',
            genre:' ',
            gouv:gouv,
            deleg :deleg
        ...
        <div className='row'>
            <div className='col-12 col-lg-4'>
                <div className="mb-4 sticky-top" style={{top:'70px'}}>
                    <InputDatCard  deleg={deleg} gouv={gouv} gouvList={gouvList}  setDeleg={setDeleg} setGouv={setGouv} delegList={delegList} FetchTargetList={FetchTargetList} loaderState={loaderState} GetDelegList={GetDelegList} />
                </div>
            </div>
            <div className='col-12 col-lg-8'>
                <h5>Listes des Factures</h5>    
                    {resultList.map( (val) => <FactureListCard key={val.F_ID} dataF={val}/>)}
                <br />
