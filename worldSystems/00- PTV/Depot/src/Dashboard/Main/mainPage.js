...
function MainPage() {
    ...
    const [commandeRTD, setCommandeRTD]= useState([])
    ...
    useEffect(() => {
        axios.post(`${GConf.ApiLink}/ma/stat`, {
          ...
          .then(function (response) {
            ...
            let commandeRT = []
            response.data.RecetteCommandes.map((data) => commandeRT.push({ name: data.State , value: data.Totale , color:'#fcba03'}))
            setCommandeRTD(commandeRT)
    ...
    const BarChtCommande = (props) => {
        ...
        <ResponsiveContainer  height={150} >
            ...
            >
                <Tooltip cursor={{fill: 'transparent'}} />
                <Bar dataKey="value" fill={GConf.themeColor}  barSize={15}  radius={[0, 10, 10, 0]}/>
                <XAxis type="number" domain={[0, (Math.max(...commandeRTD.map(o => o.value)) + 10)]} hide  dataKey="value"/>
                <YAxis type="category"   dataKey='name' />
    ...
    return (<>
        ...
        <Slide  bottom > 
            ...
            <div className='row p-0'>
                ...
                <div className='col-12 col-lg-6 mb-4'><ChartsContainer chart={<BarChtCommande />} col='5' title='Recette des Commandant' /> </div>
