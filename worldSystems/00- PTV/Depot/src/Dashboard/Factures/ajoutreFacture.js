...
function AjouterFacture() {
        ...
        axios.post(`${GConf.ApiLink}/stock/bs`, {
          ...
          .then(function (response) {      
            if(response.data.affectedRows) {
                axios.post(`${GConf.ApiLink}/facture/us`, { PID : GConf.PID,  fid: gettedFID })
    ...
    const GetPrixColis = () =>{
        if (articleNow.Qte) {
            const QteColis = (parseInt(articleNow.Qte) * articleNow.Groupage)
    ...
    const AddArticles = () =>{
        return (<>
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className='row'>
                       <div className='col-8'>  <Input icon='dropbox' type='number' value={articleNow.Qte} autoFocus={autofocusState} onChange={ (e) => setArticleNow({...articleNow, Qte : e.target.value})} size="small" iconPosition='left' placeholder='Quantite'  fluid className='mb-1' /> </div> 
