...
function LogIn() {
    ...
    const logIn = () =>{
        if (!loginD.Log) {toast.error( t('communUsed.logInPage.toast.addIdentif') , GConf.TostErrorGonf)}
            ...
            setLS(true)
            axios.post(`${GConf.SharedApi}/LogIn`, {
                LoginData : loginD,
                ...
                if(response.data.Existance ) {
                    ...
                    if (!GConf.Offline) {
                        localStorage.setItem(`${response.data.Existance.PID}_${GConf.systemTag}_Offline`, JSON.stringify(GConf.offline_default_table));
                    }
                    localStorage.setItem('PID', response.data.Existance.PID);
                    localStorage.setItem('ProfileData', JSON.stringify(response.data.Profile));
                    localStorage.setItem(`${response.data.Existance.PID}_${GConf.systemTag}_LocalD`, JSON.stringify(response.data.Profile));

                    //acording to country add position and launguage 
                    SelectDefaultCountry(response.data.Profile.Country)
                    localStorage.setItem('country', response.data.Profile.Country);
