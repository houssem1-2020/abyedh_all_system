    ...
    return(<>
            <div className='p-1'>
                        ...
                        <div className='p-1  mb-2'>
                            ...
                            <Input icon='user' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} placeholder='Nom Et Prenon ' className='w-100 border-0 shadow-sm rounded mb-1' value={clientD.ME_Name} onChange={(e) => setClientD({...clientD, ME_Name: e.target.value })} />
                        </div>
                        <div className='p-1 mb-2'>
...
function ClientInfo() {
    ...
    useEffect(() => {
        //client Info
        axios.post(`${GConf.ApiLink}/membres/info`, {
            PID : GConf.PID,
            membreId : CLID
    ...
    const EditClient = () =>{
        if (!clientD.CIN) {toast.error("Matricule Invalide !", GConf.TostErrorGonf)}
        else if (!clientD.ME_Name) {toast.error("Nom Invalide !", GConf.TostErrorGonf)}
            ...
            setLS(true)
            axios.post(`${GConf.ApiLink}/membres/modifier`, {
                PID : GConf.PID,
                clientD : clientD,
            }).then(function (response) {
    ...
    const DeleteClientFunc = () =>{
            setLS(true)
            axios.post(`${GConf.ApiLink}/membres/supprimer`, {
                PID : GConf.PID,
                clientId : CLID,
            }).then(function (response) {
                if(response.data.affectedRows) {
    ...
    const RelateToUID = () =>{
        if (!inAbyedhSearch) {toast.error("Entrer Un Code A Barre  !", GConf.TostErrorGonf)}
            ...
            setLS(true)
            axios.post(`${GConf.ApiLink}/membres/verification`, {
                PID : GConf.PID,
                UID : inAbyedhSearch,
                ME_ID : CLID,
            }).then(function (response) {
                ...
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className="img-card-container text-center">
                        <div className="card-container">
                            <img src="https://cdn.abyedh.com/images/system/gym/membre.jpg" className="rounded-circle" width="80" />                    
                        </div>
                    </div>
