...
const FinishCatd = ({saveBtnState,EditOrdonance,loaderState, factureD, setFactureD, OnKeyPressFunc, clientList, camionList}) =>{
    const [condidatNow, setCondidatNow] = useState([])
    
    const SelectCondidatFunction = (value) => {
        if (value) {       
            setFactureD({...factureD, OP_Client: value })
            let filtedClient = clientList.find((data) => data.CL_ID == value)
            setCondidatNow(filtedClient)
        }
    }

    useEffect(()=> {
        SelectCondidatFunction(factureD.OP_Client)
        setFactureD({...factureD, OP_Camion: factureD.OP_Camion })
    },[])
    return (<>
            <div className='card card-body shadow-sm mb-2 border-div'>
                ...
                <Input icon='add user' onKeyPress={event => OnKeyPressFunc(event)} list="clientList" placeholder={factureD.OP_Client}   onBlur={ (e) => SelectCondidatFunction(e.target.value)} size="small" iconPosition='left'   fluid className='mb-1' />
                        <div className='card-body border-div mb-4 mt-4 mb-3 bg-gray'>
                            <div className='row '>
                                <div className='col-6 '> <span className='bi bi-person-fill'></span> Client :  {condidatNow.CL_Name  ? condidatNow.CL_Name  : ''}</div>
                                <div className='col-6 '> <span className='bi bi-phone-fill'></span> Adresse  : {condidatNow.Gouv  ? condidatNow.Gouv  : ''} </div>
                            </div>
                        </div>
...
function AjouterFacture() {
    ...
    const panes = [
        ...
        },
        {
            ...
            render: () => <FinishCatd saveBtnState={saveBtnState} EditOrdonance={EditOrdonance} loaderState={loaderState} clientList={clientList}   camionList={camionList} factureD={factureD} setFactureD={setFactureD} OnKeyPressFunc={OnKeyPressFunc}  />,
        }
    ...
    useEffect(() => {
        axios.post(`${GConf.ApiLink}/operations/info`, {
          ...
          .then(function (response) {
                if(!response.data[0]) {
                    toast.error('Operation Introuvable !', GConf.TostSuucessGonf)
                    setTimeout(() => {  window.location.href = "/S/op"; }, 2000)
                    
                } else {
 
                    
                    setFactureD({ OP_Date: new Date(response.data[0].OP_Date).toISOString().split('T')[0], OP_Camion : response.data[0].OP_Camion, OP_Client : response.data[0].OP_Client, OP_De:JSON.parse(response.data[0].OP_De) , OP_Vers:JSON.parse(response.data[0].OP_Vers) ,  OP_Articles:JSON.parse(response.data[0].OP_Articles)})
                    setLoading(true)
    ...
    const EditOrdonance = () =>{
            ...
            else if (!factureD.OP_Camion) {toast.error("Patient De est Invalide !", GConf.TostErrorGonf)}
            else if (!factureD.OP_Client) {toast.error("Patient De est Invalide !", GConf.TostErrorGonf)}
            else if (!factureD.OP_Articles || factureD.OP_Articles.length == 0) {toast.error("article list est Invalide !", GConf.TostErrorGonf)}
            else {
                setLS(true)
                axios.post(`${GConf.ApiLink}/operations/modifier`, {
                    PID : GConf.PID,
