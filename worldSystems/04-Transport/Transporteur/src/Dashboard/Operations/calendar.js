...
function CalendarCommandes() {
    ...
    useEffect(() => {
        ...
        .then(function (response) {
            ...
            setTodayListe(response.data.filter((data) => data.OP_State == 'A' && data.OP_Date ==  new Date().toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' )))
            let calendarData = []
            response.data.map( (getData) => calendarData.push( { title: `${getData.CL_Name} : ${JSON.parse(getData.OP_Vers).Gouv}` , date: new Date(getData.OP_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ), className: GenerateColor(getData.OP_State)}))
