...
function ClientInfo() {
    ...
    const panes = [
        {
            menuItem: { key: 'home', icon: 'home', content: 'Operations' }, 
    ...
    const EditClient = () =>{
        if (!clientD.CIN) {toast.error("Matricule Invalide !", GConf.TostErrorGonf)}
            ...
            setLS(true)
            axios.post(`${GConf.ApiLink}/clients/modifier`, {
                PID : GConf.PID,
                clientD : clientD,
            }).then(function (response) {
    ...
    const DeleteClientFunc = () =>{
            setLS(true)
            axios.post(`${GConf.ApiLink}/clients/supprimer`, {
                PID : GConf.PID,
                clientId : CLID,
            }).then(function (response) {
                if(response.data.affectedRows) {
    ...
    const FindInDirectoryFunc = () =>{
        if (!inAbyedhSearch) {toast.error("Entrer Un Code A Barre  !", GConf.TostErrorGonf)}
            ...
            setLS(true)
            axios.post(`${GConf.ApiLink}/clients/checkAbyedhDb`, {
                UID : inAbyedhSearch,
            }).then(function (response) {
    ...
    const RelateToUID = () =>{
        if (!inAbyedhSearch) {toast.error("Entrer Un Code A Barre  !", GConf.TostErrorGonf)}
            ...
            setLS(true)
            axios.post(`${GConf.ApiLink}/clients/verification`, {
                PID : GConf.PID,
                UID : inAbyedhSearch,
                CL_ID : CLID,
            }).then(function (response) {
                ...
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className="img-card-container text-center">
                        <div className="card-container">
                            <img src="https://cdn.abyedh.com/images/system/transporteur/client.png" className="rounded-circle" width="80" />                    
                        </div>
                    </div>
