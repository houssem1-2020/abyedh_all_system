...
function AddPlatMenu() {
    ...
    const SaveArticle = (event) => {
            if (!voitureData.CM_Name) {toast.error("Nom est Invalide !", GConf.TostErrorGonf)}
            else if (!voitureData.CM_Genre) {toast.error("Niveaux est Invalide !", GConf.TostErrorGonf)}
            else if (!voitureData.CM_Matricule) {toast.error("Niveaux est Invalide !", GConf.TostErrorGonf)}
            else if (!voitureData.CM_Identifiant) {toast.error("Niveaux est Invalide !", GConf.TostErrorGonf)}
            else if (!voitureData.CM_Pwd) {toast.error("Niveaux est Invalide !", GConf.TostErrorGonf)}
            else{
                setLS(true)
                axios.post(`${GConf.ApiLink}/camion/ajouter`, {
                    PID : GConf.PID,
    ...
    const GenrateKey = () =>{
        ...
        setVoitureData({...voitureData, CM_Identifiant : ID , CM_Pwd:PWD})
    }
                    ...
                    <div className='col-12 col-lg-8'>
                        <div className='row'>
                                <div className='col-12 col-lg-12'>
                                    ...
                                    <Input icon='star' iconPosition='left' placeholder='Nom' className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={voitureData.CM_Name}  onChange={(e) => setVoitureData({...voitureData, CM_Name: e.target.value })}/>
                                </div>
                                <div className='col-12 col-lg-12'>
                                    <h5 className='mb-1'>Matricule: </h5>
                                    <Input icon='key' iconPosition='left' placeholder='Matricule' className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={voitureData.CM_Matricule}  onChange={(e) => setVoitureData({...voitureData, CM_Matricule: e.target.value })}/>
                                </div>
                                <div className='col-12 col-lg-12'>
                                    <h5 className='mb-1'>Marque : </h5>
                                    <Input icon='box' iconPosition='left' placeholder='Marque' className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={voitureData.CM_Genre}  onChange={(e) => setVoitureData({...voitureData, CM_Genre: e.target.value })}/>
                                </div>
                                <div className='col-12 col-lg-12'>
                                    <h5 className='mb-1'>Chauffeur : </h5>
                                    <Input icon='user' iconPosition='left' placeholder='Chauffeur' className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={voitureData.CM_Genre}  onChange={(e) => setVoitureData({...voitureData, CM_Genre: e.target.value })}/>
                                </div>
                        ...
                        <div className='row mb-3'>
                                <div className='col-12 col-lg-6'>
                                    ...
                                    <Input icon='linkify' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} placeholder='identifiant'  className='w-100 border-0 shadow-sm rounded mb-3' value={voitureData.CM_Identifiant	} onChange={(e) => setVoitureData({...voitureData, CM_Identifiant	: e.target.value })} />
                                </div>
                                <div className='col-9 col-lg-5'>
                                    <h5 className='mb-1'>Mot De Pass: </h5>
                                    <Input icon='eye' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} placeholder='Nom' className='w-100 border-0 shadow-sm rounded mb-3' value={voitureData.CM_Pwd} onChange={(e) => setVoitureData({...voitureData, CM_Pwd: e.target.value })}/>
                                </div>
