...
function RequestInfo() {
    ...
    const UpdateRequestState = (stateBtn,dataGenre,selectedData,saveNotif,actionName) =>{
        axios.post(`${GConf.SharedApi}/request/controle`, {
            ...
            TAG : 'transporteur',
            RID: CID,
            genreTag : 'transporteur_request',
            state: stateBtn,
            data: selectedData,
            dataGenre: dataGenre,
            saveNotif : saveNotif,
            actionName : `transporteur_${actionName}`,
          })
          .then(function (response) { 
    ...
    return ( <> 
        ...
        >
        <Modal.Content >
                <MapContainer  center={positionsMap} zoom={15} scrollWheelZoom={false} className="map-height-lg cursor-map-crosshair border-div"  >
                    <TileLayer
