...
function StockPage() {
    ...
    const [forfaitD, setForfaitD] = useState({Tarif_Fix: '0', Tarif:'0', Poid_Max:'0' })
    const [prixGenre, SetPrixGenre] = useState('PV')
   ...
   const forfalitGenre = [
      {id:1, value:'PF', text:'Prix Fix'},
      {id:2, value:'PV', text:'Prix Variable'},
   ]
    ...
    /*#########################[Function]##################################*/
      ...
      const SaveForfait = (event) => {
               ...
               else if (!forfaitD.Poid_Max) {toast.error("Poid Maximale est Invalide !", GConf.TostErrorGonf)}
               else if (!forfaitD.Tarif) {toast.error("Traif est  Invalide !", GConf.TostErrorGonf)}
               else{
                  setLS(true)
                  ...
                  <div className='col-12 col-lg-4  '>
                     <div className="sticky-top" style={{top:'70px'}}>
                        <div className='card card-body border-div shadow-sm mb-4'>
                              ...
                              <Select placeholder='Selectionnez Gouvernorat' fluid className='mb-2' options={forfalitGenre} value={prixGenre} onChange={(e, { value }) => SetPrixGenre(value)} />
                              {
                                 prixGenre == 'PV' ? 
                                 <>
                                    <h5 className='mb-1 mt-0'>Prix de kilométre: </h5>
                                    ...
                                    <div className='row'>
                                       <div className='col-12'>
                                             <h5 className='mb-1'> Poid Maximale : </h5>
                                             <Input icon='tag' iconPosition='left' type='number' placeholder='Nombre de Seance' value={forfaitD.Poid_Max}   className='w-100 border-0 shadow-sm rounded mb-3' onChange={(e) => setForfaitD({...forfaitD, Poid_Max: e.target.value })}/> 
                                       </div>
                                    </div>
                                 </>
                                 :
                                 <>
                                    <h5 className='mb-1 mt-0'>Prix Totale : </h5>
                                    <Input icon='dollar' iconPosition='left' placeholder='Tarif' className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={forfaitD.Tarif_Fix}  onChange={(e) => setForfaitD({...forfaitD, Tarif_Fix: e.target.value })}/>
                                 </>
                              }
