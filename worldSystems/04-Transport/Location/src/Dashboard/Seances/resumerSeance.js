...
const InputDatCard = ({targetDate, setTargetDate, FetchTargetFactures,PrintFunction, loaderState}) => {
    ...
    return(<>
        <div className='card card-body shadow-sm mb-2  border-div'>
            <h5>{t(`menuTabs.seancePage.resumerCard.cardTitle`)}   </h5>
            ...
            <div className='mt-3'>
                <Button  className='rounded-pill bg-system-btn' onClick={FetchTargetFactures} fluid><Icon name='search' /> {t(`menuTabs.seancePage.resumerCard.rechercheBtn`)} <Loader inverted active={loaderState} inline size='tiny' className='ms-2 text-danger'/> </Button>
            </div>
            {/* <div className='mt-3'>
...
function ResumerFactures() {
    ...
    return ( <>
        <BreadCrumb links={GConf.BreadCrumb.factureResumer} bcTag='factureResumer' />
        <br />
        <div className='row'>
            ...
            <div className='col-12 col-lg-9'>
                <TableGrid tableData={factureList} columns={t(`menuTabs.seancePage.resumerCard.tableHeader`).split(/,[\s]*'/).map(item => item.replace(/'/g, '')).filter(Boolean)} />                        
            </div>
        </div>
