...
const TerminerCard = ({seanceData, setSeanceData, profListe, salleListe, matiereListe, OnKeyPressFunc}) =>{
    const [profNow, setProfNow] = useState([])
    const [ matiereNow , setMatiereNow] = useState([])
    const [salleNow, setSalleNow] = useState([])
    
    const SelectProfFunction = (value) => {
        if (value) {
            setSeanceData({...seanceData, Proffeseur_ID: value })
            let filtedClient = profListe.find((data) => data.T_ID == value)
            setProfNow(filtedClient)
        }
    }

    const SelectMatiereFunction = (value) => {
        if (value) {
            setSeanceData({...seanceData, Matiere_ID: value })
            let filtedClient = matiereListe.find((data) => data.Matiere_ID == value)
            setMatiereNow(filtedClient)
        }
    }
    const SelectSalleFunction = (value) => {
        if (value) {
            setSeanceData({...seanceData, Salle_ID: value })
            let filtedClient = salleListe.find((data) => data.Salle_ID == value)
            setSalleNow(filtedClient)
        }
    }
    return (<>
                ...
                <Input icon='add user' onKeyPress={event => OnKeyPressFunc(event)} list="profListe" placeholder={seanceData.Proffeseur_ID}   onBlur={ (e) => SelectProfFunction(e.target.value)} size="small" iconPosition='left'   fluid className='mb-1' />
                <div className='p-2  rounded mb-4 mt-0 mb-3 bg-gray'>
                    <div className='row '>
                        <div className='col-6 '> <span className='bi bi-card-text'></span> Nom  :  {profNow.T_Name  ? profNow.T_Name  : ''}</div>
                        <div className='col-6 '> <span className='bi bi-person-fill'></span> Poste : {profNow.Poste  ? profNow.Poste  : ''} </div>
                    </div>
                </div>  
                ...
                <Input icon='add user' onKeyPress={event => OnKeyPressFunc(event)} list="matiereListe" placeholder={seanceData.Matiere_ID}   onBlur={ (e) => SelectMatiereFunction(e.target.value)} size="small" iconPosition='left'   fluid className='mb-1' /> 
                <div className='p-2  rounded mb-4 mt-0 mb-3 bg-gray'>
                    <div className='row '>
                        <div className='col-6 '> <span className='bi bi-card-text'></span> Matiére  :  {matiereNow.Matiere_Name  ? matiereNow.Matiere_Name  : ''}</div>
                        <div className='col-6 '> <span className='bi bi-person-fill'></span> Genre : {matiereNow.Matiere_Genre  ? matiereNow.Matiere_Genre  : ''} </div>
                    </div>
                </div>
                ...
                <Input icon='add user' onKeyPress={event => OnKeyPressFunc(event)} list="salleListe" placeholder={seanceData.Salle_ID}   onBlur={ (e) => SelectSalleFunction(e.target.value)} size="small" iconPosition='left'   fluid className='mb-1' />
                <div className='p-2  rounded mb-4 mt-0 mb-3 bg-gray'>
                    <div className='row '>
                        <div className='col-6 '> <span className='bi bi-card-text'></span> Nom  :  {salleNow.Salle_Name  ? salleNow.Salle_Name  : ''}</div>
                        <div className='col-6 '> <span className='bi bi-person-fill'></span> Numero : {salleNow.Salle_Num  ? salleNow.Salle_Num  : ''} </div>
                    </div>
                </div>
