                ...
                <div className='col-12 col-lg-8'>
                    ...
                    <div className='row mb-3'>
                        <div className='col-6'>
                            ...
                            <Input icon='truck' type='date' placeholder='Camion'  iconPosition='left'   fluid className='mb-1 shadow-sm' value={fournisseurData.RT_Date} onChange={(e) => setFSData({...fournisseurData, RT_Date: e.target.value })}/> 
                        </div>
                        <div className='col-6'>
                            <h5 className='mb-0 mt-2 text-secondary '> <span className='bi bi-alarm'></span> Heur  </h5>
                            <Input icon='map marker' type='time' onKeyPress={event => OnKeyPressFunc(event)}  iconPosition='left' placeholder='De'  fluid className='mb-1 shadow-sm'  value={fournisseurData.RT_Time_De}  onChange={(e) => setFSData({...fournisseurData, RT_Time_De : e.target.value })}/>
                        </div>
                    </div>
                    ...
                    <div className='row'>
                            ...
                            <Form>
                                <TextArea  rows="3" onKeyPress={event => OnKeyPressFunc(event)} placeholder='Cause' className='w-100 shadow-sm rounded mb-3' value={fournisseurData.RT_Cause} onChange={(e) => setFSData({...fournisseurData, RT_Cause: e.target.value })}/>
                            </Form>
                    </div>
...
function FournisseurInfo() {
         ...
         useEffect(() => {
             ...
             axios.post(`${GConf.ApiLink}/eleve/retenue/info`, {
                 PID: GConf.PID,
                 CLID : RTID
        ...
        return ( <>
                <BreadCrumb links={GConf.BreadCrumb.retenueInfo} />
                <br />
                <div className="row">
                    {
                        !loading ? <></>
                        ...
                        <>
                            ...
                            <h5 className='text-start mb-1 text-secondary'><span className='bi bi-calendar-week'></span> Date : {new Date(fournisseurData.RT_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' )} </h5>
                            <h5 className='text-start mb-1 text-secondary'><span className='bi bi-clock'></span> De : {fournisseurData.RT_Time_De} </h5>
                            <h5 className='text-start mb-1 text-secondary'><span className='bi bi-clock'></span> Vers : {fournisseurData.RT_Time_Vers} </h5>
                            <h5 className='text-start mb-1 text-secondary'><span className='bi bi-chat-dots'></span> Cause : {fournisseurData.RT_Cause} </h5>
