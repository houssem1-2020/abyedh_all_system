    ...
    const StockCard = () =>{
        return(<>
            <div className='card card-body shadow-sm mb-4 border-div'>
               ...
               {
                GConf.Sauvgarder.map((data,index) => <Checkbox key={index} className='mb-2' label={data.label} defaultChecked  onChange={(event, data) =>  UpdateTableStet(data.table,data.checked)}/>)
               }
