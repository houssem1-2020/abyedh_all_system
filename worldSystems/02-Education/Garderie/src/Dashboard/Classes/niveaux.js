...
function FamillesPlats() {
    ...
    const SaveFamille = () => {
        if (!familleD.Name) {toast.error("Non de La famille est Invamlide !", GConf.TostErrorGonf)}
            ...
            setLS(true)
            axios.post(`${GConf.ApiLink}/classes/niveaux/ajouter`, {
                PID : GConf.PID,
                familleD : familleD,
            }).then(function (response) {
    ...
    const EditFamille = () => {
        setLS(true)
        axios.post(`${GConf.ApiLink}/classes/niveaux/modifier`, {
            PID : GConf.PID,
                ...
                <Bounce bottom>
                    <div className="card card-body border-div mb-4 sticky-top" style={{top:'70px' , zIndex:'1'}}>
                        ...
                        <div className='text-end'>
                            <Button  className='text-end bg-system-btn rounded-pill' disabled={saveBtnState} onClick={SaveFamille}>   <Icon name='save' /> Enregistrer <Loader inverted active={loaderState} inline size='tiny' className='ms-2'/></Button>
                        </div>
                    </div>
                </Bounce>
                </div>
