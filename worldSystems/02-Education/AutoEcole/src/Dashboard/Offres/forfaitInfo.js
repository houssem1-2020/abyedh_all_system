...
const EditArticle = ({forfaitD, setFordaitD, OnKeyPressFunc,  EditArticleFunction,loaderState }) =>{
    return(<>
                ...
                <Input icon='dollar' iconPosition='left' type='number' placeholder='Tarif' className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={forfaitD.Tarif}  onChange={(e) => setFordaitD({...forfaitD, Tarif: e.target.value })}/>

                <div className='row'>
                    <div className='col-12'>
                        <h5 className='mb-1'>Nombre de Seance: </h5>
                        <Input icon='tag' iconPosition='left'  placeholder='Nombre de Seance' onKeyPress={event => OnKeyPressFunc(event)} value={forfaitD.NB_Seance}   className='w-100 border-0 shadow-sm rounded mb-3' onChange={(e) => setFordaitD({...forfaitD, NB_Seance: e.target.value })}/> 
                    </div>
                </div>
...
function ArticleInfo() {
    ...
    const panes = [
        {
            ...
            render: () => <> <TableGrid tableData={forfaitListe} columns={['*','ID','Condidat','Permis','Depart','Genres','Etat','Voir']} /> <br /></>,
        },
        {
   ...
   /*#########################[UseEffect]##################################*/
        ...
        axios.post(`${GConf.ApiLink}/forfait/select`, {
          ...
          .then(function (response) {
            ...
            if(!response.data.Data.F_ID) {
               ...
               response.data.Abonnement.map( (getData) => abonemmentContainer.push([
    ...
    const OnKeyPressFunc = (e) => {
        if (!((e.charCode >= 65 && e.charCode <= 90) || (e.charCode >= 97 && e.charCode <= 122) || (e.charCode >= 48 && e.charCode <= 57) || e.charCode == 42 || e.charCode == 32 || e.charCode == 47 || e.charCode == 47 || e.charCode == 46 )) {
              e.preventDefault();
        }   
     }
                ...
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className="img-card-container text-center">
                        <div className="card-container notification bg-white">
                            <img src={`https://cdn.abyedh.com/images/system/gym/tarifs.png`} className="rounded-circle" width="70px" height="70px" />                    
                        </div>
                    </div>
