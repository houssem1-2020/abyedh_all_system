...
function AjouterFacture() {
    ...
    const SaisonChoise = [
        ...
        {id:3, value :'Les Deux', text:'Les Deux'},
    ...
    const SelectCondidatFunction = (value) => {
        if (value) {
            ...
            setAbonnemmentData({...abonnemmentData, Condidat_ID: value })
            let filtedClient = membreListe.find((data) => data.CD_ID == value)
        ...
        <Bounce left>
            <div className='row'>
                <div className='col-12 col-lg-8'>
                    ...
                    <Input icon='add user' onKeyPress={event => OnKeyPressFunc(event)} list="clientList" placeholder={abonnemmentData.Condidat_ID}   onBlur={ (e) => SelectCondidatFunction(e.target.value)} size="small" iconPosition='left'   fluid className='mb-1 shadow-sm' />
                        
                        <div className='card-body border-div mb-4 mt-4 mb-3 bg-gray'>
                            <div className='row '>
                                <div className='col-6 '> <span className='bi bi-person-fill'></span> Condidat :  {condidatNow.CD_Name  ? condidatNow.CD_Name  : ''}</div>
                                <div className='col-6 '> <span className='bi bi-phone-fill'></span> Phone : {condidatNow.CD_Phone  ? condidatNow.CD_Phone  : ''} </div>
                            </div>
                        </div>
