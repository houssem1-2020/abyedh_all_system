...
function AjouterFacture() {
    ...
    const [seanceData, setSeanceData] = useState({SE_Condidat:'PASSAGER',  Remarques: 'Null', SE_Abonnemment:'', SE_Genre:'Pratique',   Moniteur_ID:'', SE_Date: Today.toISOString().split('T')[0], 	Voiture_ID: OneGConf.forPID.VO_ID , trajetListe:[]})
    ...
    const SaveSeance = () =>{
            if (!seanceData.Remarques ) {toast.error("Diagnostique est Invalide !", GConf.TostErrorGonf)}
                ...
                axios.post(`${GConf.ApiLink}/seances/ajouter`, {
                    PID : OneGConf.forPID.PID,
                    seanceData: {Moniteur_ID: seanceData.Moniteur_ID,  SE_Activite: GenerateDiagnostiqueHTml(),  SE_Genre:'Pratique',  SE_Abonnemment:seanceData.SE_Abonnemment,  SE_Date: seanceData.SE_Date ,  Moniteur_ID:seanceData.Moniteur_ID,	Voiture_ID: OneGConf.forPID.VO_ID ,  trajetListe:positions} ,
                })
                .then(function (response) {
    ...
    const OnKeyPressFuncOld = (e) => {
        if (!((e.charCode >= 65 && e.charCode <= 90) || (e.charCode >= 97 && e.charCode <= 122) || (e.charCode >= 48 && e.charCode <= 57) || e.charCode == 42 || e.charCode == 32 || e.charCode == 47 )) {
            e.preventDefault();
        }   
    }
    const OnKeyPressFunc = (e) => {
        const allowedKeys = ['*', ' ', '/'];  // Allowed special characters
        const key = e.key;
    
        // Check if the key is a letter, number, or allowed special character
        if (!((key >= 'A' && key <= 'Z') || (key >= 'a' && key <= 'z') || (key >= '0' && key <= '9') || allowedKeys.includes(key))) {
            e.preventDefault();
        }
    };
   ...
   /*#########################[Card]##################################*/
    ...
    const RecordMapCard = () =>{
        return(<>
                <div className={` card card-body mb-2 shadow-sm border-div  ${OneGConf.themeMode == 'dark text-white border-white' ? 'bg-dark-theme-2' : '' }`} >
    ...
    return (<>
            ...
            <div className={`${OneGConf.themeMode == 'dark' ? 'bg-dark-theme-2' : '' }`} style={{height: '100vh', overflow: 'scroll'}}>
                <BackCard data={OneGConf.backCard.ns}/>
                <br />                 
