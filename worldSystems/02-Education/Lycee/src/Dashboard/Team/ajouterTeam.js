...
function AjouterTeam() {
    ...
    const [teamD, setTeamD] = useState({Is_Prof: false})
    ...
    /*#########################[UseEffect]##################################*/
        ...
        .then(function (response) {
            let MatiereN = []
              response.data.map( (dta) => {MatiereN.push({value : dta.Matiere_ID, text : <>{dta.Matiere_Name}</>, key: dta.PK})})
            setMatiereListe(MatiereN)
        })
        }, [])
                                    ...
                                    <div className='row '>
                                       ...
                                       <div className='col-2 align-self-center text-center'>
                                            <div className="form-check form-switch">
                                                <input className="form-check-input form-check-input-lg" type="checkbox" checked={teamD.Is_Prof} onChange={ () => setTeamD({...teamD, Is_Prof:  !teamD.Is_Prof })}   />
                                            </div>
                                        </div> 
                                    </div>
                                    {
                                        teamD.Is_Prof ? <Select placeholder='Choisir Uu Poste'  options={matiereListe}  className='w-100 shadow-sm rounded mb-1 mt-3' value={teamD.Has_Matiere} onChange={(e, data) => setTeamD({...teamD, Has_Matiere: data.value })} /> : <></>
                                    }
                                </div>
                            </div>
