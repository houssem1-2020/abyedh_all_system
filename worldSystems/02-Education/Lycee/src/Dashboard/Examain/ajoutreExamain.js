...
function AjouterFacture() {
    ...
    const CamionSelected = (e, { value }) =>{
        ...
        .then(function (response) {
               ...
               //setDefaultEmploi(response.data.Examain)
               let classesListeContainer = []
               //response.data.Examain.map( (getData) => classesListeContainer.push( { title: `${getData.Matiere_ID} (${getData.salle})`,  date: getData.EX_Date,  className:'border-0 rounded-0 m-0 p-0', backgroundColor:  'TargertColor(getData.matiere)' }))
        ...
        return(<>
                <div className='row'>
                    ...
                    <div className='col-12 col-lg-7'>
                            ...
                            {eleveListe.map( (val) => <ArticleListCard key={val.id} dataA={val}/>)}
                            <br />
