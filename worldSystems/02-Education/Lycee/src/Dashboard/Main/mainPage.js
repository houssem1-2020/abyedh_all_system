...
function MainPage() {
    ...
    const panes = [
            ...
            {
                menuItem: 'Eléve',
                render: () => <ChartsContainer chart={<PieChartCard data={EleveData}/>} col='4' title='Distrubition des client' />,
            },
            {
                menuItem: 'Proff.',
                render: () => <ChartsContainer chart={<PieChartCard data={dataBar}/>} col='4' title='Distrubition des client' />,
            },
    ]
   ...
   /*#########################[UseEffect]##################################*/
    useEffect(() => {
        axios.post(`${GConf.ApiLink}/ma/stat`, {
          ...
          .then(function (response) {
            ...
            let clientDis = []
            response.data.eleveDistro.map((datacld) => clientDis.push({ name: datacld.Gouv, value: datacld.Totale }))
            setEleveData(clientDis)

            let genreDist = []
            response.data.classesDistro.map((datacld) => genreDist.push({ name: datacld.CL_Niveaux, value: datacld.Totale }))
            setGenreD(genreDist)

            let commandeDist = []
            response.data.abonnemmentDistro.map((datacld) => commandeDist.push({ name: datacld.F_Name, value: datacld.Totale }))
            setPieData(commandeDist)
            
            let camionRT = []
            response.data.teamDistro.map((data) => camionRT.push({ name:  data.Poste, value: data.Totale }))
            setCommandeD(camionRT)
    ...
    const PieChartCard =  (props) =>{
        ...
        const COLORS = ['#9C27B0', '#2196F3', '#E91E63', '#FF9800', '#9E9E9E', '#607D8B', '#673AB7', '#3F51B5', '#FF5252', '#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#4CAF50', '#FF5722'];
        return (
