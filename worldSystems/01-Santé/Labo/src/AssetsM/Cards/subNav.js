...
function SubNav(props) {
        ...
        const ItemCard = (props)=>{
            return(<>
                <NavLink  className="nav-link  fw-bold"  exact="true" to={props.data.link}><span className='rounded-circle icon-nav-div ' style={{backgroundColor:GConf.themeColor ,  }}> <span className={`bi bi-${props.data.icon} text-white`}></span> </span> {t(`SubNavs.${props.snTag}.${props.data.id}`)}  </NavLink>
            </>)
        }
        // return (<>
            ...
            <div className="d-flex p-2 rounded-pill responsive-bar border shadow-sm" style={{backgroundColor:GConf.themeColorLigths, color:GConf.themeColor}}>
                {props.dataForNav.map((data) => (
                    <span className="d-flex" key={data.id}> 
                        {!data.dropD ?  <ItemCard data={data} snTag={props.snTag} /> : <DropDowdnCard  data={data} /> } 
