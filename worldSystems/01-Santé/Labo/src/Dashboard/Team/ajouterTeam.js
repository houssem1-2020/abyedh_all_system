...
const FindInDirectory = ({inDirArticle, setInDirA,FindInDirectoryFunc, loaderState, OnKeyPressFunc}) =>{
    ...
    return(<>
      <div className='card card-body border-div shadow-sm mb-3 mt-4'>
          <h5>{t('menuTabs.teamPage.addTeamPage.rechercheAbyedh.title')} </h5>
          ...
          <div className='text-end'>
              <Button  className='bg-system-btn rounded-pill' fluid onClick={() => FindInDirectoryFunc()}>   <Icon name='search' /> {t('menuTabs.teamPage.addTeamPage.rechercheAbyedh.btnText')} <Loader inverted active={loaderState} inline size='tiny' className='ms-2'/></Button>
          </div>
      </div>
    </>)
}
...
function AjouterTeam() {
    ...
    const panes = [
        {
            menuItem: { key: 'client', icon: 'user', content:  t('menuTabs.teamPage.addTeamPage.rechercheAbyedh.tabsNames.enter') }, 
        ...
        {
            menuItem: { key: 'start', icon: 'add circle', content: t('menuTabs.teamPage.addTeamPage.rechercheAbyedh.tabsNames.scan') }, 
                        ...
                        {scanResultSeance ? 
                            ...
                            />
                                ...
                                <div className='text-center mt-4'>
                                    ...
                                    <Button onClick={() => setScanResultSeance(true)}> {t('menuTabs.teamPage.addTeamPage.rechercheAbyedh.clicToScan')} </Button>
                                </div>
                            )}
    ...
    return ( <>
            <BreadCrumb links={GConf.BreadCrumb.TeamAdd} bcTag='TeamAdd' />
            <br />
            <Bounce left>
                    ...
                    <div className='row'>
                        <div className='col-12 col-lg-8'>
                            <div className='p-1 mb-2'>
                                <h5 className='mb-1'> {t('menuTabs.teamPage.addTeamPage.cin')} </h5>
                                <Input icon='key' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} placeholder={t('menuTabs.teamPage.addTeamPage.cin')} className='w-100 border-0 shadow-sm rounded mb-1' value={teamD.T_CIN} onBlur={checkFSExistance}  onChange={(e) => setTeamD({...teamD, T_CIN: e.target.value })}/>
                            </div>
                            <div className='p-1  mb-2'>
                                <h5 className='mb-1'> {t('menuTabs.teamPage.addTeamPage.nomEtPrenom')}</h5>
                                <Input icon='user' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} placeholder={t('menuTabs.teamPage.addTeamPage.nomEtPrenom')} className='w-100 border-0 shadow-sm rounded mb-1' value={teamD.Name} onChange={(e) => setTeamD({...teamD, Name: e.target.value })} />
                            </div>
                            <div className='p-1 mb-2'>
                                <h5 className='mb-1'> {t('menuTabs.teamPage.addTeamPage.phone')} </h5>
                                <Input icon='phone' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} placeholder={t('menuTabs.teamPage.addTeamPage.phone')} className='w-100 border-0 shadow-sm rounded mb-1' value={teamD.Phone} onChange={(e) => setTeamD({...teamD, Phone: e.target.value })} />
                            </div>
                            <div className='p-1 mb-2'>
                                <h5 className='mb-1'>  {t('menuTabs.teamPage.addTeamPage.poste')}</h5>
                            ...
                            <div className='p-1 mb-2'>
                                <h5 className='mb-1'>  {t('menuTabs.teamPage.addTeamPage.adresse')}</h5>
                                <Form>
                                    <TextArea  rows="3" placeholder={t('menuTabs.teamPage.addTeamPage.adresse')} onKeyPress={event => OnKeyPressFunc(event)} className='w-100 shadow-sm rounded mb-3' value={teamD.T_Adress} onChange={(e) => setTeamD({...teamD, Adress: e.target.value })}/>
                                </Form> 
                            </div>
                            <div className='text-end mb-5'>
                                <Button  onClick={SaveTeam} disabled={saveBtnState} className='text-end rounded-pill bg-system-btn ' positive>  <Icon name='save outline' /> {t('menuTabs.teamPage.addTeamPage.saveBtn')} <Loader inverted active={loaderState} inline size='tiny' className='ms-2 text-danger'/></Button>
                            </div>
                        </div>
