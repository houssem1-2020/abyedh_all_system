...
function ClientPage() {
    ...
    useEffect(() => {
        ...
        .then(function (response) {
            ...
            getData.Adress,
            _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/cl/info/${getData.PA_ID}`)}><span className='d-none d-lg-inline'> {t('menuTabs.patientPage.infoText')} </span><Icon  name='angle right' /></Button>)
            ],))
    ...
    return (<>
        <div className='row'>
            <div className='col-12 col-lg-8'><SubNav dataForNav={GConf.SubNavs.client} snTag='client' /></div>
            <div className='col-12 col-lg-4 text-end align-self-center'>
                <MainSubNavCard text={t('menuTabs.patientPage.assuranceText')} link='pt' icon='card-heading' /> 
            </div>
        </div>
        <Fade>
