...
const FindInDirectory = ({inDirArticle, setInDirA,FindInDirectoryFunc, loaderState, OnKeyPressFunc}) =>{
    return(<>
      <div  className={`card card-body border-div shadow-sm mb-3 mt-4 ${OneGConf.themeMode == 'dark' ? 'bg-dark-theme-3 text-white border-dark' : 'bg-white' } `}>
...
function AjouterClient() {
    ...
    const panes = [
        {
            menuItem: { key: 'client', icon: 'user', content:  'Entrer' , className:` ${OneGConf.themeMode == 'dark' ? 'text-white' : '' }` }, 
        ...
        {
            menuItem: { key: 'start', icon: 'add circle', content: 'Scaneer ' , className:` ${OneGConf.themeMode == 'dark' ? 'text-white' : '' }` }, 
            render: () => <div className={`card card-body border-div shadow-sm mb-3 mt-4 ${OneGConf.themeMode == 'dark' ? 'bg-dark-theme-3 text-white border-dark' : 'bg-white' } `}>
                        {scanResultSeance ? 
                            (
                            <QrReader
                            ...
                            />
                                ...
                                <div className='text-center mt-4'>
                                    <div className='bi bi-qr-code mb-4 bi-lg' style={{color: OneGConf.themeMode == 'dark' ? 'white' : GConf.themeColor , fontSize:'150px'}}></div>
                ...
                <div className='row'>
                    <div className='col-12 col-lg-8'>
                         ...
                         <div className='p-1  mb-2'>
                            <h5 className={`mb-1 ${OneGConf.themeMode == 'dark' ? 'text-white' : '' } `} >Nom Et Prenon :</h5>
                            <Input icon='user' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} placeholder='Nom Et Prenon '   className={` w-100 border-0 shadow-sm rounded mb-1 ${OneGConf.themeMode == 'dark' ? 'bg-dark' : 'bg-white'} `} value={patientD.Name} onChange={(e) => setPatientD({...patientD, Name: e.target.value })} />
                        </div>
                        <div className={`mb-1 mt-3 ${OneGConf.themeMode == 'dark' ? 'text-white' : '' } `}>
