...
function FournisseurPage() {
    ...
    return ( 
        <>
                ...
                <TableGrid tableData={clientList} columns={t(`TableHead.medicammentPage`).split(/,[\s]*'/).map(item => item.replace(/'/g, '')).filter(Boolean)}  />
                ...
                >
                    <Modal.Header><h4> {t('menuTabs.ordonancePage.stockCard.addCard.title')} </h4></Modal.Header>
                    <Modal.Content scrolling>
                            ...
                            <div className='text-end mb-2'>
                                <Button  onClick={() => SaveFournisseur()} disabled={saveBtnState} className='text-end rounded-pill bg-system-btn ' positive>  <Icon name='save outline' /> {t('menuTabs.ordonancePage.stockCard.addCard.enregistrerBtn')} </Button>
                    ...
                    <Modal.Actions>
                                <Button className='rounded-pill' negative onClick={ () => setModalS(false)}> <span className='bi bi-x' ></span> {t('menuTabs.ordonancePage.stockCard.addCard.fermerBtn')}</Button>
                    </Modal.Actions>
                </Modal>
