...
const MainDataCard = ({ordonanceD, setOrdonanceD,allClientList, OnKeyPressFunc, SelectClientFunction,clientNow}) =>{
    ...
    return (<>
            <div className='card card-body shadow-sm mb-2'>
                <h5> {t('menuTabs.ordonancePage.addOrdoPage.dateClientCard.title')}  </h5>
                ...
                <h4 className='mb-1 mt-1'> {t('menuTabs.ordonancePage.addOrdoPage.dateClientCard.nomClientText')} {clientNow.PA_Name  ? clientNow.PA_Name  : ''}</h4>
                <h4 className='mt-1 mb-1'> {t('menuTabs.ordonancePage.addOrdoPage.dateClientCard.adresseClientText')} {clientNow.Adress  ? clientNow.Adress  : ''} </h4>
            </div>
    </>)
}
...
function AjouterFacture() {
    ...
    const panes = [
        {
            menuItem: { key: 'start', icon: 'add circle', content: t('menuTabs.ordonancePage.addOrdoPage.tabsText.enter') }, 
            render: () => <AddArticles />,
        },
        {
            menuItem: { key: 'client', icon: 'user', content:  t('menuTabs.ordonancePage.addOrdoPage.tabsText.dateClient') }, 
        ...
        {
            menuItem: { key: 'articles', icon: 'save', content:  t('menuTabs.ordonancePage.addOrdoPage.tabsText.save') }, 
            render: () => <ButtonsCard />,
        }
    ...
    const AddMedicammentToListe = ()=>{
        if (!articleNow.PK) {toast.error("Code à barre Invalide !", GConf.TostErrorGonf)}
                ...
                let arrayToAdd = {id: ordonanceD.OR_Articles.length + 1 , PK: articleNow.PK, Nom: articleNow.Nom, Dosage: articleNow.Dosage, DCI: articleNow.DCI, Forme: articleNow.Forme, Presentation: articleNow.Presentation, Emploi_Mode: articleNow.Emploi_Mode}
   ...
   const AddArticles = () =>{
    return (<>
            <div className='card card-body shadow-sm mb-2 border-div'>
                <h5>{t('menuTabs.ordonancePage.addOrdoPage.enterCard.title')}</h5> 
                ...
                <Input icon='pin' list="articlesList" placeholder={t('menuTabs.ordonancePage.addOrdoPage.enterCard.addPlch')}  onBlur={ (e) => GetMedicammentData(e.target.value)} size="small" iconPosition='left'   fluid className='mb-1' /> 
                <div className='m-2 mb-0 text-secondary'><b> <span className='bi bi-upc '></span> {t('menuTabs.ordonancePage.addOrdoPage.enterCard.code')} {articleNow.PK} </b></div>
                <div className='m-2 mb-0 text-danger'><b><span className='bi bi-star-fill '></span> {t('menuTabs.ordonancePage.addOrdoPage.enterCard.Nom')} {articleNow.Nom} </b></div> 
                ...
                <div className='m-2 mb-0 text-info'><b><span className='bi bi-star-fill '></span> {t('menuTabs.ordonancePage.addOrdoPage.enterCard.Dosage')} {articleNow.Dosage} </b></div> 
                <div className='m-2 mb-0 text-primary'><b><span className='bi bi-star-fill '></span> {t('menuTabs.ordonancePage.addOrdoPage.enterCard.Forme')} {articleNow.Forme} </b></div> 
                <div className='m-2 mb-2 text-warning'><b><span className='bi bi-star-fill '></span> {t('menuTabs.ordonancePage.addOrdoPage.enterCard.Presentation')} {articleNow.Presentation} </b></div> 
                 
                <div className='row'>
                   <div className='col-12'>  <Input icon='dropbox' type='text' value={articleNow.Emploi_Mode} autoFocus={autofocusState} onChange={ (e) => {articleNow.Emploi_Mode = e.target.value}} size="small" iconPosition='left' placeholder={t('menuTabs.ordonancePage.addOrdoPage.enterCard.modePlch')}  fluid className='mb-1' /> </div> 
                </div>
                
                <br />
                <Button disabled={saveBtnState} className='rounded-pill bg-system-btn' onClick={AddMedicammentToListe}>  <Icon name='edit outline' /> {t('menuTabs.ordonancePage.addOrdoPage.enterCard.btnText')}</Button>
            </div>
    </>)
    }
    ...
    const ButtonsCard = () =>{
        return (<>
                <div className='card card-body shadow-sm mb-2 border-div'>
                    <h5>{t('menuTabs.ordonancePage.addOrdoPage.finisCard.titleText')}</h5>
                    <div className='row mb-2'>
                        <div className='col-12'>
                            <Button  className='rounded-pill bg-system-btn' disabled={saveBtnState} fluid onClick={SaveOrdonance}><Icon name='save' /> {t('menuTabs.ordonancePage.addOrdoPage.finisCard.saveBtn')} <Loader inverted active={loaderState} inline size='tiny' className='ms-2'/></Button>
                        </div>
                    </div>
                    <div className='row mb-2'>
                        <div className='col-12'>
                            <Button  className='rounded-pill btn-imprimer' disabled={!saveBtnState} fluid onClick={(e) => PrintFunction('printOrdonance')}><Icon name='print' /> {t('menuTabs.ordonancePage.addOrdoPage.finisCard.printBnt')} </Button>
                        </div>
    ...
    return (<>
        ...
        <div className='row'>
            ...
            <div className='col-12 col-lg-7'>
                ...
                <h5>{t('menuTabs.ordonancePage.addOrdoPage.listeText')}</h5>    
