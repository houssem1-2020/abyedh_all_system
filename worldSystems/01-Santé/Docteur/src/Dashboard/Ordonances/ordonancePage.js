...
function FacturePage() {
    ...
    const [selectedArticle, setSelectedArticle] = useState([])
     ...
     useEffect(() => {
        ...
        .then(function (response) {
            ...
            _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/or/info/${getData.OR_ID}`)}><span className='d-none d-lg-inline'> {t('menuTabs.ordonancePage.InfoBtn')} </span><Icon  name='angle right' /></Button>)
            ],))
    ...
    const StateCard = ({ status }) => {
        ...
        const statusCard = React.useCallback(() => {
          switch(status) {
            case 'O': return <StateCard color='success' text={t('menuTabs.ordonancePage.ordoState.terminer')} />;  
            case 'Credit': return <StateCard color='danger' text={t('menuTabs.ordonancePage.ordoState.annule')} /> ;
    ...
    return (<>
        <Fade>
          ...
          <div className='row'>
                <div className='col-12 col-lg-8'><SubNav dataForNav={GConf.SubNavs.camion} snTag='camion' /></div>
                <div className='col-12 col-lg-4 text-end align-self-center'><MainSubNavCard text={t('menuTabs.ordonancePage.StockText')} link='sk' icon='window-dock' />  </div>
            </div>
            <br />
            <TableGrid tableData={facturesList} columns={t(`TableHead.ordonance`).split(/,[\s]*'/).map(item => item.replace(/'/g, '')).filter(Boolean)}  />
        </Fade>
        <Modal
                size='small'
                open={modalS}
