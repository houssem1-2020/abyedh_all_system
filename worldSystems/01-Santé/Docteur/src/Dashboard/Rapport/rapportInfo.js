...
function FactureInfo() {
    ...
    const FactureHeader = () =>{
        return(<>
                <h2 className='text-center'> {t('menuTabs.rapportPage.rapportInfo.titleText')} </h2> 
                <div className='row'>
                    <div className='col-6'>
                        <div className='text-secondary'><b> {t('menuTabs.rapportPage.rapportInfo.rapportId')} : </b> {RPID}</div>
                        <div className='text-secondary'><b> {t('menuTabs.rapportPage.rapportInfo.rapportTitre')} : </b> {factureData.RA_Titre}</div>
                        <div className='text-secondary'><b> {t('menuTabs.rapportPage.rapportInfo.sujetText')} {factureData.RA_Sujet} </b> 
                    </div>
                    </div>
                    <div className='col-6'>
                        <div className='text-danger'><b> {t('menuTabs.rapportPage.rapportInfo.dateText')} </b> {new Date(factureData.RA_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' )} </div>
                        <div className='text-secondary'><b> {t('menuTabs.rapportPage.rapportInfo.GenreText')}  </b> {factureData.RA_Genre} </div>
    ...
    const BtnsCard = () =>{
        return(<>
                <div className='card card-body border-div shadow-sm mb-2'>
                    <h5>{t('menuTabs.rapportPage.rapportInfo.controlCard.title')}</h5>
                    <div className='row mb-2'>
                        <div className='col-6'>
                                <Button as='a' onClick={ (e) => NavigateFunction(`/S/rp/modifier/${RPID}`)} animated   className='rounded-pill bg-system-btn'  fluid>
                                    <Button.Content visible><Icon name='edit outline' /> {t('menuTabs.rapportPage.rapportInfo.controlCard.editBtn')} </Button.Content>
                        ...
                        <div className='col-6 mb-3'>
                            <Button  className='rounded-pill btn-imprimer'  fluid onClick={(e) => PrintFunction('printRapport')}><Icon name='print' /> {t('menuTabs.rapportPage.rapportInfo.controlCard.printBtn')} </Button>
                        </div>
                        <div className='col-12 '>
                            <Button  className='rounded-pill bg-danger text-white'  fluid  onClick={() => setModalS(true)}><Icon name='trash' /> {t('menuTabs.rapportPage.rapportInfo.controlCard.deleteBtn')} </Button>
                        </div>
