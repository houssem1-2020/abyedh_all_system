...
function RequestPage() {
    ...
    useEffect(() => {
        ...
        .then(function (response) {
            if (!response.data) {
              ...
              _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/rq/rs/info/${commandeDate.R_ID}`)}><span className='d-none d-lg-inline'> {t('menuTabs.rdvPage.infoBtn')} </span><Icon  name='angle right' /></Button>)
          ],))
   ...
   /*#########################[Function]##################################*/
      ...
      return(<>
        <div className='row mb-3'>
              ...
              <div style={{width:'100%', overflowX: 'auto', overflowY : 'hidden',  whiteSpace:'nowrap'}}>
                <Menu secondary >
                    ...
                    <Menu.Item active={activeIndex == 1} className='rounded-pill' onClick={ () => FetchByGenreReserv('S',1) }>
                      <span className='text-primary' onClick={ () => FetchByGenreReserv('W') }>
                        <b>
                    ...
                    <Menu.Item active={activeIndex == 4} className='rounded-pill' onClick={ () => FetchByGenreReserv('RT',4) }>
                    ...
                    <Menu.Item active={activeIndex == 5} className='rounded-pill' onClick={ () => FetchByGenreReserv('RD',5) }>
                    ...
                    <Menu.Item active={activeIndex == 6} className='rounded-pill' onClick={ () => FetchByGenreReserv('T',6) }>
          ...
          <div className='col-12 col-lg-2 align-self-center text-end'>
              ...
              <MainSubNavCard avCard text={t(`menuTabs.rdvPage.calendarLink`)} link='rq/calendrier' icon='calendar-week' />
          </div>
        </div>
      </>)
    }
    ...
    return (<>
        ...
        <Fade>
                  <CustomTabs  /> 
                  <TableGrid tableData={reservationList} columns={t(`TableHead.reservation`).split(/,[\s]*'/).map(item => item.replace(/'/g, '')).filter(Boolean)} />
