    ...
    useEffect(() => {
        ...
        .then(function (response) {
            if (!response.data) {
              ...
              _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/${OneGConf.routerName}/L/rdv/info/${commandeDate.R_ID}`)}><span className='d-none d-lg-inline'> {t('menuTabs.rdvPage.infoBtn')} </span><Icon  name='angle right' /></Button>)
          ],))
   ...
   /*#########################[Function]##################################*/
    ...
    return (<>
           <div className={`${OneGConf.themeMode == 'dark' ? 'bg-dark-theme-2' : '' }`} style={{height: '100vh', overflow: 'scroll'}}>
              <BackCard data={OneGConf.backCard.rdv}/>
              <br />
              <div className='container'>
                <Fade>
                          <CustomTabs  /> 
