...
function RequestInfo() {
    ...
    const SaveClientFunction = () =>{
        ...
        axios.post(`${GConf.ApiLink}/patient/ajouter`, {
            ...
            if(response.data.affectedRows) {
                ...
                //SaveNotification('clientAjouter',OneGConf.forPID.PID, clientD)
                setLS(false)
            }
            else{
                ...
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className='row '>
                        ...
                        <div className='col-12 mb-2'>
                            <Button onClick={() => AddToCalendar(true)}  animated  className='rounded-pill bg-system-btn'  fluid>
        ...
        return(<>
                <div className='card card-body bg-transparent border-div mb-3 mt-2'>
                    ...
                    <div className="table-responsive">
                        ...
                        <tbody>
                            <tr>
                                <th scope="row"><span className='bi bi-person'></span> {t('menuTabs.rdvPage.rdvInfoCardData.infoReqCard.nameOfUser')}   </th>
                                <td>{loading ? requestData.Name : <SimpleLoadinCard />}</td>
                            </tr>
                            <tr>
                                <th scope="row"><span className='bi bi-calendar'></span> {t('menuTabs.rdvPage.rdvInfoCardData.infoReqCard.wantedDate')}   </th>
                                <td>{loading ? new Date(requestData.RDV_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ) : ''}</td>
                            </tr>
                            <tr>
                                <th scope="row"><span className='bi bi-clock'></span> {t('menuTabs.rdvPage.rdvInfoCardData.infoReqCard.wantedTime')}  </th>
                                <td>{loading ? requestData.RDV_Time : <SimpleLoadinCard />}</td>
                            </tr>
                            <tr>
                                <th scope="row"><span className='bi bi-calendar-check'></span> {t('menuTabs.rdvPage.rdvInfoCardData.infoReqCard.reqDay')}  </th>
                                <td>{loading ? new Date(requestData.R_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ) : <SimpleLoadinCard />}</td>
                            </tr>
                            <tr>
                                <th scope="row"><span className='bi bi-chat-dots-fill'></span> {t('menuTabs.rdvPage.rdvInfoCardData.infoReqCard.comment')} </th>
    ...
    return ( <> 
        ...
        <div className={`${OneGConf.themeMode == 'dark' ? 'bg-dark-theme-2' : '' }`} style={{height: '100vh', overflow: 'scroll'}}>
            <BackCard data={OneGConf.backCard.rdvInf}/>
            <br />
            <div className='container'>
            <div className="row">
            <div className="col-12 col-lg-8">
                ...
                <div className='card card-body mb-4 border-div'>
                        ...
                        <div className='card-body mb-2'>
                            ...
                            {
                                messagesListe.map((data,index) => <div className='mb-2'><span className={`bi bi-${data.PID == OneGConf.forPID.PID ? 'laptop'  : 'person'} text-gray me-2`}></span> <span   className={`${data.PID == OneGConf.forPID.PID ? 'bg-gray  text-dark '  : 'bg-info'} rounded-pill p-1  ps-2 pe-2`} key={index}>{data.Content} {data.PID == OneGConf.forPID.PID ? <Button size='tiny' icon className='p-0 bg-transparent text-danger' onClick={() => DeleteMessage(data.PK)} > <span className='bi bi-trash'></span></Button> : <></>} </span></div>)
                            }
