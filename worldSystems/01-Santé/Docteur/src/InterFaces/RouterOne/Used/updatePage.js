...
function UploadeCamionPage() {
    ...
    let [rendredData, setRendredData] = useState([]); 
    ...
    let caisseD = JSON.parse(localStorage.getItem(`${OneGConf.routerTagName}_forPID`));
    ...
    useEffect(() => {   
        ...
        axios.post(`${GConf.SharedApi}/subsystem/update`, {
            forPID : OneGConf.forPID.PID,
            offlineData : OneGConf.offlineData,
        ...
        <div className='col-12 col-lg '>
            <div className={`card card-body shadow-sm mb-2 ${OneGConf.themeMode == 'dark' ? 'bg-dark-theme-1 text-white' : 'text-danger' }`}>
                <div className='text-left mb-2  '>
                    <b>{props.data.textName}</b>
                </div>
                <div className='align-self-center mb-3'>
                    <div style={{ width: 100, height: 100 , textAlign:'center'}}>
                        <CircularProgressbar className='text-center' strokeWidth={2}  value={GetPourcentageValue(rendredData[props.data.genreName],Offline[props.data.offLineValue])} maxValue={100} minValue={0} text={`${GetPourcentageValue(rendredData[props.data.genreName],Offline[props.data.offLineValue])}%`} />
                    </div> 
                </div>
                <div className='text-center'><Button size='tiny' disabled={isDisabled} fluid className=' rounded-pill bg-system-btn' icon onClick={(e) => UpdateItem(rendredData[props.data.genreName], props.data.offLineValue)}>  <Icon name='retweet' /> Mettre à Jour </Button></div> 
            </div> 
        </div>
        </>)
    }
    ...
    <div className={` spesific-commp ${OneGConf.themeMode == 'dark' ? 'bg-dark-theme-2 text-white' : 'bg-ligth-theme-2' }`} style={{height: '100vh', overflow: 'scroll'}}>
        ...
        <div className='container'>
            ...
            <br />
            <div className='row'>
                {OneGConf.offlineData.map((data,index) => <OfflineItem key={index} data={data} clickFunction={UpdateItem}/>)}
            </div>
            <br />
            <br />
            <div className='p-2'>
