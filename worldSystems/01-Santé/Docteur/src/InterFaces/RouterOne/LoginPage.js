...
function LoginPage() {
    ...
    /*#########################[Functions]##################################*/
    const logInSystem = () =>{
        if (!loginD.PID) {toast.error("Entrer Un PID !", GConf.TostErrorGonf)}
            ...
            axios.post(`${GConf.SharedApi}/subsystem/LogIn`, {
                LoginData : loginD,
                ...
                if(response.data.userData != false ) {
                        ...
                        toast.success(" Membre Exist  !", GConf.TostSuucessGonf)
                }
                else{
                    toast.error('Compte Introuvable  ! ', GConf.TostSuucessGonf)
                }
            }).catch((error) => {
                if(error.request) {
    ...
    const CheckLogin = () =>{
        if (!loginCheckValue) {toast.error("Entrer Le Smart ID !", GConf.TostErrorGonf)}
            ...
            axios.post(`${GConf.SharedApi}/subsystem/LogIn/check`, {
                LoginData : loginD,
                ...
                if(response.data.length != 0 ) {
                        ...
                        localStorage.setItem(`${OneGConf.routerTagName}_forPID`, JSON.stringify(response.data));
