...
function FacturePage() {
    ...
    const [selectedArticle, setSelectedArticle] = useState([])
     ...
     useEffect(() => {
        ...
        .then(function (response) {
            ...
            _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/sa/info/${getData.S_ID}`)}><span className='d-none d-lg-inline'> {t(`menuTabs.seancePage.infoBtn`)} </span><Icon  name='angle right' /></Button>)
            ],))
    ...
    return (<>
        <Fade>
            ...
            <div className='row'>
                <div className='col-12 col-lg-8'><SubNav dataForNav={GConf.SubNavs.facture} snTag='facture' /></div>
                <div className='col-12 col-lg-4 text-end align-self-center'><MainSubNavCard text={t(`menuTabs.seancePage.tarifName`)} link='of' icon='cash-coin' />  </div>
            </div>
            <br />
            <TableGrid tableData={facturesList} columns={t(`TableHead.seance`).split(/,[\s]*'/).map(item => item.replace(/'/g, '')).filter(Boolean)}  />
        </Fade>
        <Modal
                size='small'
                open={modalS}
