...
const EditArticle = ({forfaitD, setFordaitD, OnKeyPressFunc,  EditArticleFunction,loaderState }) =>{
    ...
    return(<>
                ...
                <h5 className='mb-1 mt-0'>{t('menuTabs.seancePage.tarifInfoCard.modifierCard.nom')}   </h5>
                <Input icon='star' iconPosition='left' placeholder={t('menuTabs.seancePage.tarifInfoCard.modifierCard.nom')} className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={forfaitD.Tarif_Name}  onChange={(e) => setFordaitD({...forfaitD, Tarif_Name: e.target.value })}/>
                
                <h5 className='mb-1 mt-0'>{t('menuTabs.seancePage.tarifInfoCard.modifierCard.tarif')} </h5>
                <Input icon='dollar' iconPosition='left' placeholder={t('menuTabs.seancePage.tarifInfoCard.modifierCard.tarif')} className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={forfaitD.Tarif}  onChange={(e) => setFordaitD({...forfaitD, Tarif: e.target.value })}/>

                <div className='row'>
                    <div className='col-12'>
                        <h5 className='mb-1'> {t('menuTabs.seancePage.tarifInfoCard.modifierCard.descr')} </h5>
                        <Form>
                            <TextArea rows={2} placeholder={t('menuTabs.seancePage.tarifInfoCard.modifierCard.descr')} onKeyPress={event => OnKeyPressFunc(event)} value={forfaitD.Tarif_Description} onChange={(e) => setFordaitD({...forfaitD, Tarif_Description: e.target.value })}/>
                        </Form>
                    </div>
                </div>
                <div className='text-end mb-5 mt-3'>
                    <Button onClick={EditArticleFunction} className='text-end rounded-pill bg-system-btn' positive>  <Icon name='edit' /> {t('menuTabs.seancePage.tarifInfoCard.modifierCard.modifierBtn')} <Loader inverted active={loaderState} inline size='tiny' className='ms-2 text-danger'/></Button>
                </div>
    </>)
}
...
function ArticleInfo() {
    ...
    const panes = [
        {
            menuItem: { key: 'resumer', icon: 'file excel', content: t(`menuTabs.seancePage.tarifInfoCard.leftTab.seance`) }, 
            render: () => <> <TableGrid tableData={forfaitListe} columns={t(`menuTabs.seancePage.tarifInfoCard.seanceTableH`).split(/,[\s]*'/).map(item => item.replace(/'/g, '')).filter(Boolean)} /> <br /></>,
        },
        {
            menuItem: { key: 'edit', icon: 'edit outline', content: t(`menuTabs.seancePage.tarifInfoCard.leftTab.modifier`) }, 
    ...
    useEffect(() => {
        axios.post(`${GConf.ApiLink}/forfait/select`, {
          ...
          .then(function (response) {
            ...
            if(!response.data.Data.Tarif_ID) {
   ...
   /*#########################[Card]##################################*/
                ...
                <div className='card card-body shadow-sm mb-2 border-div'>
                    ...
                    <div className="img-card-container text-center">
                        <div className="card-container notification bg-white">
                            <img src={`https://cdn.abyedh.com/images/system/docteur/tarif.png`} className="rounded-circle" width="70px" height="70px" />                    
                        </div>
                    </div>
                    <div className="mt-5 text-center">
                            <h4 className='mt-2'>{loading ? props.data.Tarif_Name : SKLT.BarreSkl } </h4> 
                            <h6 className="text-secondary">  {loading ? <><span className="bi bi-bookmark-star-fill"></span> { props.data.Tarif_Description }   </>: SKLT.BarreSkl} </h6>
                            <Divider horizontal className='text-secondary mt-4'>{t('menuTabs.seancePage.tarifInfoCard.prixText')}</Divider>
                                <Statistic color='red' size='tiny' className='mb-0'>
                                    {loading ?  
                                        ...
                                        : SKLT.ProfileSkl }  
                                        <Statistic.Label>{t('menuTabs.seancePage.tarifInfoCard.prixText')}</Statistic.Label>
                                </Statistic>
    ...
    return ( <> 
                <BreadCrumb links={GConf.BreadCrumb.forfraitInfo} bcTag='forfraitInfo' />
                <br />
                <div className="row">
