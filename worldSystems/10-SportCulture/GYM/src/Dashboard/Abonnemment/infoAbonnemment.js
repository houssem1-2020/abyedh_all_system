    ...
    <>
      ...
      <datalist id="clientList">
        {membreListe.map((test) => (
          <option key={test.ME_ID} value={test.ME_ID}>
            {test.ME_Name} : {test.Phone}
          </option>
        ))}
      </datalist>
      <Input
        ...
        list="clientList"
        placeholder={abonnemmentData.Membre_ID}
        onBlur={(e) =>
          setAbonnemmentData({ ...abonnemmentData, Membre_ID: e.target.value })
        }
        size="small"
        iconPosition="left"
        fluid
      ...
      <Dropdown
        ...
        className="mb-1 shadow-sm"
        onChange={(e, { value }) =>
          setAbonnemmentData({ ...abonnemmentData, Forfait_ID: value })
        }
      ...
      <div className="row">
        <div className="col-6">
          ...
          <Select
            ...
            value={abonnemmentData.AB_Saisson}
            onChange={(e, data) =>
              setAbonnemmentData({ ...abonnemmentData, AB_Saisson: data.value })
            }
          />
        </div>
        <div className="col-6">
          ...
          <Input
            ...
            className="mb-1 shadow-sm"
            value={
              new Date(abonnemmentData.AB_Depart_Date)
                .toISOString()
                .split("T")[0]
            }
            onChange={(e) =>
      ...
      <div className="row">
        <div className="col-6">
          ...
          <Input
            ...
            fluid
            className="mb-1 shadow-sm"
            value={abonnemmentData.AB_Depart_Time}
            onChange={(e) =>
              setAbonnemmentData({
                ...abonnemmentData,
                AB_Depart_Time: e.target.value,
              })
            }
          />
        </div>
        <div className="col-6">
          <h5 className="mb-0 mt-2 text-secondary ">
            Temps d'entrainemment Terminer{" "}
          </h5>
          <Input
            icon="time"
            type="time"
            ...
            fluid
            className="mb-1 shadow-sm"
            value={abonnemmentData.AB_Termine_Time}
            onChange={(e) =>
              setAbonnemmentData({
                ...abonnemmentData,
                AB_Termine_Time: e.target.value,
              })
            }
          />
        </div>
...
function FactureInfo() {
  ...
  const [renouvellmmentData, setRnouvellmmentData] = useState({
    AB_Depart_Date: new Date().toISOString().split("T")[0],
  });
        ...
        <>
          <Tab.Pane attached={false}>
            <EditArticle
              ...
              setAbonnemmentData={setAbonnemmentData}
              forfaitListe={forfaitListe}
              membreListe={membreListe}
              EditAbonnemtInfo={EditAbonnemtInfo}
              loaderState={loaderState}
            />
          </Tab.Pane>
          <br />
  ...
  useEffect(() => {
    axios
      .post(`${GConf.ApiLink}/abonnement/select`, {
        PID: GConf.PID,
        fid: FID,
      })
      .then(function (response) {
        if (response.data.Data.length == 0) {
          ...
          setTimeout(() => {
            window.location.href = "/S/ab";
          }, 2000);
        } else {
          setAbonnemmentData(response.data.Data);
          let SeanceL = [];
          response.data.Seances.map((Seance) =>
            SeanceL.push({
              title: Seance.SE_Date,
              date: Seance.SE_Date,
  ...
  const EditAbonnemtInfo = (event) => {
    setLS(true);
    axios
      .post(`${GConf.ApiLink}/abonnement/modifier`, {
        PID: GConf.PID,
        abonnemmentData: abonnemmentData,
        AB_ID: FID,
      })
  ...
  const CheckPaymmentOfMonth = (value) => {
    if (loading) {
      let searchForMonth = JSON.parse(abonnemmentData.AB_Paymment).find(
        (data) => data.mois == new Date().getMonth() + 1
      );
      if (searchForMonth) {
        return true;
      } else {
  ...
  const SavePymmentFunc = () => {
    let arrayToAdd = {
      id: paymmentMonth,
      mois: paymmentMonth,
      moisNom: MoinsListe[paymmentMonth - 1].text,
      payee: abonnemmentData.Tarif,
      datePM: new Date().toISOString().split("T")[0],
    };
    ...
    setAbonnemmentData({
      ...abonnemmentData,
      AB_Paymment: JSON.stringify(toAdd),
    });
    axios
      .post(`${GConf.ApiLink}/abonnement/payee`, {
        PID: GConf.PID,
  ...
  const DeleteTrancheFunc = (itemToRemove) => {
      let paymmentList = JSON.parse(abonnemmentData.AB_Paymment).filter(item => 
          item.mois !== itemToRemove.mois || 
          item.moisNom !== itemToRemove.moisNom ||
          item.payee !== itemToRemove.payee ||
          item.datePM !== itemToRemove.datePM
        );
          ...
          <div className="card card-body shadow-sm mb-2 border-div">
            ...
            <div className="img-card-container text-center">
              <div className="card-container notification">
                <img
                  src={`https://cdn.abyedh.com/images/system/gym/abonnemment.png`}
            ...
            <div className="mt-5 text-center">
              <h4 className="mb-0 mt-1">
                ...
                {loading ? props.data.ME_Name : SKLT.BarreSkl}{" "}
              </h4>
              <h4 className="mb-0 mt-1">
                <span className="bi bi-bookmark-star-fill"></span>{" "}
                {loading ? props.data.F_Name : SKLT.BarreSkl}{" "}
              </h4>
                  ...
                  <Statistic color="red" size="tiny">
                    {loading ? (
                      <Statistic.Value>
                        <h3>
                          {" "}
                          {new Date(props.data.AB_Depart_Date)
              ...
              <div className="row text-center">
                <div className="col-6  align-self-center border-end">
                  <h2 className="mb-0">
                    {loading ? props.data.NB_Seance : ""}
                  </h2>
                  <small className="text-secondary">Deja</small>
                </div>
                <div className="col-6 align-self-center">
                  <h2 className="mb-0">
                    {loading ? props.data.NB_Seance - seanceListe.length : ""}
                  </h2>
        ...
        <div className="row">
          <div className="col-6">
            {CheckPaymmentOfMonth() ? (
              ...
              <>
                <Select
                  placeholder="Choisir Uu Poste"
                  options={MoinsListe}
                  className="w-100 shadow-sm rounded mb-3"
                  value={paymmentMonth}
                  onChange={(e, data) => setPaymmentMonth(data.value)}
                />
                <Button
                  onClick={SavePymmentFunc}
                  className="text-end rounded-pill bg-system-btn "
                  disabled={CheckPaymmentOfMonth()}
                  positive
                >
                  {" "}
                  <Icon name="save outline" /> Payee Le Mois Sélectionner{" "}
                  <Loader
                    inverted
                    active={loaderState}
                    inline
          ...
          <div className="col-6">
            ...
            <ul></ul>
            {JSON.parse(abonnemmentData.AB_Paymment).map((data, index) => (
              <li key={index}>
                {data.moisNom} : {data.payee} (<Icon onClick={() => DeleteTrancheFunc(data)} className='text-danger' name='trash' />)
              </li>
            ))}
          </div>
        </div>
