      ...
      <div className="p-1">
        <div className="p-1 mb-2">
          ...
          <Input
            ...
            value={teamData.T_CIN}
            onChange={(e) =>
              setTeamData({ ...teamData, T_CIN: e.target.value })
            }
          />
        </div>
        <div className="p-1  mb-2">
          ...
          <Input
            ...
            className="w-100 border-0 shadow-sm rounded mb-1"
            value={teamData.T_Name}
            onChange={(e) =>
              setTeamData({ ...teamData, T_Name: e.target.value })
            }
          />
        </div>
        <div className="p-1 mb-2">
          ...
          <Input
            ...
            className="w-100 border-0 shadow-sm rounded mb-1"
            value={teamData.T_Phone}
            onChange={(e) =>
              setTeamData({ ...teamData, T_Phone: e.target.value })
            }
          />
        </div>
        <div className="p-1 mb-2">
          ...
          <Select
            ...
            value={teamData.Poste}
            onChange={(e, data) =>
              setTeamData({ ...teamData, Poste: data.value })
            }
          />
        ...
        <div className="p-1 mb-2">
          ...
          <Select
            ...
            className="mb-2"
            options={TunMap.Gouv}
            value={teamData.T_Gouv}
            onChange={(e, { value }) => GetDelegList(value)}
          />
          <Select
            placeholder="Selectionnez Delegation "
            fluid
            value={teamData.T_Deleg}
            options={delegList}
            onChange={(e, { value }) =>
              setTeamData({ ...teamData, T_Deleg: value })
            }
          />
        </div>
        {/* <div className='p-1 mb-2'>
        ...
        <div className="p-1 mb-2">
          ...
          <Form>
            <TextArea
              ...
              className="w-100 shadow-sm rounded mb-3"
              value={teamData.T_Adress}
              onChange={(e) =>
                setTeamData({ ...teamData, T_Adress: e.target.value })
              }
            />
          </Form>
        </div>
      ...
      >
        ...
        <Modal.Content>
          ...
          <div className="mb-0 p-0">
            <h5> Date : {editAvanceD.Date}</h5>
          </div>
          <div>
            <h5> Key : {editAvanceD.PK} </h5>
          </div>
        </Modal.Content>
        <Modal.Actions>
...
function TeamInfo() {
  ...
  const panes = [
    {
      ...
      render: () => (
        <>
          <TableGrid
            tableData={presence}
            columns={["*", "Nom", "Jour", "Description", "X"]}
          />{" "}
        </>
      ),
    },
    ...
    {
      menuItem: { key: 'caisse', icon: 'edit', content: t('menuTabs.teamPage.teamInfoPage.TabsCardText.caissePWD') }, 
    ...
    axios
      ...
      .then(function (response) {
        ...
        if (!response.data.Data.length == 0) {
          ...
          let presenceTable = [];
          response.data.Presence.map((getData, index) =>
            presenceTable.push([
              getData.PK,
              response.data.Data.T_Name,
              new Date(getData.PR_Date)
              ...
              _(
                <Button
                  className="rounded-pill bg-danger text-white "
                  icon
                  size="mini"
                  onClick={() => openEditModal(getData, false)}
                >
                  {" "}
          ...
          let avancesTable = [];
          response.data.Avances.map((getData, index) =>
            avancesTable.push([
              getData.PK,
              response.data.Data.T_Name,
              parseFloat(getData.Valeur).toFixed(3),
              new Date(getData.AV_Date)
                .toLocaleDateString("fr-FR")
                .split("/")
                .reverse()
                .join("-"),
              _(
                <Button
                  ...
                  size="mini"
                  onClick={(e) => NavigateFunction(`/S/tm/avances`)}
                >
  ...
  const EditTeam = () => {
    ...
    } else if (!teamData.T_Name) {
      toast.error("Nom Invalide !", GConf.TostErrorGonf);
    } else if (!teamData.T_Phone) {
      toast.error("Phone Invalide !", GConf.TostErrorGonf);
    } else if (!teamData.Poste) {
      toast.error("Nom Sociale  Invalide !", GConf.TostErrorGonf);
    } else if (!teamData.T_Gouv) {
      toast.error("Gouvernorat Invalide !", GConf.TostErrorGonf);
    } else if (!teamData.T_Deleg) {
      toast.error("Delegation Invalide !", GConf.TostErrorGonf);
    } else if (!teamData.T_Adress) {
      toast.error("Adresee Invalide !", GConf.TostErrorGonf);
    } else {
      setLS(true);
      axios
        .post(`${GConf.ApiLink}/team/modifier`, {
          PID: GConf.PID,
          teamData: teamData,
        })
        .then(function (response) {
  ...
  const DeleteTeamFunc = () => {
    setLS(true);
    axios
      .post(`${GConf.ApiLink}/fournisseur/supprimer`, {
        tag: GConf.PID,
        clientId: TID,
      })
      .then(function (response) {
  ...
  const RelateToUID = () => {
    if (!inAbyedhSearch) {
      ...
      setLS(true);
      axios
        .post(`${GConf.ApiLink}/team/verification`, {
          PID: GConf.PID,
          UID: inAbyedhSearch,
          T_ID: TID,
        })
  ...
  const DeletePresence = () => {
    if (!editAvanceD.PK) {
      toast.error("Avance Invalide !", GConf.TostErrorGonf);
    } else {
      axios
        .post(`${GConf.ApiLink}/team/presence/supprimer`, {
          PID: GConf.PID,
          PK: editAvanceD.PK,
        })
        .then(function (response) {
        ...
        .catch((error) => {
          if (error.request) {
            toast.error(
              <>
                <div>
                  <h5>Probleme de Connextion</h5> Chargemment des ancien Mmebre{" "}
                </div>
              </>,
              GConf.TostInternetGonf
            );
          ...
          <div className="card card-body shadow-sm mb-2 border-div">
            ...
            <div className="img-card-container text-center">
              <div className="card-container">
                <img
                  src="https://cdn.abyedh.com/images/system/profile.png"
            ...
            <div className="mt-5 text-center">
              <h4 className="mt-2">
                {loading ? teamData.T_Name : SKLT.BarreSkl}{" "}
              </h4>
              <h6 className="text-secondary">
                {" "}
                {loading ? (
                  <>
                    <span className="bi bi-star-fill"></span> {teamData.Poste}{" "}
                  </>
                ) : (
                  SKLT.BarreSkl
                )}{" "}
                ...
                <div className="col-12 mb-3">
                  {loading ? (
                    <Statistic color="green" size="tiny">
                      <Statistic.Value>{teamData.T_Phone}</Statistic.Value>
                    </Statistic>
                  ) : (
                    SKLT.BarreSkl
                  )}
    ...
    return (
      <>
        <h3 className="text-secondary">
          Voulez-Vous Vraimment Supprimer Ce Mmebre ?
        </h3>
        <div className="row">
          <div className="col-9">
            <h5 className="text-danger text-left">
  ...
  return (
    <>
      ...
      <DeleteModal
        ...
        editAvanceD={editAvanceD}
        setEditAvanceD={setEditAvanceD}
