...
function DocteurSpecific() {
     ...
     const [returnedProfileData, setReturnedProfileData] = useState({SP_Tarif:[], SP_Classes:[], SP_Emplois:[]})
   ...
   /*#########################[Function]##################################*/
    ...
    const DiplomeListeCard = (props) =>{
        return(<>
          {returnedProfileData.SP_Emplois == '' ? 
          ...
          <>
            ...
            <br />
            <Button    fluid className='rounded-pill' size='tiny' color='blue' onClick={() => UpdateFunction('SP_Emplois')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
          </> 
          } 
      </>)
    }
