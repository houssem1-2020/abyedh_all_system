...
function InputLoginPage() {
    ...
    /*#########################[UseEffect]##################################*/
    useEffect(() => {
        const inputIsLogged = localStorage.getItem(`Admin_Dir_LocalD`);
        if (inputIsLogged) {window.location.href = "/Dir/L";}
        ...
        if (!loginD.Log) {toast.error("Entrer Un identifiant !", GConf.TostErrorGonf)}
            ...
            axios.post(`${GConf.ApiInputLink}/LogIn`, {
                ...
                if(response.data.length != 0 ) {
                        ...
                        if (!Offline) {
                            localStorage.setItem(`Admin_Dir_Offline`, JSON.stringify({localData: [], ajoutee: [],  modifiee:[],  positions:[],  horaire:[]}));
                        }
                        localStorage.setItem(`Admin_Dir_LocalD`, JSON.stringify(response.data));
