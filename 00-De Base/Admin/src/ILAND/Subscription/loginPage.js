...
function InputLoginPage() {
    ...
    let Offline = JSON.parse(localStorage.getItem(`Admin_Subs_Offline`));

    /*#########################[UseEffect]##################################*/
    useEffect(() => {
        const inputIsLogged = localStorage.getItem(`Admin_Subs_LocalD`);
        if (inputIsLogged) {window.location.href = "/Subs/L";}
    ...
    const logInSystem = () =>{
        if (!loginD.Log) {toast.error("Entrer Un identifiant !", GConf.TostErrorGonf)}
            ...
            axios.post(`${GConf.ApiInputLink}/LogIn`, {
                LoginData : loginD,
                ...
                if(response.data.length != 0 ) {
                        ...
                        if (!Offline) {
                            localStorage.setItem(`Admin_Subs_Offline`, JSON.stringify({localData: [], ajoutee: [],  modifiee:[],  positions:[],  horaire:[]}));
                        }
                        localStorage.setItem(`Admin_Subs_LocalD`, JSON.stringify(response.data));
                        window.location.href = "/Subs";
                }
                else{
                    toast.error('Ce membre n\'existe pas ! ', GConf.TostSuucessGonf)
