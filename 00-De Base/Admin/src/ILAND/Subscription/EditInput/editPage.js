       ...
       if (!selectedTable) {toast.error("Selectionne un tableaux !", GConf.TostErrorGonf)}
        ...
        axios.post(`https://api.abyedh.tn/apiAbyedh/Search/search`, {
          ...
          let testTable = []
            ...
            getData.Deleg,
            getData.Adress,
            _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/Dir/L/ed/modifier/${selectedTable}/${getData.PID}`)}><span className='d-none d-lg-inline'> Info </span><Icon  name='angle right' /></Button>)
            ],))
