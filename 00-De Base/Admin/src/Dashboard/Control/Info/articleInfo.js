...
function CamionArticleInfo() {
    ...
    /* ############################### UseEffect ################################*/ 
     useEffect(() => {
        ...
        .then(function (response) {
            ...
            let suivieTable = []
            response.data.FromSuivie.map( (getData) => suivieTable.push([new Date(getData.Jour).toLocaleDateString('en-US'), GetTargetArticleQteFromSuivie(getData.Articles)],))
    ...
    const GetTargetArticleQteFromSuivie = (value) =>{
        const JsValue = JSON.parse(value)
        const searchObject = JsValue.findIndex((article) => article[0] == code);
        return (JsValue[searchObject][2])
    }
