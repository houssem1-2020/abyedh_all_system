...
function TransporteurSpecific() {
     ...
     const [returnedProfileData, setReturnedProfileData] = useState({SP_Tarif:[], SP_Camions:[], SP_Trips:[]})
   ...
   /*#########################[Function]##################################*/
    ...
    const AddTwoDataCard = (props) =>{
      ...
      const AddArticleToList = () =>{
        if (!dataNow.Forfait) { toast.error("أدخل  إسم  الشاحنة   !", GConf.TostErrorGonf) } 
        else if (!dataNow.Prix) { toast.error("أدخل المعرف المنجمي  !", GConf.TostErrorGonf) } 
        else if (!dataNow.Description) { toast.error("أدخل الوصف  !", GConf.TostErrorGonf) } 
        else {
            if (returnedProfileData.SP_Camions == '') {
                let emptyArray = []
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Camions:JSON.stringify(emptyArray) })
                setTwoDataWas(true)
            } else {    
                let emptyArray = JSON.parse(returnedProfileData.SP_Camions)
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Camions: JSON.stringify(emptyArray) })
                setTwoDataWas(true)
            }
      ...
      return(<>
       ...
       <div className='card-body  mb-4 text-end' >
          <input placeholder='Nom du Camion'   className='text-start form-control mb-1 rounded' onChange={ (e) => setDataNow({...dataNow, Forfait: e.target.value })} />
          <input  placeholder='Matricule'   className='text-start form-control mb-1 rounded' onChange={ (e) => setDataNow({...dataNow, Prix: e.target.value })} />
          <input placeholder='Marque de Camion'   className='text-start form-control mb-1 rounded' onChange={ (e) => setDataNow({...dataNow, Description: e.target.value })} />
          <br />
    ...
    const TwoDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
            {returnedProfileData.SP_Camions == '' || (JSON.parse(returnedProfileData.SP_Camions).length == 0 && !addTwoDataWas) ? <ListeVide icon='diagram-3-fill' />
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
              {JSON.parse(returnedProfileData.SP_Camions).map((data,index) => 
                <div className='  p-2 border-div mb-2' key={index}>
                    <div className='row'>
                        <div className='col-10 align-self-center'><h5 className='mt-0 mb-1'> {index + 1 } - {data.Forfait}</h5> <small className='mb-0'>{data.Description} -- {data.Prix}</small></div> 
                         
                        <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Camions')}></Button></div> 
                    </div>
                </div>
              )}
              </div>
              <br />
              <div className='text-end '>
                  <Button   className='rounded-pill ' size='tiny' disabled={addTwoDataWas ? false : true} color={addTwoDataWas ? 'blue' : undefined}  onClick={() => UpdateFunction('SP_Camions')}>  <Icon name='check' className='ms-2' /> Modifier </Button>
              </div>
            </>
            } 
          </>}
    ...
    const AddThreeDataCard = (props) =>{
        ...
        const AddArticleToList = () =>{
          if (dataNow.de == '') { toast.error("أدخل مكان الأنطلاق !", GConf.TostErrorGonf) } 
          else if (dataNow.annee == '') { toast.error("أدخل  يوم الأنطلاق  !", GConf.TostErrorGonf) } 
          else if (dataNow.vers == '') { toast.error("أدخل مكان الوصول  !", GConf.TostErrorGonf) } 
          else {
              if (returnedProfileData.SP_Trips == '') {
                  let emptyArray = []
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Trips:JSON.stringify(emptyArray) })
                  setThreeDataWas(true)
              } else {    
                  let emptyArray = JSON.parse(returnedProfileData.SP_Trips)
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Trips: JSON.stringify(emptyArray) })
                  setThreeDataWas(true)
              }
        ...
        return(<>
            <div className='card-body  mb-4 text-end'>
                <input placeholder='De '   className='text-start form-control mb-1 rounded' onChange={ (e) => setDataNow({...dataNow, de: e.target.value })} />
                <input placeholder='Vers'   className='text-start form-control mb-1 rounded' onChange={ (e) => setDataNow({...dataNow, vers: e.target.value })} />
    ...
    const ThreeDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
          {returnedProfileData.SP_Trips == '' || (JSON.parse(returnedProfileData.SP_Trips).length == 0 && !addThreeDataWas) ?  <ListeVide icon='table' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Trips).map((data,index) => 
              <div className='  p-2 border-div mb-2' key={index}>
                  <div className='row'>
                      <div className='col-10 align-self-center'><h5 className='mt-0 mb-1'>{data.de} à  {data.vers} </h5> <small className='mb-0'>{data.annee}</small></div> 
                      <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Trips')}></Button></div> 
                  </div>
              </div>
            )}
            </div>
            <br />
            <div className='text-end '>
                <Button   className='rounded-pill' size='tiny'   disabled ={addThreeDataWas ? false : true}  color={addThreeDataWas ? 'blue' : undefined} onClick={() => UpdateFunction('SP_Trips')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
            </div>
    ...
    return (<>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-truck'></span>  Camions  : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-map'></span>  Voyages  : </h4></div>
          ...
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-geo-alt-fill'></span>  Positions  : </h4></div>
