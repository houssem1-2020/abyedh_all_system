...
function AvocatSpecific() {
     ...
     const [returnedProfileData, setReturnedProfileData] = useState({SP_Tarif:[], SP_Promotion:[], SP_Diplomes:[]})
   ...
   /*#########################[Function]##################################*/
    ...
    const TwoDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
            {returnedProfileData.SP_Promotion == '' || (JSON.parse(returnedProfileData.SP_Promotion).length == 0 && !addTwoDataWas) ? <ListeVide icon='megaphone' />
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
              ...
              <br />
              <div className='text-end '>
                  <Button   className='rounded-pill ' size='tiny' disabled={addTwoDataWas ? false : true} color={addTwoDataWas ? 'blue' : undefined}  onClick={() => UpdateFunction('SP_Promotion')}>  <Icon name='check' className='ms-2' /> Modifier </Button>
              </div>
            </>
            } 
          </>}
