...
function DocteurSpecific() {
     ...
     const [addCoash, setAddCoash] = useState(false)
     ...
     const [returnedProfileData, setReturnedProfileData] = useState({SP_Tarif:[], SP_Props:[], SP_Teams:[], SP_Coach:[]})
   ...
   /*#########################[Function]##################################*/
    ...
    const AddTwoDataCard = (props) =>{
      ...
      const AddArticleToList = () =>{
        if (!dataNow.Forfait) { toast.error("أدخل  إسم العرض   !", GConf.TostErrorGonf) } 
            ...
            if (returnedProfileData.SP_Props == '') {
                let emptyArray = []
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Props:JSON.stringify(emptyArray) })
                setTwoDataWas(true)
            } else {    
                let emptyArray = JSON.parse(returnedProfileData.SP_Props)
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Props: JSON.stringify(emptyArray) })
                setTwoDataWas(true)
            }
    ...
    const TwoDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
            {returnedProfileData.SP_Props == '' || (JSON.parse(returnedProfileData.SP_Props).length == 0 && !addTwoDataWas) ? <ListeVide icon='boxes' />
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
              {JSON.parse(returnedProfileData.SP_Props).map((data,index) => 
                <div className='  p-2 border-div mb-2' key={index}>
                    <div className='row'>
                        <div className='col-7 align-self-center'><h5 className='mt-0 mb-1'> {index + 1 } - {data.Forfait}</h5> <small className='mb-0'>{data.Description}</small></div> 
                        <div className='col-3 align-self-center'>{data.Prix} D.T</div> 
                        <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Props')}></Button></div> 
                    </div>
                </div>
              )}
              </div>
              <br />
              <div className='text-end '>
                  <Button   className='rounded-pill ' size='tiny' disabled={addTwoDataWas ? false : true} color={addTwoDataWas ? 'blue' : undefined}  onClick={() => UpdateFunction('SP_Props')}>  <Icon name='check' className='ms-2' /> Modifier </Button>
              </div>
            </>
            } 
          </>}
    ...
    const AddThreeDataCard = (props) =>{
        ...
        const AddArticleToList = () =>{
          if (dataNow.diplome == '') { toast.error("أدخل  إسم الشهادة   !", GConf.TostErrorGonf) } 
              ...
              if (returnedProfileData.SP_Teams == '') {
                  let emptyArray = []
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Teams:JSON.stringify(emptyArray) })
                  setThreeDataWas(true)
              } else {    
                  let emptyArray = JSON.parse(returnedProfileData.SP_Teams)
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Teams: JSON.stringify(emptyArray) })
                  setThreeDataWas(true)
              }
    ...
    const ThreeDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
          {returnedProfileData.SP_Teams == '' || (JSON.parse(returnedProfileData.SP_Teams).length == 0 && !addThreeDataWas) ?  <ListeVide icon='person-lines-fill' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Teams).map((data,index) => 
              <div className='  p-2 border-div mb-2' key={index}>
                  <div className='row'>
                      ...
                      <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Teams')}></Button></div> 
                  </div>
              </div>
            )}
            </div>
            <br />
            <div className='text-end '>
                <Button   className='rounded-pill' size='tiny'   disabled ={addThreeDataWas ? false : true}  color={addThreeDataWas ? 'blue' : undefined} onClick={() => UpdateFunction('SP_Teams')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
            </div>
    ...
    const AddFourDataCard = (props) =>{
      ...
      const AddArticleToList = () =>{
        if (dataNow.diplome == '') { toast.error("أدخل  إسم الشهادة   !", GConf.TostErrorGonf) } 
            ...
            if (returnedProfileData.SP_Coach == '') {
                let emptyArray = []
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Coach:JSON.stringify(emptyArray) })
                setFourDataWas(true)
            } else {    
                let emptyArray = JSON.parse(returnedProfileData.SP_Coach)
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Coach: JSON.stringify(emptyArray) })
                setFourDataWas(true)
            }
    ...
    const FourDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
          {returnedProfileData.SP_Coach == '' || (JSON.parse(returnedProfileData.SP_Coach).length == 0 && !addFourDataWas) ?  <ListeVide icon='people' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Coach).map((data,index) => 
              <div className='  p-2 border-div mb-2' key={index}>
                  <div className='row'>
                      <div className='col-10 align-self-center'><h5 className='mt-0 mb-1'>{data.annee} : {data.diplome}</h5> <small className='mb-0'>{data.faculte}</small></div> 
                      <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Coach')}></Button></div> 
                  </div>
              </div>
            )}
            </div>
            <br />
            <div className='text-end '>
                <Button   className='rounded-pill' size='tiny'   disabled ={addFourDataWas ? false : true}  color={addFourDataWas ? 'blue' : undefined} onClick={() => UpdateFunction('SP_Coach')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
            </div>
    ...
    const AssuranceListeCard = (props) =>{
          return(<>
            {returnedProfileData.SP_Props == '' ? 
            <ListeVide icon='boxes' /> 
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
                {JSON.parse(returnedProfileData.SP_Props).map((data,index) => 
              ...
              <br />
              <Button    fluid className='rounded-pill' size='tiny' color='blue' onClick={() => UpdateFunction('SP_Props')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
            </> 
            } 
        </>)
    }
    ...
    const DiplomeListeCard = (props) =>{
        return(<>
          {returnedProfileData.SP_Teams == '' ? 
          <ListeVide icon='person-lines-fill' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Teams).map((data,index) => 
            ...
            <br />
            <Button    fluid className='rounded-pill' size='tiny' color='blue' onClick={() => UpdateFunction('SP_Teams')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
          </> 
          } 
      </>)
    }

    const AddCoash = (props) =>{
      const [dataNow, setDataNow] = useState({PK: 1 , Name:'', Qte: ''}) 
    ...
    const CoashListeCard = (props) =>{
        return(<>
          {returnedProfileData.SP_Coach == '' ? 
          <ListeVide icon='people' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Coach).map((data,index) => 
            ...
            <br />
            <Button    fluid className='rounded-pill' size='tiny' color='blue' onClick={() => UpdateFunction('SP_Coach')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
          </> 
          } 
      </>)
    }
    return (<>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-boxes'></span>  Dimension  : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-person-lines-fill'></span>  Equipes  : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-people'></span>  Coash  : </h4></div>
