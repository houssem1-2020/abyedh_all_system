...
function BoutiqueSpecific() {
     ...
     const [returnedProfileData, setReturnedProfileData] = useState({SP_Articles:[], SP_Genres:[], SP_Promotion:[]})
   ...
   /*#########################[Function]##################################*/
    ...
    const AddOneDataCard = (props) =>{
      ...
      const AddArticleToList = () =>{
        if (!dataNow.Forfait) { toast.error("أدخل  إسم العرض   !", GConf.TostErrorGonf) } 
            ...
            if (returnedProfileData.SP_Articles == '') {
                let emptyArray = []
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Articles:JSON.stringify(emptyArray) })
                setOneDataWas(true)
            } else {    
                let emptyArray = JSON.parse(returnedProfileData.SP_Articles)
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Articles: JSON.stringify(emptyArray) })
                setOneDataWas(true)
            }
    ...
    const OneDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
            {returnedProfileData.SP_Articles == '' || (JSON.parse(returnedProfileData.SP_Articles).length == 0 && !addOneDataWas) ? <ListeVide icon='star-fill' />
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
              {JSON.parse(returnedProfileData.SP_Articles).map((data,index) => 
                <div className='  p-2 border-div mb-2' key={index}>
                    <div className='row'>
                        ...
                        <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Articles')}></Button></div> 
                    </div>
                </div>
              )}
              </div>
              ...
              <div className='text-end '>
                  <Button   className='rounded-pill ' size='tiny' disabled={addOneDataWas ? false : true} color={addOneDataWas ? 'blue' : undefined}  onClick={() => UpdateFunction('SP_Articles')}>  <Icon name='check' className='ms-2' /> Modifier </Button>
              </div>
            </>
            } 
          </>}
    ...
    const AddTwoDataCard = (props) =>{
      ...
      const AddArticleToList = () =>{
        if (!dataNow.Forfait) { toast.error("أدخل  إسم العرض   !", GConf.TostErrorGonf) } 
            ...
            if (returnedProfileData.SP_Genres == '') {
                let emptyArray = []
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Genres:JSON.stringify(emptyArray) })
                setTwoDataWas(true)
            } else {    
                let emptyArray = JSON.parse(returnedProfileData.SP_Genres)
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Genres: JSON.stringify(emptyArray) })
                setTwoDataWas(true)
            }
    ...
    const TwoDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
            {returnedProfileData.SP_Genres == '' || (JSON.parse(returnedProfileData.SP_Genres).length == 0 && !addTwoDataWas) ? <ListeVide icon='bookmarks-fill' />
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
              {JSON.parse(returnedProfileData.SP_Genres).map((data,index) => 
                <div className='  p-2 border-div mb-2' key={index}>
                    <div className='row'>
                        <div className='col-7 align-self-center'><h5 className='mt-0 mb-1'> {index + 1 } - {data.Forfait}</h5> <small className='mb-0'>{data.Description}</small></div> 
                        <div className='col-3 align-self-center'>{data.Prix} D.T</div> 
                        <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Genres')}></Button></div> 
                    </div>
                </div>
              )}
              </div>
              <br />
              <div className='text-end '>
                  <Button   className='rounded-pill ' size='tiny' disabled={addTwoDataWas ? false : true} color={addTwoDataWas ? 'blue' : undefined}  onClick={() => UpdateFunction('SP_Genres')}>  <Icon name='check' className='ms-2' /> Modifier </Button>
              </div>
            </>
            } 
          </>}
    ...
    const AddThreeDataCard = (props) =>{
        ...
        const AddArticleToList = () =>{
          if (dataNow.diplome == '') { toast.error("أدخل  إسم الشهادة   !", GConf.TostErrorGonf) } 
              ...
              if (returnedProfileData.SP_Promotion == '') {
                  let emptyArray = []
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Promotion:JSON.stringify(emptyArray) })
                  setThreeDataWas(true)
              } else {    
                  let emptyArray = JSON.parse(returnedProfileData.SP_Promotion)
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Promotion: JSON.stringify(emptyArray) })
                  setThreeDataWas(true)
              }
    ...
    const ThreeDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
          {returnedProfileData.SP_Promotion == '' || (JSON.parse(returnedProfileData.SP_Promotion).length == 0 && !addThreeDataWas) ?  <ListeVide icon='megaphone' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Promotion).map((data,index) => 
              <div className='  p-2 border-div mb-2' key={index}>
                  <div className='row'>
                      ...
                      <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Promotion')}></Button></div> 
                  </div>
              </div>
            )}
            </div>
            <br />
            <div className='text-end '>
                <Button   className='rounded-pill' size='tiny'   disabled ={addThreeDataWas ? false : true}  color={addThreeDataWas ? 'blue' : undefined} onClick={() => UpdateFunction('SP_Promotion')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
            </div>
    ...
    return (<>
          ...
          <div className='row mb-3'>
            <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-star-fill'></span> Liste des Articles : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-bookmarks-fill'></span>  Genes des Articles  : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-megaphone'></span>  Promotion  : </h4></div>
