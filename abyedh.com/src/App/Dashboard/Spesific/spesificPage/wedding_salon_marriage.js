...
function WeddingSallonMariageSpecific() {
     /*#########################[Const]##################################*/
     ...
     const [returnedProfileData, setReturnedProfileData] = useState({SP_Tarif:[], SP_Calendrier:[], SP_Espaces:[], SP_Structure:[]})
   ...
   /*#########################[Function]##################################*/
    ...
    const AddTwoDataCard = (props) =>{
      ...
      const AddArticleToList = () =>{
        if (!dataNow.Forfait) { toast.error("أدخل  إسم العرض   !", GConf.TostErrorGonf) } 
            ...
            if (returnedProfileData.SP_Calendrier == '') {
                let emptyArray = []
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Calendrier:JSON.stringify(emptyArray) })
                setTwoDataWas(true)
            } else {    
                let emptyArray = JSON.parse(returnedProfileData.SP_Calendrier)
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Calendrier: JSON.stringify(emptyArray) })
                setTwoDataWas(true)
            }
    ...
    const TwoDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
            {returnedProfileData.SP_Calendrier == '' || (JSON.parse(returnedProfileData.SP_Calendrier).length == 0 && !addTwoDataWas) ? <ListeVide icon='calendar-date' />
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
              {JSON.parse(returnedProfileData.SP_Calendrier).map((data,index) => 
                <div className='  p-2 border-div mb-2' key={index}>
                    <div className='row'>
                        <div className='col-7 align-self-center'><h5 className='mt-0 mb-1'> {index + 1 } - {data.Forfait}</h5> <small className='mb-0'>{data.Description}</small></div> 
                        <div className='col-3 align-self-center'>{data.Prix} D.T</div> 
                        <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Calendrier')}></Button></div> 
                    </div>
                </div>
              )}
              </div>
              <br />
              <div className='text-end '>
                  <Button   className='rounded-pill ' size='tiny' disabled={addTwoDataWas ? false : true} color={addTwoDataWas ? 'blue' : undefined}  onClick={() => UpdateFunction('SP_Calendrier')}>  <Icon name='check' className='ms-2' /> Modifier </Button>
              </div>
            </>
            } 
          </>}
    ...
    const AddThreeDataCard = (props) =>{
        ...
        const AddArticleToList = () =>{
          if (dataNow.diplome == '') { toast.error("أدخل  إسم الشهادة   !", GConf.TostErrorGonf) } 
              ...
              if (returnedProfileData.SP_Espaces == '') {
                  let emptyArray = []
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Espaces:JSON.stringify(emptyArray) })
                  setThreeDataWas(true)
              } else {    
                  let emptyArray = JSON.parse(returnedProfileData.SP_Espaces)
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Espaces: JSON.stringify(emptyArray) })
                  setThreeDataWas(true)
              }
    ...
    const ThreeDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
          {returnedProfileData.SP_Espaces == '' || (JSON.parse(returnedProfileData.SP_Espaces).length == 0 && !addThreeDataWas) ?  <ListeVide icon='grid-3x3-gap' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Espaces).map((data,index) => 
              <div className='  p-2 border-div mb-2' key={index}>
                  <div className='row'>
                      ...
                      <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Espaces')}></Button></div> 
                  </div>
              </div>
            )}
            </div>
            <br />
            <div className='text-end '>
                <Button   className='rounded-pill' size='tiny'   disabled ={addThreeDataWas ? false : true}  color={addThreeDataWas ? 'blue' : undefined} onClick={() => UpdateFunction('SP_Espaces')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
            </div>
    ...
    const AddFourDataCard = (props) =>{
      ...
      const AddArticleToList = () =>{
        if (dataNow.diplome == '') { toast.error("أدخل  إسم الشهادة   !", GConf.TostErrorGonf) } 
            ...
            if (returnedProfileData.SP_Structure == '') {
                let emptyArray = []
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Structure:JSON.stringify(emptyArray) })
                setFourDataWas(true)
            } else {    
                let emptyArray = JSON.parse(returnedProfileData.SP_Structure)
                emptyArray.push(dataNow)
                setReturnedProfileData({...returnedProfileData, SP_Structure: JSON.stringify(emptyArray) })
                setFourDataWas(true)
            }
    ...
    const FourDataListeCard = (props) =>{
        ...
        {loading ? <SekeltonCard /> : 
          <>
          {returnedProfileData.SP_Structure == '' || (JSON.parse(returnedProfileData.SP_Structure).length == 0 && !addFourDataWas) ?  <ListeVide icon='columns-gap' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Structure).map((data,index) => 
              <div className='  p-2 border-div mb-2' key={index}>
                  <div className='row'>
                      <div className='col-10 align-self-center'><h5 className='mt-0 mb-1'>{data.annee} : {data.diplome}</h5> <small className='mb-0'>{data.faculte}</small></div> 
                      <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Structure')}></Button></div> 
                  </div>
              </div>
            )}
            </div>
            <br />
            <div className='text-end '>
                <Button   className='rounded-pill' size='tiny'   disabled ={addFourDataWas ? false : true}  color={addFourDataWas ? 'blue' : undefined} onClick={() => UpdateFunction('SP_Structure')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
            </div>
    ...
    return (<>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-calendar-date'></span>  Calendrier  : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-grid-3x3-gap'></span>  Espaces  : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4 style={{color : APPConf.landing[APPConf.systemTag].colorTheme}}> <span className='bi bi-columns-gap'></span>  Structure  : </h4></div>
