...
function DocteurSpecific() {
     ...
     const [returnedProfileData, setReturnedProfileData] = useState({SP_Tarif:[], SP_Calendar:[], SP_Structure:[]})
   ...
   /*#########################[Function]##################################*/
    ...
    const AddAssurance = (props) =>{
        ...
        const AddArticleToList = () =>{
          if (dataNow.Forfait == '') { toast.error("أدخل  إسم العرض   !", GConf.TostErrorGonf) } 
              ...
              if (returnedProfileData.SP_Calendar == '') {
                  let emptyArray = []
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Calendar:JSON.stringify(emptyArray) })
              } else {    
                  let emptyArray = JSON.parse(returnedProfileData.SP_Calendar)
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Calendar: JSON.stringify(emptyArray) })
              }
          }
    ...
    const AssuranceListeCard = (props) =>{
          return(<>
            {returnedProfileData.SP_Calendar == '' ? 
            <ListeVide icon='calendar3' /> 
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
                {JSON.parse(returnedProfileData.SP_Calendar).map((data,index) => 
                  <div className='card p-2 border-div mb-2' key={index}>
                      <div className='row'>
                          <div className='col-7 align-self-center'><h5 className='mt-0 mb-1'>{data.Forfait}</h5> <small className='mb-0'>{data.Description}</small></div> 
                          <div className='col-3 align-self-center'>{data.Prix}</div> 
                          <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Calendar')}></Button></div> 
                      </div>
                  </div>
                )}
              </div>
              <br />
              <Button    fluid className='rounded-pill' size='tiny' color='blue' onClick={() => UpdateFunction('SP_Calendar')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
            </> 
            } 
        </>)
    }
    ...
    const DiplomeListeCard = (props) =>{
        return(<>
          {returnedProfileData.SP_Structure == '' ? 
          <ListeVide icon='grid-1x2' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            ...
            <br />
            <Button    fluid className='rounded-pill' size='tiny' color='blue' onClick={() => UpdateFunction('SP_Structure')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
          </> 
          } 
      </>)
    }
    ...
    return (<>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4>  Calendrier  : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4>  Structure  : </h4></div>
