...
function DocteurSpecific() {
     ...
     const [returnedProfileData, setReturnedProfileData] = useState({SP_Tarif:[], SP_Previous:[], SP_Cuisines:[]})
   ...
   /*#########################[Function]##################################*/
    ...
    const AssuranceListeCard = (props) =>{
          return(<>
            ...
            <ListeVide icon='person-bounding-box' /> 
            :
            <>
              <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
    ...
    const AddDiplome = (props) =>{
        ...
        const AddArticleToList = () =>{
          if (dataNow.Forfait == '') { toast.error("أدخل  إسم العرض   !", GConf.TostErrorGonf) } 
              ...
              if (returnedProfileData.SP_Cuisines == '') {
                  let emptyArray = []
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Cuisines:JSON.stringify(emptyArray) })
              } else {    
                  let emptyArray = JSON.parse(returnedProfileData.SP_Cuisines)
                  emptyArray.push(dataNow)
                  setReturnedProfileData({...returnedProfileData, SP_Cuisines: JSON.stringify(emptyArray) })
              }
          }
    ...
    const DiplomeListeCard = (props) =>{
        return(<>
          {returnedProfileData.SP_Cuisines == '' ? 
          <ListeVide icon='star-half' /> 
          :
          <>
            <div style={{maxHeight:'300px', overflowX:'auto', overflowX:'hidden'}}  >
            {JSON.parse(returnedProfileData.SP_Cuisines).map((data,index) => 
              <div className='card p-2 border-div mb-2' key={index}>
                  <div className='row'>
                      <div className='col-7 align-self-center'><h5 className='mt-0 mb-1'>{data.Forfait}</h5> <small className='mb-0'>{data.Description}</small></div> 
                      <div className='col-3 align-self-center'>{data.Prix}</div> 
                      <div className='col-2 align-self-center'><Button icon="trash alternate" className='rounded-circle p-2 text-danger bg-white ' onClick={() => DeleteFromTarifList(index, 'SP_Cuisines')}></Button></div> 
                  </div>
              </div>
            )}
            </div>
            <br />
            <Button    fluid className='rounded-pill' size='tiny' color='blue' onClick={() => UpdateFunction('SP_Cuisines')}>  <Icon name='plus' className='ms-2' /> Modifier </Button>
          </> 
          } 
      </>)
    }
    ...
    return (<>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4>  Travailles précédent   : </h4></div>
          ...
          <br />
          <div className='row mb-3'>
              <div className='col-10 align-self-center'><h4>  Cuisines   : </h4></div>
