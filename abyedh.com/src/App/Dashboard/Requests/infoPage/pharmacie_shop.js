    ...
    return(<>
            <div className='card card-body shadow-sm mb-2 border-div'>
                ...
                <div className='card-body'>
                    ...
                    <br />
                    Veuillez entrer le totale estimeé de la commande  : 
                    <Input icon='dollar' type='text' size="small" iconPosition='left'   fluid className='mb-1'  onChange={(e) => setRequestData({...requestData, Totale: e.target.value })}/>
                </div>
                 
                <div className=' mb-2'>
                    <Button fluid disabled={FindBtnState(reqState).acceptState} className='rounded-pill bg-success text-white'    onClick={ () => UpdateRequestState('A','Totale',requestData.Totale,true,'accepted')}><Icon name='check square' /> Accepter </Button>
                </div>
            </div>
    </>)
}
...
const LivrerCard = ({requestData, setRequestData, reqState, FindBtnState, UpdateRequestState, OnKeyPressFunc}) =>{
    return<>
        <div className='card card-body border-div mb-4 shadow-sm ltr-force' dir='ltr'>
            <div className='card-body'> Livreé Le   :  
                <h5 className='mb-1 mt-1'>Date   </h5>
                <Input icon='calendar' type='date' defaultValue={new Date().toISOString().split('T')[0]} size="small" iconPosition='left'   fluid className='mb-1'  onChange={(e) => setRequestData({...requestData, Livre_At: { ...requestData.Livre_At, Date : e.target.value  } })}/>

                <h5 className='mb-1 mt-3'>Temps   </h5>
                <Input icon='time' defaultValue={new Date().toLocaleTimeString('fr-FR')} type='time' size="small" iconPosition='left'   fluid className='mb-1'  onChange={(e) => setRequestData({...requestData, Livre_At: { ...requestData.Livre_At, Temps: e.target.value }  })}/>
            </div>
            <div className='text-end mt-3'>
                <Button disabled={FindBtnState(reqState).reterderState} fluid className='rounded-pill text-secondary btn-imprimer'  onClick={(e) => UpdateRequestState('LV','Livre_At',JSON.stringify(requestData.Livre_At),true,'livred')}><Icon name='time' /> Retarder  </Button>  
            </div>
        </div>
    </>
}
const ModifierCard = ({requestData, setRequestData, reqState, FindBtnState, UpdateRequestState, OnKeyPressFunc}) =>{
    const RemoveItemFromList = (itemID) =>{
        setRequestData({...requestData, Articles : JSON.stringify(JSON.parse(requestData.Articles).filter(person => person.Name !== itemID)), Modifier_To : JSON.stringify(JSON.parse(requestData.Articles).filter(person => person.Name !== itemID))})
        //setRequestData({...requestData, Modifier_To : requestData.Articles })
    }
    return(<>
        <div className='card card-body border-div mb-4 shadow-sm ltr-force' dir='ltr'>
            <div className='card-body'> 
                Annuler les articles en repture  : 
                <br /> 
                <br /> 
                <ul>
                    {JSON.parse(requestData.Articles).map((data,index) => <li key={index}> {data.Qte} * {data.Name}  <span className='bi bi-trash text-danger' onClick={() => RemoveItemFromList(data.Name)}></span></li> )}
                </ul> 
            </div>
            <div className='text-end mt-3'>
                <Button disabled={FindBtnState(reqState).redirectState} fluid className='rounded-pill text-secondary btn-imprimer'  onClick={(e) => UpdateRequestState('MD','Modifier_To',JSON.stringify(requestData.Modifier_To),true,'edited')}><Icon name='edit outline' /> Redirecter </Button>  
            </div>
        </div> 
    </>)
}
...
function DocteurSpecific() {
    ...
    const FindBtnState = (reqState) =>{
        switch(reqState) {
            ...
            case 'LV': return {seenState: true, acceptState: false, refuseState: false, reterderState: false, redirectState:true , terminerState:true};  
            case 'MD': return {seenState: true, acceptState: false, refuseState: false, reterderState: true, redirectState:false , terminerState:false};  
