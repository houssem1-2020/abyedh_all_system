...
function CentreMdSpecific() {
    ...
    const SalleAttentCard = () =>{
        return(<>
            <div className='card card-body shadow-sm border-div mb-4 text-center '>
                <h5 className={`${isRTL ? 'text-end' :'text-start' }`}  style={{color: GConf.ADIL[tag].themeColor}}> {t(`profilePage.SpesificData.${tag}.watingRoom`)}  </h5>
    ...
    const RoleTimeEstimateCard = () =>{
        return(<>
            <div className='card card-body shadow-sm border-div mb-4 text-center '>
                <h5 className={`${isRTL ? 'text-end' :'text-start' }`} style={{color: GConf.ADIL[tag].themeColor}}> {t(`profilePage.SpesificData.${tag}.estimateTime`)}  </h5>
    ...
    const CertificatCard = () =>{
        return(<>
            <div className='card card-body shadow-sm border-div mb-4 text-center  '>
                ...
                <h1 className='display-1' style={{color: GConf.ADIL[tag].themeColor}}><span className='bi bi-postage bi-lg'></span></h1>
            </div>
        </>)
    }
    const SimpleCard = () =>{
