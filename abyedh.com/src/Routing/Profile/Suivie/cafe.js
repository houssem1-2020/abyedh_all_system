...
function CafeSpecific() {
    ...
    /* ############### UseEffect #################*/
        useEffect(() => {
            axios.post(`${GConf.ApiLink}/suivie/restaurant`, {
                tag : tag,
                PID : PID,
                UID : UID,
                ...
                let rdvContainer = []
                response.data.commandes.map( (getData) => rdvContainer.push([
                ...
                let seanceContainer = []
                response.data.reservation.map( (getData) => seanceContainer.push([
                _(<Button className='rounded-pill text-white' icon style={{backgroundColor:GConf.ADIL[tag].themeColor}} size='mini' onClick={ (e) => OpenModalFunction('rdv',getData)}><Icon  name='arrows alternate' /></Button>),
                new Date(getData.R_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ),
                new Date(getData.R_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ),
                getData.State,
                ...
                response.data.factures.map( (getData) => ordonanceContainer.push([
