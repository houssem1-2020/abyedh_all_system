...
function ArtMuseeSpecific() {
    ...
    const panes = [
        {
          menuItem: { key: 'save', icon: 'calendar alternate', content:  <span className='me-2'>تذاكر</span> , dir:'rtl' },
          render: () => <TableGrid tableData={rendyVousD}  columns={['فتح','عدد','يوم','يوم']} />,
        },
        {
            menuItem: { key: 'edit', icon: 'pin', content:  <span className='me-2'>فواتير</span> , dir:'rtl' },
    ...
    const GetTheLastRDV = (dateList) =>{
        if (dateList.length === 0) {
            return <h1 className='display-1' style={{color: GConf.ADIL[tag].themeColor}}><span className='bi bi-award-fill bi-lg'></span></h1>;
          }
    ...
    const Statistics = () =>{
        return(<>
            <div className='card card-body shadow-sm border-div mb-4 text-center '>
                ...
                <div className='row'>
                    <div className='col-6'><h2 style={{color:GConf.ADIL[tag].themeColor}}>{ordonanceListe.length} </h2>  فاتورة </div>
                    <div className='col-6 border-start'><h2 style={{color:GConf.ADIL[tag].themeColor}}>{rendyVousD.length} </h2> تذكرة</div>
