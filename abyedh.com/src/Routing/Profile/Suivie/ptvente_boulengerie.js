...
function PtvVBoulengerieSpecific() {
    /* ############### Const #################*/
    ...
    const panes = [
        {
          menuItem: { key: 'save', icon: 'calendar alternate', content:  <span className='me-2'>طلب</span> , dir:'rtl' },
          render: () => <TableGrid tableData={rendyVousD}  columns={['فتح','عدد','يوم','يوم']} />,
        },
        {
            menuItem: { key: 'edit', icon: 'pin', content:  <span className='me-2'>فاتورة</span> , dir:'rtl' },
            render: () => <TableGrid tableData={seanceListe}  columns={['فتح','عدد','يوم','معرف']} />,
        },
        {
            menuItem: { key: 'oug', icon: 'list alternate outline', content:  <span className='me-2'>ديون</span> , dir:'rtl' },
    ...
    const GetTheLastRDV = (dateList) =>{
        if (dateList.length === 0) {
            return <h1 className='display-1' style={{color: GConf.ADIL[tag].themeColor}}><span className='bi bi-cart4 bi-lg'></span></h1>;
          }
    ...
    const NextRendyVous = () =>{
        return(<>
            <div className='card card-body shadow-sm border-div mb-4 text-center '>
                <h5 className='text-end text-secondary'> الشراء الشهري </h5> 
        ...
        return(<>
            <div className='card card-body shadow-sm border-div mb-4 text-center '>
                ...
                <div className='row' dir='rtl'>
                    <div className='col-4 border-start'><h2 style={{color:GConf.ADIL[tag].themeColor}}>{ordonanceListe.length} </h2>  طلب  </div>
                    <div className='col-4 border-start'><h2 style={{color:GConf.ADIL[tag].themeColor}}>{rendyVousD.length} </h2> فاتورة </div>
                    <div className='col-4 '><h2 style={{color:GConf.ADIL[tag].themeColor}}>{seanceListe.length} </h2> ديون </div>
                </div>
            </div>
        </>)
    }
...
export default PtvVBoulengerieSpecific;