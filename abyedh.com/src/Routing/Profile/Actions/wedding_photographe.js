...
function WeddingPhotographeSpecific(props) {
    ...
    const serviceOptions = [
        ...
        {key:6, value:'حفلة تخرج ', text:'حفلة تخرج'},
        {key:7, value:'غير محدد ', text:'غير محدد'},
    ]
    const photosOptions = [
        {key:1, value:'صور فقط', text:'صور فقط'},
        {key:2, value:'فيديو فقط', text:'فيديو فقط'},
        {key:3, value:' صور و فيديو', text:' صور و فيديو'},
        {key:7, value:'غير محدد ', text:'غير محدد'},
    ]
   ...
   const SaveCMDFunc = () =>{
    ...
    else if (!commandeData.Res_Genre  ) {toast.error("أدخل  سبب الحجز   !", GConf.TostErrorGonf)}
    else if (!commandeData.Res_Position.Gouv ) {toast.error("أدخل  المكان    !", GConf.TostErrorGonf)}
    else if (!commandeData.Res_Position.Deleg) {toast.error("أدخل  المكان   !", GConf.TostErrorGonf)}
    else if (!commandeData.Shooting_Genre) {toast.error("أدخل  نوع التصوير المطلوب   !", GConf.TostErrorGonf)}
 
    else{

        setLS(true)
        axios.post(`${GConf.ApiLink}/Action/photographe-reserver`, {
            UID : props.UID,
            PID : props.PID ,
            TAG : props.TAG ,
    ...
    return ( <>
        <div className='m-0'>
                <div   dir='rtl' className='card card-body shadow-sm pt-4 border-div'>
                    ...
                    <h5 className='mb-2 ' style={{color: GConf.ADIL[props.TAG].themeColor}}> <span className='bi bi-person-x-fill'></span> نوع التصوير  </h5>
                    <Select fluid placeholder='نوع  التصوير' options={photosOptions} onChange={ (e,data) => setCommandeD({...commandeData, Shooting_Genre: data.value})} />
