...
const EnterCard = ({articleNow, setArticleNow, disabledSaveBtn, commandeData,tag, AddArticleToList}) =>{
    ...
    return (<>
        <div className='card-body mt-2'>
            ...
            <Input icon='pin'  placeholder='إختر مشروب' value={articleNow.Name}  onChange={ (e) => setArticleNow({...articleNow, Name: e.target.value })} size="small" iconPosition='left'   fluid className='mb-1' />
...
function CafeAction(props) {
    ...
    const panes = [
        {
          ...
          render: () => <Tab.Pane className='border-div shadow-sm' attached={false} dir='rtl'> 
                    ...
                    <h5 className='mb-1 ' style={{color: GConf.ADIL[props.TAG].themeColor}}> <span className='bi bi-person-x-fill'></span> أدخل المشروبات المطلوبة  </h5>
    ...
    const SaveCommande = () =>{
        if (commandeData.articles.length == 0) {toast.error("قائمة الأطباق فارغة!", GConf.TostErrorGonf)}
            ...
            axios.post(`${GConf.ApiLink}/Action/cafe-commande`, {
                UID : props.UID,
                PID : props.PID ,
                TAG : props.TAG ,
    ...
    const SaveReservation = () =>{
          if (!reservationD.Wanted_Date) {toast.error("ادخل موعد الحجز  !", GConf.TostErrorGonf)}
            ...
            setLS(true)
            axios.post(`${GConf.ApiLink}/Action/cafe-reservation`, {
                UID : props.UID,
                PID : props.PID ,
                TAG : props.TAG ,
