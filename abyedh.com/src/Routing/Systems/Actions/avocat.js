...
const CommandeCard = ({commandeData, setCommandeD, SaveCMDFunc , disabledSaveBtn, tag, loaderState}) =>{       
    return(<>
        ...
        <div class="input-group mb-1">
            <input type="text" class="form-control" id="souscrire-genre" dir="rtl" required placeholder="نوع القضية" />
        </div>
        <div class="input-group mb-1">
            <textarea type="text" class="form-control"  rows="4" id="souscrire-description" dir="rtl" required placeholder="وصف سـريع ( دون ذكر التفاصيل)"></textarea>
        </div>
...
const RendyVousCard = ({rendyVousD, setRdvData, RendyVousFunc, disabledSaveBtn, tag, loaderState }) =>{
    return(<>
            ...
            <div class="input-group mb-1">
                <textarea type="text" class="form-control"  rows="4" id="rdv-comment" dir="rtl" required placeholder="ملاحضات"></textarea>
            </div>
            <div class="text-right text-danger mb-1 mr-2"><span>تاريخ الموعد</span></div>
...
function AvocatSpecific(props) {
    ...
    const panes = [
        {
          menuItem: { key: 'save', icon: 'checkmark box', content:  <span className='me-2'> تسجيل قضية  </span> , dir:'rtl'},
