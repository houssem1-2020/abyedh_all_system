...
const CommandeCard = ({commandeData, setCommandeD, SaveCMDFunc , disabledSaveBtn, tag, loaderState}) =>{       
    return(<>
        ...
        <div class="input-group mb-1">
        <input type="text" class="form-control" id="reserver-request" dir="rtl" required placeholder="الخدمة المطلوبة مثال : لحية ..." />
        </div>
...
const RendyVousCard = ({rendyVousD, setRdvData, RendyVousFunc, disabledSaveBtn, tag, loaderState }) =>{
    return(<>
            <div class="input-group mb-1">
                <input type="text" class="form-control" id="rdv-name" dir="rtl" required placeholder="الاسم و اللقب" />
            </div>
            <div class="input-group mb-1">
                <input type="text" class="form-control" id="rdv-request" dir="rtl" required placeholder="الخدمة المطلوبة مثال : لحية ..." />
            </div>

            <div class="text-right text-danger mb-1 mr-2"><span>التاريخ و التوقيت</span></div>
            <div class="input-group mb-1 float-right">
            <input type="date" class="form-control" id="rdv-jour" value="<?php echo date('Y-m-d'); ?>" min="2019-01-01" max="2020-12-29" />
            <input type="time" class="form-control" id="rdv-heur" value="<?php echo date('H:i'); ?>" />
            </div>
            <div class="input-group mb-3">
            <textarea type="text" class="form-control"  rows="2" id="rdv-comment" dir="rtl" required placeholder="ملاحضات"></textarea>
            </div>
            <input type="hidden" value="<?php echo $PID; ?>" id="rdv-pid" />
            <div class="text-left">
                <button class="btn btn-success card-1 btn-sm" id="rdv" >تأكيد <span class="fa fa-check-circle"></span></button>
                <button class="btn btn-danger card-1 btn-sm" data-dismiss="modal">إلغاء <span class="fa fa-times-circle"></span></button>
            </div> 
    </>)
}
...
function CoiffureSpecific(props) {
    ...
    const panes = [
        {
          menuItem: { key: 'save', icon: 'checkmark box', content:  <span className='me-2'> طلب خدمة  </span> , dir:'rtl'},
        ...
        {
            menuItem: { key: 'edit', icon: 'calendar alternate', content:  <span className='me-2'> حجز موعد  </span> , dir:'rtl' },
