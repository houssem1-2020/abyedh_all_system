...
function DocteurActions(props) {
   ...
   /* ############### Functions #################*/
    ...
    return ( <>
        <div className='m-0'>
                <p>تم تأجيل الموعد ليوم {JSON.parse(props.requestData.Retarder_Vers).Date} مع الساعة {JSON.parse(props.requestData.Retarder_Vers).Temps} </p>
                    
                <div className='row'>
                    <div className='col-12'>
                        <Button className='rounded-pill text-success'  size='tiny' onClick={saveFunction} disabled={disabledSaveBtn}  icon   > <Icon name='check' />    قبول </Button>
                        <Button className='rounded-pill text-danger' size='tiny' onClick={saveFunction} disabled={disabledSaveBtn}  icon   > <Icon name='times' /> رفض </Button>
                    </div>
                </div>
