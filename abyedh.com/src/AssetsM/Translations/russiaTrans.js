const RussiaTrans = {
    translation : {
        loginPage: {
            inputIdentifiantText: "Номер телефона",
            inputPasswordText: "Пароль",
            btnLogInText: "Войти",
            inscrireLinkText: "Зарегистрироваться",
            inscrireisFreeText: "Бесплатная регистрация",
            pleaseInscrireOneText: "Abyad - это тунисская платформа, созданная в 2018 году с целью создания полноценной цифровой среды.",
            pleaseInscrireTwoText: "Регистрация на Abyad бесплатна и всегда будет таковой. Учетная запись может помочь вам найти то, что вам нужно для выполнения ваших повседневных задач.",
            toastText: {
                enterLog: "Введите свой номер телефона",
                enterPWD: "Введите пароль",
                saved: "Успешно сохранено!",
                notSaved: "Эта учетная запись не существует"
            }
        },
        signUpPage: {
            mainLoderText: "Регистрация новой учетной записи",
            infoGeneraleTitle: "Общая информация",
            locationGeoTitle: "Географическое местоположение",
            selectPhotoTitle: "Выбрать фотографию профиля",
            inputPassswordTitle: "Пароль",
            infoGenerale: {
                nomEtPrenomText: "Имя и фамилия",
                naissanceText: "Дата рождения",
                selectGenreText: "Пол",
                maleGenreText: "Мужской",
                femaleGenreText: "Женский",
                phoneText: "Номер телефона",
                phoneTextPlaceholder: "Обязательный номер телефона"
            },
            locationGeo: {
                positionGeoText: "Выберите область",
                selectGouvText: "Выберите область",
                selectDelegText: "Выберите район"
            },
            selectPhoto: {
                selectPhotoText: ""
            },
            inputPasssword: {
                inputPassswordText: "Пароль",
                inputIdentifText: "Подтвердите пароль",
                inputPWDText: "Пароль",
                repeatPWDText: "Повторите пароль",
                repeatPWDPlaceholderText: ""
            },
            nextBtnText: "Далее",
            finishBtnText: "Завершить",
            toastText: {
                failOne: "",
                failTwo: ""
            }
        },
        mainPage: {
            mainTitle: 'Abyedh ',
            mainsubads: 'Множество приложений в одном приложении',
            mainlogInText: 'Вход',
            mainsearchInput: '... Поиск',
            qrCodeScan: '... Сканировать',
            dropDownProfile: {
                profile: 'Профиль',
                systemLogIn: 'Вход в систему',
                systemSuivie: 'Отслеживание',
                parametres: 'Изменить язык',
                logOut: 'Выйти',
            },
            mainadsCard: 'Abyedh помогает в нескольких областях поиска:',

            mainTagsName: {
                sante: 'Здоровье',
                education: 'Образование',
                transportation: 'Транспорт',
                nutrition: 'Питание',
                houseCar: 'Дом и Автомобиль',
                construction: 'Строительство',
                life: 'Жизнь и Красота',
                culture: 'Культура и Молодежь',
                politique: 'Политика и Права',
                finance: 'Финансы',
                agricole: 'Сельское хозяйство',
                tourism: 'Туризм',
                generale: 'Общие услуги',
                commerceSmall: 'Розничная торговля',
                trasnportationSmall: 'Транспорт',
                lifeSmall: 'Жизнь и Красота',
                restaurationSmall: 'Гостеприимство и Путешествия',
                constructionSmall: 'Строительство',
                artisanatSmall: 'Ремесленные услуги',
                financeSmall: 'Финансы и Бизнес',
             },
            itemsNames: {
                docteur: 'Доктор',
                infirmier: 'Медсестра',
                pharmacie: 'Аптека',
                labo: 'Лаборатория',
                clinique: 'Клиника',
                centreMD: 'Медицинский Центр',
                admin_s_hospital: 'Администрация Больницы',
                admin_s_csb: 'Администрация ЦСБ',
                garderie: 'Детский Сад',
                ecole: 'Школа',
                lycee: 'Лицей',
                universite: 'Университет',
                formation: 'Обучение',
                librairie: 'Книжный Магазин',
                admin_e_centre: 'Администрация Центра',
                admin_e_biblio: 'Администрация Библиотеки',
                admin_e_ecole: 'Администрация Школы',
                admin_e_lycee: 'Администрация Лицея',
                admin_e_universite: 'Администрация Университета',
                admin_e_ss: 'Администрация Услуг',
                transporteur: 'Транспортная Компания',
                autoecole: 'Автошкола',
                location: 'Прокат Автомобилей',
                qiosque: 'Киоск',
                parking: 'Парковка',
                mecanicien: 'Механик',
                Taxi: 'Такси',
                Louage: 'Коллективное Такси',
                magazin: 'Магазин',
                boulengerie: 'Пекарня',
                fruiterie: 'Фруктовый Магазин',
                patesserie: 'Кондитерская',
                epicerie: 'Продуктовый Магазин',
                electromenager: 'Магазин Электроприборов',
                meublerie: 'Магазин Мебели',
                boutique: 'Бутик',
                bijouterie: 'Ювелирный Магазин',
                quincaillerie: 'Хозяйственный Магазин',
                depot: 'Склад',
                boucherie: 'Мясной Магазин',
                cafe: 'Кафе',
                restaurant: 'Ресторан',
                hotels: 'Отель',
                vg_agence: 'Туристическое Агентство',
                coiffure: 'Парикмахерская',
                salle_marriage: 'Свадебный Зал',
                fourniture_marriage: 'Свадебные Аксессуары',
                orchestre: 'Оркестр',
                chef: 'Шеф-повар',
                photographe: 'Фотограф',
                forgeron: 'Кузнец',
                menuisier: 'Плотник',
                peinture: 'Живопись',
                electricien: 'Электрик',
                plombier: 'Сантехник',
                cristalerie: 'Магазин Стекла',
                marbrerie: 'Мраморная Мастерская',
                courtier: 'Брокер',
                contracteur: 'Подрядчик',
                architecture: 'Архитектор',
                gym: 'Фитнес-зал',
                pyscine: 'Бассейн',
                stade: 'Стадион',
                cinema: 'Кинотеатр',
                theatre: 'Театр',
                musee: 'Музей',
                admin_c_mj: 'Центр Молодежи',
                admin_c_mc: 'Культурный Центр',
                admin_c_me: 'Детский Клуб',
                comptable: 'Бухгалтер',
                bank: 'Банк',
                admin_f_poste: 'Почтовое Отделение',
                admin_f_rf: 'Финансовый Офис',
                avocat: 'Адвокат',
                admin_a_mu: 'Муниципалитет',
                admin_a_police: 'Полицейский Участок',
                admin_a_ar: 'Администрация/Представительство',
                admin_a_court:
             
              'Суд',
                admin_a_mosq: 'Мечеть',
                ambassade: 'Посольство',
                fourragerie: 'Зоомагазин',
                veterinaire: 'Ветеринар',
                agritools: 'Сельскохозяйственные Инструменты',
             },
            googlePlayAds: 'Скачайте приложение Abiad для лучшего пользовательского опыта',
            googlePlayAdsAppVolume: 'Размер: 2,85 МБ | Последнее обновление 25/11/2023',
            toolsTitls: 'Инструменты Abiad',
            toolsSSText: 'Откройте серию инструментов Abiad для вашей повседневной жизни',
            toolsBSText1: 'Abiad предоставляет ряд инструментов, направленных на создание информационной среды, помогающей всем пользователям в их повседневных делах',
            toolsBSText2: 'Откройте инструменты Abiad здесь',
            footerVision: 'Видение Abiad',
            footerHow: 'Изменить страну',
            footerWho: 'Кто мы?',
            googlePlayAds1: '',
            googlePlayAds2: '',
            googlePlayAds3: '',
            googlePlayAds4: '',
            toastText:{
                failOne:'',
                failTwo:'',
            },
        },
        landingPage: {
            selectText:{
                docteur: "Выберите необходимую специальность",
                infirmier: "Медсестра",
                pharmacie: "Выберите тип аптеки",
                labo: "Выберите тип услуги",
                clinique: "Выберите тип услуги",
                centreMD: "Выберите тип услуги",
                admin_s_hospital: "Выберите тип услуги",
                admin_s_csb: "Выберите тип услуги",
                garderie: "Выберите тип услуги",
                ecole: "Выберите тип услуги",
                lycee: "Выберите тип услуги",
                universite: "Выберите тип услуги",
                formation: "Выберите тип услуги",
                librairie: "Выберите тип услуги",
                admin_e_centre: "Выберите тип услуги",
                admin_e_biblio: "Выберите тип услуги",
                admin_e_ecole: "Выберите тип услуги",
                admin_e_lycee: "Выберите тип услуги",
                admin_e_universite: "Выберите тип услуги",
                admin_e_ss: "Выберите тип услуги",
                transporteur: "Выберите тип услуги",
                autoecole: "Выберите тип услуги",
                location: "Выберите тип услуги",
                qiosque: "Выберите тип услуги",
                parking: "Выберите тип услуги",
                mecanicien: "Выберите тип услуги",
                Taxi: "Выберите тип услуги",
                Louage: "Выберите тип услуги",
                magazin: "Выберите тип услуги",
                boulengerie: "Выберите тип услуги",
                fruiterie: "Выберите тип услуги",
                patesserie: "Выберите тип услуги",
                epicerie: "Выберите тип услуги",
                electromenager: "Выберите тип услуги",
                meublerie: "Выберите тип услуги",
                boutique: "Выберите тип услуги",
                bijouterie: "Выберите тип услуги",
                quincaillerie: "Выберите тип услуги",
                depot: "Выберите тип услуги",
                boucherie: "Выберите тип услуги",
                cafe: "Выберите тип услуги",
                restaurant: "Выберите тип услуги",
                hotels: "Выберите тип услуги",
                vg_agence: "Выберите тип услуги",
                coiffure: "Выберите тип услуги",
                salle_marriage: "Выберите тип услуги",
                fourniture_marriage: "Выберите тип услуги",
                orchestre: "Выберите тип услуги",
                chef: "Выберите тип услуги",
                photographe: "Выберите тип услуги",
                forgeron: "Выберите тип услуги",
                menuisier: "Выберите тип услуги",
                peinture: "Выберите тип услуги",
                electricien: "Выберите тип услуги",
                plombier: "Выберите тип услуги",
                cristalerie: "Выберите тип услуги",
                marbrerie: "Выберите тип услуги",
                courtier: "Выберите тип услуги",
                contracteur: "Выберите тип услуги",
                architecture: "Выберите тип услуги",
                gym: "Выберите тип услуги",
                pyscine: "Выберите тип услуги",
                stade: "Выберите тип услуги",
                cinema: "Выберите тип услуги",
                theatre: "Выберите тип услуги",
                musee: "Выберите тип услуги",
                admin_c_mj: "Выберите тип услуги",
                admin_c_mc: "Выберите тип услуги",
                admin_c_me: "Выберите тип услуги",
                comptable: "Выберите тип услуги",
                bank: "Выберите тип услуги",
                admin_f_poste: "Выберите тип услуги",
                admin_f_rf: "Выберите тип услуги",
                avocat: "Выберите тип услуги",
                admin_a_mu: "Выберите тип услуги",
                admin_a_police: "Выберите тип услуги",
                admin_a_ar: "Выберите тип услуги",
                admin_a_court: "Выберите тип услуги",
                admin_a_mosq: "Выберите тип услуги",
                ambassade: "Выберите тип услуги",
                fourragerie: "Выберите тип услуги",
                veterinaire: "Выберите тип услуги",
                agritools: "Выберите тип услуги"
                
            },
            itemsToSelect : {
                docteur : {
                    one:'',
                    two:'',
                    three:'',
                },
                infirmier :{
                    one:'',
                    two:'',
                    three:'',
                },
                pharmacie :{
                    one:'',
                    two:'',
                    three:'',
                },
                labo :{
                    one:'',
                    two:'',
                    three:'',
                },
                clinique :{
                    one:'',
                    two:'',
                    three:'',
                },
                centreMD :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_s_hospital :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_s_csb :{
                    one:'',
                    two:'',
                    three:'',
                },
                garderie :{
                    one:'',
                    two:'',
                    three:'',
                },
                ecole :{
                    one:'',
                    two:'',
                    three:'',
                },
                lycee :{
                    one:'',
                    two:'',
                    three:'',
                },
                universite :{
                    one:'',
                    two:'',
                    three:'',
                },
                formation :{
                    one:'',
                    two:'',
                    three:'',
                },
                librairie :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_e_centre :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_e_biblio :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_e_ecole :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_e_lycee :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_e_universite :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_e_ss :{
                    one:'',
                    two:'',
                    three:'',
                },
                transporteur :{
                    one:'',
                    two:'',
                    three:'',
                },
                autoecole :{
                    one:'',
                    two:'',
                    three:'',
                },
                location :{
                    one:'',
                    two:'',
                    three:'',
                },
                qiosque :{
                    one:'',
                    two:'',
                    three:'',
                },
                parking :{
                    one:'',
                    two:'',
                    three:'',
                },
                mecanicien :{
                    one:'',
                    two:'',
                    three:'',
                },
                Taxi :{
                    one:'',
                    two:'',
                    three:'',
                },
                Louage :{
                    one:'',
                    two:'',
                    three:'',
                },
                magazin :{
                    one:'',
                    two:'',
                    three:'',
                },
                boulengerie :{
                    one:'',
                    two:'',
                    three:'',
                },
                fruiterie :{
                    one:'',
                    two:'',
                    three:'',
                },
                patesserie :{
                    one:'',
                    two:'',
                    three:'',
                },
                epicerie :{
                    one:'',
                    two:'',
                    three:'',
                },
                electromenager :{
                    one:'',
                    two:'',
                    three:'',
                },
                meublerie :{
                    one:'',
                    two:'',
                    three:'',
                },
                boutique :{
                    one:'',
                    two:'',
                    three:'',
                },
                bijouterie :{
                    one:'',
                    two:'',
                    three:'',
                },
                quincaillerie :{
                    one:'',
                    two:'',
                    three:'',
                },
                depot :{
                    one:'',
                    two:'',
                    three:'',
                },
                boucherie :{
                    one:'',
                    two:'',
                    three:'',
                },
                cafe :{
                    one:'',
                    two:'',
                    three:'',
                },
                restaurant :{
                    one:'',
                    two:'',
                    three:'',
                },
                hotels :{
                    one:'',
                    two:'',
                    three:'',
                },
                vg_agence :{
                    one:'',
                    two:'',
                    three:'',
                },
                coiffure :{
                    one:'',
                    two:'',
                    three:'',
                },
                salle_marriage :{
                    one:'',
                    two:'',
                    three:'',
                },
                fourniture_marriage :{
                    one:'',
                    two:'',
                    three:'',
                },
                orchestre :{
                    one:'',
                    two:'',
                    three:'',
                },
                chef :{
                    one:'',
                    two:'',
                    three:'',
                },
                photographe :{
                    one:'',
                    two:'',
                    three:'',
                },
                forgeron :{
                    one:'',
                    two:'',
                    three:'',
                },
                menuisier :{
                    one:'',
                    two:'',
                    three:'',
                },
                peinture :{
                    one:'',
                    two:'',
                    three:'',
                },
                electricien :{
                    one:'',
                    two:'',
                    three:'',
                },
                plombier :{
                    one:'',
                    two:'',
                    three:'',
                },
                cristalerie :{
                    one:'',
                    two:'',
                    three:'',
                },
                marbrerie :{
                    one:'',
                    two:'',
                    three:'',
                },
                courtier :{
                    one:'',
                    two:'',
                    three:'',
                },
                contracteur :{
                    one:'',
                    two:'',
                    three:'',
                },
                architecture :{
                    one:'',
                    two:'',
                    three:'',
                },
                gym :{
                    one:'',
                    two:'',
                    three:'',
                },
                pyscine :{
                    one:'',
                    two:'',
                    three:'',
                },
                stade :{
                    one:'',
                    two:'',
                    three:'',
                },
                cinema :{
                    one:'',
                    two:'',
                    three:'',
                },
                theatre :{
                    one:'',
                    two:'',
                    three:'',
                },
                musee :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_c_mj :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_c_mc :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_c_me :{
                    one:'',
                    two:'',
                    three:'',
                },
                comptable :{
                    one:'',
                    two:'',
                    three:'',
                },
                bank :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_f_poste :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_f_rf :{
                    one:'',
                    two:'',
                    three:'',
                },
                avocat :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_a_mu :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_a_police :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_a_ar :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_a_court :{
                    one:'',
                    two:'',
                    three:'',
                },
                admin_a_mosq :{
                    one:'',
                    two:'',
                    three:'',
                },
                ambassade :{
                    one:'',
                    two:'',
                    three:'',
                },
                fourragerie :{
                    one:'',
                    two:'',
                    three:'',
                },
                veterinaire :{
                    one:'',
                    two:'',
                    three:'',
                },
                agritools :{
                    one:'',
                    two:'',
                    three:'',
                },
                
            },
            movingAdsText : {
                docteur : {
                    one:'',
                    two:'',
                },
                infirmier :{
                    one:'',
                    two:'',
                },
                pharmacie :{
                    one:'',
                    two:'',
                },
                labo :{
                    one:'',
                    two:'',
                },
                clinique :{
                    one:'',
                    two:'',
                },
                centreMD :{
                    one:'',
                    two:'',
                },
                admin_s_hospital :{
                    one:'',
                    two:'',
                },
                admin_s_csb :{
                    one:'',
                    two:'',
                },
                garderie :{
                    one:'',
                    two:'',
                },
                ecole :{
                    one:'',
                    two:'',
                },
                lycee :{
                    one:'',
                    two:'',
                },
                universite :{
                    one:'',
                    two:'',
                },
                formation :{
                    one:'',
                    two:'',
                },
                librairie :{
                    one:'',
                    two:'',
                },
                admin_e_centre :{
                    one:'',
                    two:'',
                },
                admin_e_biblio :{
                    one:'',
                    two:'',
                },
                admin_e_ecole :{
                    one:'',
                    two:'',
                },
                admin_e_lycee :{
                    one:'',
                    two:'',
                },
                admin_e_universite :{
                    one:'',
                    two:'',
                },
                admin_e_ss :{
                    one:'',
                    two:'',
                },
                transporteur :{
                    one:'',
                    two:'',
                },
                autoecole :{
                    one:'',
                    two:'',
                },
                location :{
                    one:'',
                    two:'',
                },
                qiosque :{
                    one:'',
                    two:'',
                },
                parking :{
                    one:'',
                    two:'',
                },
                mecanicien :{
                    one:'',
                    two:'',
                },
                Taxi :{
                    one:'',
                    two:'',
                },
                Louage :{
                    one:'',
                    two:'',
                },
                magazin :{
                    one:'',
                    two:'',
                },
                boulengerie :{
                    one:'',
                    two:'',
                },
                fruiterie :{
                    one:'',
                    two:'',
                },
                patesserie :{
                    one:'',
                    two:'',
                },
                epicerie :{
                    one:'',
                    two:'',
                },
                electromenager :{
                    one:'',
                    two:'',
                },
                meublerie :{
                    one:'',
                    two:'',
                },
                boutique :{
                    one:'',
                    two:'',
                },
                bijouterie :{
                    one:'',
                    two:'',
                },
                quincaillerie :{
                    one:'',
                    two:'',
                },
                depot :{
                    one:'',
                    two:'',
                },
                boucherie :{
                    one:'',
                    two:'',
                },
                cafe :{
                    one:'',
                    two:'',
                },
                restaurant :{
                    one:'',
                    two:'',
                },
                hotels :{
                    one:'',
                    two:'',
                },
                vg_agence :{
                    one:'',
                    two:'',
                },
                coiffure :{
                    one:'',
                    two:'',
                },
                salle_marriage :{
                    one:'',
                    two:'',
                },
                fourniture_marriage :{
                    one:'',
                    two:'',
                },
                orchestre :{
                    one:'',
                    two:'',
                },
                chef :{
                    one:'',
                    two:'',
                },
                photographe :{
                    one:'',
                    two:'',
                },
                forgeron :{
                    one:'',
                    two:'',
                },
                menuisier :{
                    one:'',
                    two:'',
                },
                peinture :{
                    one:'',
                    two:'',
                },
                electricien :{
                    one:'',
                    two:'',
                },
                plombier :{
                    one:'',
                    two:'',
                },
                cristalerie :{
                    one:'',
                    two:'',
                },
                marbrerie :{
                    one:'',
                    two:'',
                },
                courtier :{
                    one:'',
                    two:'',
                },
                contracteur :{
                    one:'',
                    two:'',
                },
                architecture :{
                    one:'',
                    two:'',
                },
                gym :{
                    one:'',
                    two:'',
                },
                pyscine :{
                    one:'',
                    two:'',
                },
                stade :{
                    one:'',
                    two:'',
                },
                cinema :{
                    one:'',
                    two:'',
                },
                theatre :{
                    one:'',
                    two:'',
                },
                musee :{
                    one:'',
                    two:'',
                },
                admin_c_mj :{
                    one:'',
                    two:'',
                },
                admin_c_mc :{
                    one:'',
                    two:'',
                },
                admin_c_me :{
                    one:'',
                    two:'',
                },
                comptable :{
                    one:'',
                    two:'',
                },
                bank :{
                    one:'',
                    two:'',
                },
                admin_f_poste :{
                    one:'',
                    two:'',
                },
                admin_f_rf :{
                    one:'',
                    two:'',
                },
                avocat :{
                    one:'',
                    two:'',
                },
                admin_a_mu :{
                    one:'',
                    two:'',
                },
                admin_a_police :{
                    one:'',
                    two:'',
                },
                admin_a_ar :{
                    one:'',
                    two:'',
                },
                admin_a_court :{
                    one:'',
                    two:'',
                },
                admin_a_mosq :{
                    one:'',
                    two:'',
                },
                ambassade :{
                    one:'',
                    two:'',
                },
                fourragerie :{
                    one:'',
                    two:'',
                },
                veterinaire :{
                    one:'',
                    two:'',
                },
                agritools :{
                    one:'',
                    two:'',
                },
                
            },
            fastSearchText: "Займ",
            pleaseSelectGouvText: "Или выполните поиск внутри",
            pleaseSelectGouvTextSecondary: "Выберите область поиска",
            selectGouvText: "Выберите регион",
            selectDelegText: "Выберите отделение",
            rechercheBtnText: "Поиск",

            titleSystemAds1: "Вы",
            titleSystemAds2: "и хотите зарегистрироваться на платформе?",
            textSystemAds1: "Откройте мини-версию",
            textSystemAds2: "которая поможет вам представить себя и получать запросы от ваших клиентов.",
            systemNames: {
                docteur: 'Система управления клиникой',
                pharmacie: 'Система управления аптекой',
                clinique: 'Система управления частными клиниками',
                labo: 'Система управления медицинскими лабораториями',
                infirmier: 'Система управления медсестринскими кабинетами',
                centreMD: 'Система управления медицинскими центрами',
                admin_s_hospital: 'Система управления больницами',
                admin_s_csb: 'Система управления центрами здоровья',
                garderie: 'Система управления детскими садами',
                ecole: 'Система управления частными школами',
                lycee: 'Система управления частными лицеями',
                universite: 'Система управления частными университетами',
                formation: 'Система управления учебными центрами',
                impremerie: 'Система управления типографиями',
                librairie: 'Система управления книжными магазинами',
                admin_e_centre: 'Система управления учебными центрами',
                admin_e_biblio: 'Система управления публичными библиотеками',
                admin_e_ecole: 'Система управления публичными школами',
                admin_e_lycee: 'Система управления публичными лицеями',
                admin_e_universite: 'Система управления публичными университетами',
                admin_e_ss: 'Система управления государственными службами',
                transporteur: 'Система управления транспортными агентствами',
                autoecole: 'Система управления автошколами',
                cafe: 'Система управления кафе',
                restaurant: 'Система управления ресторанами',
                magazin: 'Система управления магазинами',
                boulengerie: 'Система управления пекарнями',
                boucherie: 'Система управления мясными магазинами',
                fruiterie: 'Система управления фруктовыми магазинами',
                patesserie: 'Система управления кондитерскими',
                epicerie: 'Система управления продуктовыми магазинами',
                electromenager: 'Система управления магазинами бытовой техники',
                meublerie: 'Система управления магазинами мебели',
                location: 'Система управления агентствами по прокату автомобилей',
                parking: 'Система управления парковками',
                qiosque: 'Система управления киосками',
                mecanicien: 'Система управления автосервисами',
                courtier: 'Система управления агентствами недвижимости',
                contracteur: 'Система управления строительными компаниями',
                architecture: 'Система управления архитектурными бюро',
                quincaillerie: 'Система управления хозяйственными магазинами',
                forgeron: 'Система управления кузнечными мастерскими',
                menuisier: 'Система управления столярными мастерскими',
                peinture: 'Система управления магазинами красок',
                electricien: 'Система управления электриками',
                plombier: 'Система управления сантехниками',
                cristalerie: 'Система управления стекольными мастерскими',
                marbrerie: 'Система управления мраморными мастерскими',
                coiffure: 'Система управления парикмахерскими',
                boutique: 'Система управления бутиками',
                salle_marriage: 'Система управления залами для свадеб',
                orchestre: 'Система управления оркестрами',
                chef: 'Система управления кулинарами',
                photographe: 'Система управления фотостудиями',
                fourniture_marriage: 'Система управления арендой свадебного оборудования',
                bijouterie: 'Система управления ювелирными магазинами',
                gym: 'Система управления спортивными залами',
                pyscine: 'Система управления бассейнами',
                stade: 'Система управления стадионами',
                cinema: 'Система управления кинотеатрами',
                theatre: 'Система управления театрами',
                musee: 'Система управления музеями',
                admin_c_mj: 'Система управления молодежными домами',
                admin_c_mc: 'Система управления культурными центрами',
                admin_c_me: 'Система управления клубами для детей',
                avocat: 'Система управления адвокатскими кабинетами',
                depot: 'Система управления оптовыми складами',
                comptable: 'Система управления бухгалтерскими компаниями',
                socite: 'Система управления компаниями',
                bank: 'Система управления банками',
                admin_f_poste: 'Система управления почтовыми отделениями',
                admin_f_rf: 'Система управления казначействами',
                veterinaire: 'Система управления ветеринарными клиниками',
                fourragerie: 'Система управления магазинами кормов для животных',
                agritools: 'Система управления продажей сельскохозяйственного оборудования',
                hotels: 'Система управления гостиницами',
                vg_agence: 'Система управления туристическими агентствами',
                admin_a_mu: 'Система управления клиниками',
                admin_a_police: 'Система управления клиниками',
                admin_a_ar: 'Система управления клиниками',
                admin_a_court: 'Система управления судами',
                admin_a_mosq: 'Система управления клиниками'
              },
              systemOwnersNames: {
                docteur: 'Доктор',
                pharmacie: 'Аптекарь',
                clinique: 'Частная клиника',
                labo: 'Лаборатория',
                infirmier: 'Медсестра',
                centreMD: 'Медицинский центр',
                admin_s_hospital: 'Доктор',
                admin_s_csb: 'Доктор',
                garderie: 'Детский сад',
                ecole: 'Частная школа',
                lycee: 'Частный лицей',
                universite: 'Частный университет',
                formation: 'Учебный центр',
                impremerie: 'Типография',
                librairie: 'Книжный магазин',
                admin_e_centre: 'Доктор',
                admin_e_biblio: 'Доктор',
                admin_e_ecole: 'Доктор',
                admin_e_lycee: 'Доктор',
                admin_e_universite: 'Доктор',
                admin_e_ss: 'Доктор',
                transporteur: 'Перевозчик',
                autoecole: 'Автошкола',
                cafe: 'Кафе',
                restaurant: 'Ресторан',
                magazin: 'Магазин',
                boulengerie: 'Пекарня',
                boucherie: 'Мясной магазин',
                fruiterie: 'Фруктовый магазин',
                patesserie: 'Кондитерская',
                epicerie: 'Продуктовый магазин',
                electromenager: 'Магазин бытовой техники',
                meublerie: 'Мебельный магазин',
                location: 'Агентство по прокату',
                parking: 'Парковка',
                qiosque: 'Киоск',
                mecanicien: 'Автосервис',
                courtier: 'Агент по недвижимости',
                contracteur: 'Строительная компания',
                architecture: 'Архитектор',
                quincaillerie: 'Хозяйственный магазин',
                forgeron: 'Кузнец',
                menuisier: 'Плотник',
                peinture: 'Магазин красок',
                electricien: 'Электрик',
                plombier: 'Сантехник',
                cristalerie: 'Стекольная мастерская',
                marbrerie: 'Мраморная мастерская',
                coiffure: 'Парикмахерская',
                boutique: 'Бутик',
                salle_marriage: 'Зал для свадеб',
                orchestre: 'Оркестр',
                chef: 'Повар',
                photographe: 'Фотостудия',
                fourniture_marriage: 'Аренда свадебного оборудования',
                bijouterie: 'Ювелирный магазин',
                gym: 'Спортивный зал',
                pyscine: 'Бассейн',
                stade: 'Стадион',
                cinema: 'Кинотеатр',
                theatre: 'Театр',
                musee: 'Музей',
                admin_c_mj: 'Доктор',
                admin_c_mc: 'Доктор',
                admin_c_me: 'Доктор',
                avocat: 'Адвокат',
                depot: 'Оптовый склад',
                comptable: 'Бухгалтер',
                socite: 'Компания',
                bank: 'Банкир',
                admin_f_poste: 'Доктор',
                admin_f_rf: 'Доктор',
                veterinaire: 'Ветеринар',
                fourragerie: 'Продажа кормов',
                agritools: 'Сельскохозяйственное оборудование',
                hotels: 'Гостиница',
                vg_agence: 'Туристическое агентство',
                admin_a_mu: 'Доктор',
                admin_a_police: 'Доктор',
                admin_a_ar: 'Доктор',
                admin_a_court: 'Доктор',
                admin_a_mosq: 'Доктор',
                agriToolsOld: 'Доктор'
              },
            goToSystemBtnText: "Получить доступ",
            SubscribeBtnText: "Подписаться",
            suggestionAndFavText: "Избранные и предложения",
            bottomAbyedhText: "abyedh.com: Russia",
            toastText:{
                failOne:'',
                failTwo:'',
            },
        },
        resultPage : {
            actionTextName : {
                docteur : 'طبيب',
                infirmier :'مَمَرٍّضْ',
                pharmacie :'صَيْدَلِية',
                labo :'مَخْبَرْ تَحَالِيلْ',
                clinique :'مِصَحَّة',
                centreMD :'مَرْكَزْ عِلاَجْ',
                admin_s_hospital :'مٌسْتَشْفَي',
                admin_s_csb :'مٌسْتَشْفَي',
                garderie :'مَرْكِزْ صِحَّة',
                ecole :'مَرْكِزْ صِحَّة',
                lycee :'مَرْكِزْ صِحَّة',
                universite :'مَرْكِزْ صِحَّة',
                formation :'مَرْكِزْ صِحَّة',
                librairie :'مَرْكِزْ صِحَّة',
                admin_e_centre :'مَرْكِزْ صِحَّة',
                admin_e_biblio :'مَرْكِزْ صِحَّة',
                admin_e_ecole :'مَرْكِزْ صِحَّة',
                admin_e_lycee :'مَرْكِزْ صِحَّة',
                admin_e_universite :'مَرْكِزْ صِحَّة',
                admin_e_ss :'مَرْكِزْ صِحَّة',
                transporteur :'مَرْكِزْ صِحَّة',
                autoecole :'مَرْكِزْ صِحَّة',
                location :'مَرْكِزْ صِحَّة',
                qiosque :'مَرْكِزْ صِحَّة',
                parking :'مَرْكِزْ صِحَّة',
                mecanicien :'مَرْكِزْ صِحَّة',
                Taxi :'مَرْكِزْ صِحَّة',
                Louage :'مَرْكِزْ صِحَّة',
                magazin :'مَرْكِزْ صِحَّة',
                boulengerie :'مَرْكِزْ صِحَّة',
                fruiterie :'مَرْكِزْ صِحَّة',
                patesserie :'مَرْكِزْ صِحَّة',
                epicerie :'مَرْكِزْ صِحَّة',
                electromenager :'مَرْكِزْ صِحَّة',
                meublerie :'مَرْكِزْ صِحَّة',
                boutique :'مَرْكِزْ صِحَّة',
                bijouterie :'مَرْكِزْ صِحَّة',
                quincaillerie :'مَرْكِزْ صِحَّة',
                depot :'مَرْكِزْ صِحَّة',
                boucherie :'مَرْكِزْ صِحَّة',
                cafe :'مَرْكِزْ صِحَّة',
                restaurant :'مَرْكِزْ صِحَّة',
                hotels :'مَرْكِزْ صِحَّة',
                vg_agence :'مَرْكِزْ صِحَّة',
                coiffure :'مَرْكِزْ صِحَّة',
                salle_marriage :'مَرْكِزْ صِحَّة',
                fourniture_marriage :'مَرْكِزْ صِحَّة',
                orchestre :'مَرْكِزْ صِحَّة',
                chef :'مَرْكِزْ صِحَّة',
                photographe :'مَرْكِزْ صِحَّة',
                forgeron :'مَرْكِزْ صِحَّة',
                menuisier :'مَرْكِزْ صِحَّة',
                peinture :'مَرْكِزْ صِحَّة',
                electricien :'مَرْكِزْ صِحَّة',
                plombier :'مَرْكِزْ صِحَّة',
                cristalerie :'مَرْكِزْ صِحَّة',
                marbrerie :'مَرْكِزْ صِحَّة',
                courtier :'مَرْكِزْ صِحَّة',
                contracteur :'مَرْكِزْ صِحَّة',
                architecture :'مَرْكِزْ صِحَّة',
                gym :'مَرْكِزْ صِحَّة',
                pyscine :'مَرْكِزْ صِحَّة',
                stade :'مَرْكِزْ صِحَّة',
                cinema :'مَرْكِزْ صِحَّة',
                theatre :'مَرْكِزْ صِحَّة',
                musee :'مَرْكِزْ صِحَّة',
                admin_c_mj :'مَرْكِزْ صِحَّة',
                admin_c_mc :'مَرْكِزْ صِحَّة',
                admin_c_me :'مَرْكِزْ صِحَّة',
                comptable :'مَرْكِزْ صِحَّة',
                bank :'مَرْكِزْ صِحَّة',
                admin_f_poste :'مَرْكِزْ صِحَّة',
                admin_f_rf :'مَرْكِزْ صِحَّة',
                avocat :'مَرْكِزْ صِحَّة',
                admin_a_mu :'مَرْكِزْ صِحَّة',
                admin_a_police :'مَرْكِزْ صِحَّة',
                admin_a_ar :'مَرْكِزْ صِحَّة',
                admin_a_court :'مَرْكِزْ صِحَّة',
                admin_a_mosq :'مَرْكِزْ صِحَّة',
                ambassade :'مَرْكِزْ صِحَّة',
                fourragerie :'مَرْكِزْ صِحَّة',
                veterinaire :'مَرْكِزْ صِحَّة',
                agritools :'مَرْكِزْ صِحَّة',
                
            },
            pasDeResultat: {
                desoleText: "Извините, в настоящее время нет результатов по следующим причинам:",
                reasonOne: "Действительно нет результатов",
                reasonTwo: "Мы не смогли найти результаты",
                searchNext: "Вы можете вернуться назад и искать в других местах рядом с вами, убедитесь, что вы найдете то, что ищете"
            },
            bottomAbyedhText: "abyedh.com",
            toastText:{
                failOne:'',
                failTwo:'',
            },
        },
        profilePage: {
            generalData: {
                generalDataText: "Общая информация",
                horaireText: "Часы работы",
                weekDayes: {
                    lundi: "Понедельник",
                    mardi: "Вторник",
                    mercredi: "Среда",
                    jeudi: "Четверг",
                    vendredi: "Пятница",
                    samedi: "Суббота",
                    dimanche: "Воскресенье",
                },
                photoesText: "Фотографии",
                mapText: "Географическое местоположение",
                avisText: "Отзывы",
                commentText: "Комментарии",
                addCommentText: "Добавить",
                voirCommentText: "Просмотр",
                inputCommentPlaceholderText: "Ваш комментарий здесь",
                saveCommentBtnText: "Сохранить комментарий",
            },            
            SpesificData : {
                docteur : {
                    one:'',
                    two:'',
                },
                infirmier :{
                    one:'',
                    two:'',
                },
                pharmacie :{
                    one:'',
                    two:'',
                },
                labo :{
                    one:'',
                    two:'',
                },
                clinique :{
                    one:'',
                    two:'',
                },
                centreMD :{
                    one:'',
                    two:'',
                },
                admin_s_hospital :{
                    one:'',
                    two:'',
                },
                admin_s_csb :{
                    one:'',
                    two:'',
                },
                garderie :{
                    one:'',
                    two:'',
                },
                ecole :{
                    one:'',
                    two:'',
                },
                lycee :{
                    one:'',
                    two:'',
                },
                universite :{
                    one:'',
                    two:'',
                },
                formation :{
                    one:'',
                    two:'',
                },
                librairie :{
                    one:'',
                    two:'',
                },
                admin_e_centre :{
                    one:'',
                    two:'',
                },
                admin_e_biblio :{
                    one:'',
                    two:'',
                },
                admin_e_ecole :{
                    one:'',
                    two:'',
                },
                admin_e_lycee :{
                    one:'',
                    two:'',
                },
                admin_e_universite :{
                    one:'',
                    two:'',
                },
                admin_e_ss :{
                    one:'',
                    two:'',
                },
                transporteur :{
                    one:'',
                    two:'',
                },
                autoecole :{
                    one:'',
                    two:'',
                },
                location :{
                    one:'',
                    two:'',
                },
                qiosque :{
                    one:'',
                    two:'',
                },
                parking :{
                    one:'',
                    two:'',
                },
                mecanicien :{
                    one:'',
                    two:'',
                },
                Taxi :{
                    one:'',
                    two:'',
                },
                Louage :{
                    one:'',
                    two:'',
                },
                magazin :{
                    one:'',
                    two:'',
                },
                boulengerie :{
                    one:'',
                    two:'',
                },
                fruiterie :{
                    one:'',
                    two:'',
                },
                patesserie :{
                    one:'',
                    two:'',
                },
                epicerie :{
                    one:'',
                    two:'',
                },
                electromenager :{
                    one:'',
                    two:'',
                },
                meublerie :{
                    one:'',
                    two:'',
                },
                boutique :{
                    one:'',
                    two:'',
                },
                bijouterie :{
                    one:'',
                    two:'',
                },
                quincaillerie :{
                    one:'',
                    two:'',
                },
                depot :{
                    one:'',
                    two:'',
                },
                boucherie :{
                    one:'',
                    two:'',
                },
                cafe :{
                    one:'',
                    two:'',
                },
                restaurant :{
                    one:'',
                    two:'',
                },
                hotels :{
                    one:'',
                    two:'',
                },
                vg_agence :{
                    one:'',
                    two:'',
                },
                coiffure :{
                    one:'',
                    two:'',
                },
                salle_marriage :{
                    one:'',
                    two:'',
                },
                fourniture_marriage :{
                    one:'',
                    two:'',
                },
                orchestre :{
                    one:'',
                    two:'',
                },
                chef :{
                    one:'',
                    two:'',
                },
                photographe :{
                    one:'',
                    two:'',
                },
                forgeron :{
                    one:'',
                    two:'',
                },
                menuisier :{
                    one:'',
                    two:'',
                },
                peinture :{
                    one:'',
                    two:'',
                },
                electricien :{
                    one:'',
                    two:'',
                },
                plombier :{
                    one:'',
                    two:'',
                },
                cristalerie :{
                    one:'',
                    two:'',
                },
                marbrerie :{
                    one:'',
                    two:'',
                },
                courtier :{
                    one:'',
                    two:'',
                },
                contracteur :{
                    one:'',
                    two:'',
                },
                architecture :{
                    one:'',
                    two:'',
                },
                gym :{
                    one:'',
                    two:'',
                },
                pyscine :{
                    one:'',
                    two:'',
                },
                stade :{
                    one:'',
                    two:'',
                },
                cinema :{
                    one:'',
                    two:'',
                },
                theatre :{
                    one:'',
                    two:'',
                },
                musee :{
                    one:'',
                    two:'',
                },
                admin_c_mj :{
                    one:'',
                    two:'',
                },
                admin_c_mc :{
                    one:'',
                    two:'',
                },
                admin_c_me :{
                    one:'',
                    two:'',
                },
                comptable :{
                    one:'',
                    two:'',
                },
                bank :{
                    one:'',
                    two:'',
                },
                admin_f_poste :{
                    one:'',
                    two:'',
                },
                admin_f_rf :{
                    one:'',
                    two:'',
                },
                avocat :{
                    one:'',
                    two:'',
                },
                admin_a_mu :{
                    one:'',
                    two:'',
                },
                admin_a_police :{
                    one:'',
                    two:'',
                },
                admin_a_ar :{
                    one:'',
                    two:'',
                },
                admin_a_court :{
                    one:'',
                    two:'',
                },
                admin_a_mosq :{
                    one:'',
                    two:'',
                },
                ambassade :{
                    one:'',
                    two:'',
                },
                fourragerie :{
                    one:'',
                    two:'',
                },
                veterinaire :{
                    one:'',
                    two:'',
                },
                agritools :{
                    one:'',
                    two:'',
                },
                
            },
            ActionTabData : {
                docteur : {
                    one:'',
                    two:'',
                },
                infirmier :{
                    one:'',
                    two:'',
                },
                pharmacie :{
                    one:'',
                    two:'',
                },
                labo :{
                    one:'',
                    two:'',
                },
                clinique :{
                    one:'',
                    two:'',
                },
                centreMD :{
                    one:'',
                    two:'',
                },
                admin_s_hospital :{
                    one:'',
                    two:'',
                },
                admin_s_csb :{
                    one:'',
                    two:'',
                },
                garderie :{
                    one:'',
                    two:'',
                },
                ecole :{
                    one:'',
                    two:'',
                },
                lycee :{
                    one:'',
                    two:'',
                },
                universite :{
                    one:'',
                    two:'',
                },
                formation :{
                    one:'',
                    two:'',
                },
                librairie :{
                    one:'',
                    two:'',
                },
                admin_e_centre :{
                    one:'',
                    two:'',
                },
                admin_e_biblio :{
                    one:'',
                    two:'',
                },
                admin_e_ecole :{
                    one:'',
                    two:'',
                },
                admin_e_lycee :{
                    one:'',
                    two:'',
                },
                admin_e_universite :{
                    one:'',
                    two:'',
                },
                admin_e_ss :{
                    one:'',
                    two:'',
                },
                transporteur :{
                    one:'',
                    two:'',
                },
                autoecole :{
                    one:'',
                    two:'',
                },
                location :{
                    one:'',
                    two:'',
                },
                qiosque :{
                    one:'',
                    two:'',
                },
                parking :{
                    one:'',
                    two:'',
                },
                mecanicien :{
                    one:'',
                    two:'',
                },
                Taxi :{
                    one:'',
                    two:'',
                },
                Louage :{
                    one:'',
                    two:'',
                },
                magazin :{
                    one:'',
                    two:'',
                },
                boulengerie :{
                    one:'',
                    two:'',
                },
                fruiterie :{
                    one:'',
                    two:'',
                },
                patesserie :{
                    one:'',
                    two:'',
                },
                epicerie :{
                    one:'',
                    two:'',
                },
                electromenager :{
                    one:'',
                    two:'',
                },
                meublerie :{
                    one:'',
                    two:'',
                },
                boutique :{
                    one:'',
                    two:'',
                },
                bijouterie :{
                    one:'',
                    two:'',
                },
                quincaillerie :{
                    one:'',
                    two:'',
                },
                depot :{
                    one:'',
                    two:'',
                },
                boucherie :{
                    one:'',
                    two:'',
                },
                cafe :{
                    one:'',
                    two:'',
                },
                restaurant :{
                    one:'',
                    two:'',
                },
                hotels :{
                    one:'',
                    two:'',
                },
                vg_agence :{
                    one:'',
                    two:'',
                },
                coiffure :{
                    one:'',
                    two:'',
                },
                salle_marriage :{
                    one:'',
                    two:'',
                },
                fourniture_marriage :{
                    one:'',
                    two:'',
                },
                orchestre :{
                    one:'',
                    two:'',
                },
                chef :{
                    one:'',
                    two:'',
                },
                photographe :{
                    one:'',
                    two:'',
                },
                forgeron :{
                    one:'',
                    two:'',
                },
                menuisier :{
                    one:'',
                    two:'',
                },
                peinture :{
                    one:'',
                    two:'',
                },
                electricien :{
                    one:'',
                    two:'',
                },
                plombier :{
                    one:'',
                    two:'',
                },
                cristalerie :{
                    one:'',
                    two:'',
                },
                marbrerie :{
                    one:'',
                    two:'',
                },
                courtier :{
                    one:'',
                    two:'',
                },
                contracteur :{
                    one:'',
                    two:'',
                },
                architecture :{
                    one:'',
                    two:'',
                },
                gym :{
                    one:'',
                    two:'',
                },
                pyscine :{
                    one:'',
                    two:'',
                },
                stade :{
                    one:'',
                    two:'',
                },
                cinema :{
                    one:'',
                    two:'',
                },
                theatre :{
                    one:'',
                    two:'',
                },
                musee :{
                    one:'',
                    two:'',
                },
                admin_c_mj :{
                    one:'',
                    two:'',
                },
                admin_c_mc :{
                    one:'',
                    two:'',
                },
                admin_c_me :{
                    one:'',
                    two:'',
                },
                comptable :{
                    one:'',
                    two:'',
                },
                bank :{
                    one:'',
                    two:'',
                },
                admin_f_poste :{
                    one:'',
                    two:'',
                },
                admin_f_rf :{
                    one:'',
                    two:'',
                },
                avocat :{
                    one:'',
                    two:'',
                },
                admin_a_mu :{
                    one:'',
                    two:'',
                },
                admin_a_police :{
                    one:'',
                    two:'',
                },
                admin_a_ar :{
                    one:'',
                    two:'',
                },
                admin_a_court :{
                    one:'',
                    two:'',
                },
                admin_a_mosq :{
                    one:'',
                    two:'',
                },
                ambassade :{
                    one:'',
                    two:'',
                },
                fourragerie :{
                    one:'',
                    two:'',
                },
                veterinaire :{
                    one:'',
                    two:'',
                },
                agritools :{
                    one:'',
                    two:'',
                },
                
            },
            FollowTabData : {
                docteur : {
                    one:'',
                    two:'',
                },
                infirmier :{
                    one:'',
                    two:'',
                },
                pharmacie :{
                    one:'',
                    two:'',
                },
                labo :{
                    one:'',
                    two:'',
                },
                clinique :{
                    one:'',
                    two:'',
                },
                centreMD :{
                    one:'',
                    two:'',
                },
                admin_s_hospital :{
                    one:'',
                    two:'',
                },
                admin_s_csb :{
                    one:'',
                    two:'',
                },
                garderie :{
                    one:'',
                    two:'',
                },
                ecole :{
                    one:'',
                    two:'',
                },
                lycee :{
                    one:'',
                    two:'',
                },
                universite :{
                    one:'',
                    two:'',
                },
                formation :{
                    one:'',
                    two:'',
                },
                librairie :{
                    one:'',
                    two:'',
                },
                admin_e_centre :{
                    one:'',
                    two:'',
                },
                admin_e_biblio :{
                    one:'',
                    two:'',
                },
                admin_e_ecole :{
                    one:'',
                    two:'',
                },
                admin_e_lycee :{
                    one:'',
                    two:'',
                },
                admin_e_universite :{
                    one:'',
                    two:'',
                },
                admin_e_ss :{
                    one:'',
                    two:'',
                },
                transporteur :{
                    one:'',
                    two:'',
                },
                autoecole :{
                    one:'',
                    two:'',
                },
                location :{
                    one:'',
                    two:'',
                },
                qiosque :{
                    one:'',
                    two:'',
                },
                parking :{
                    one:'',
                    two:'',
                },
                mecanicien :{
                    one:'',
                    two:'',
                },
                Taxi :{
                    one:'',
                    two:'',
                },
                Louage :{
                    one:'',
                    two:'',
                },
                magazin :{
                    one:'',
                    two:'',
                },
                boulengerie :{
                    one:'',
                    two:'',
                },
                fruiterie :{
                    one:'',
                    two:'',
                },
                patesserie :{
                    one:'',
                    two:'',
                },
                epicerie :{
                    one:'',
                    two:'',
                },
                electromenager :{
                    one:'',
                    two:'',
                },
                meublerie :{
                    one:'',
                    two:'',
                },
                boutique :{
                    one:'',
                    two:'',
                },
                bijouterie :{
                    one:'',
                    two:'',
                },
                quincaillerie :{
                    one:'',
                    two:'',
                },
                depot :{
                    one:'',
                    two:'',
                },
                boucherie :{
                    one:'',
                    two:'',
                },
                cafe :{
                    one:'',
                    two:'',
                },
                restaurant :{
                    one:'',
                    two:'',
                },
                hotels :{
                    one:'',
                    two:'',
                },
                vg_agence :{
                    one:'',
                    two:'',
                },
                coiffure :{
                    one:'',
                    two:'',
                },
                salle_marriage :{
                    one:'',
                    two:'',
                },
                fourniture_marriage :{
                    one:'',
                    two:'',
                },
                orchestre :{
                    one:'',
                    two:'',
                },
                chef :{
                    one:'',
                    two:'',
                },
                photographe :{
                    one:'',
                    two:'',
                },
                forgeron :{
                    one:'',
                    two:'',
                },
                menuisier :{
                    one:'',
                    two:'',
                },
                peinture :{
                    one:'',
                    two:'',
                },
                electricien :{
                    one:'',
                    two:'',
                },
                plombier :{
                    one:'',
                    two:'',
                },
                cristalerie :{
                    one:'',
                    two:'',
                },
                marbrerie :{
                    one:'',
                    two:'',
                },
                courtier :{
                    one:'',
                    two:'',
                },
                contracteur :{
                    one:'',
                    two:'',
                },
                architecture :{
                    one:'',
                    two:'',
                },
                gym :{
                    one:'',
                    two:'',
                },
                pyscine :{
                    one:'',
                    two:'',
                },
                stade :{
                    one:'',
                    two:'',
                },
                cinema :{
                    one:'',
                    two:'',
                },
                theatre :{
                    one:'',
                    two:'',
                },
                musee :{
                    one:'',
                    two:'',
                },
                admin_c_mj :{
                    one:'',
                    two:'',
                },
                admin_c_mc :{
                    one:'',
                    two:'',
                },
                admin_c_me :{
                    one:'',
                    two:'',
                },
                comptable :{
                    one:'',
                    two:'',
                },
                bank :{
                    one:'',
                    two:'',
                },
                admin_f_poste :{
                    one:'',
                    two:'',
                },
                admin_f_rf :{
                    one:'',
                    two:'',
                },
                avocat :{
                    one:'',
                    two:'',
                },
                admin_a_mu :{
                    one:'',
                    two:'',
                },
                admin_a_police :{
                    one:'',
                    two:'',
                },
                admin_a_ar :{
                    one:'',
                    two:'',
                },
                admin_a_court :{
                    one:'',
                    two:'',
                },
                admin_a_mosq :{
                    one:'',
                    two:'',
                },
                ambassade :{
                    one:'',
                    two:'',
                },
                fourragerie :{
                    one:'',
                    two:'',
                },
                veterinaire :{
                    one:'',
                    two:'',
                },
                agritools :{
                    one:'',
                    two:'',
                },
                
            },
            PostsTabs :{
                noPostToShow: '',
                textText: '',
                photoText: '',
                videoText: '',
            },
            toastText:{
                failOne:'',
                failTwo:'',
            },
        },
        userProfile: {
            notificationPage : {
                docteur_rdv_saved: 'La demande de rendez-vous avec le médecin {{one}} a été enregistrée avec succès à la date de {{one}}.',
                docteur_rdv_accepted: 'Carte d\'acceptation du rendez-vous avec le médecin',
                docteur_rdv_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                docteur_rdv_retarted: 'Carte de retard du médecin',
                docteur_rdv_redirected: 'La demande de rendez-vous avec le médecin {{one}} a été redirigée vers le médecin {{one}} (téléphone : {{one}}, adresse : {{one}})',
                infirmier_rdv_saved: 'La demande de rendez-vous avec le médecin {{one}} a été enregistrée avec succès à la date de {{one}}.',
                infirmier_rdv_accepted: 'Carte d\'acceptation du rendez-vous avec le médecin',
                infirmier_rdv_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                infirmier_rdv_retarted: 'Carte de retard du médecin',
                infirmier_rdv_redirected: 'La demande de rendez-vous avec le médecin {{one}} a été redirigée vers le médecin {{one}} (téléphone : {{one}}, adresse : {{one}})',
                pharmacie_shop_saved: 'La demande d\'achat de médicaments ({{one}} produits) auprès de la pharmacie {{one}} a été enregistrée.',
                pharmacie_shop_accepted: 'Carte d\'acceptation de la commande de pharmacie',
                pharmacie_shop_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                pharmacie_shop_livred: 'La commande a été expédiée depuis la pharmacie {{one}} <br /> <small> Vous recevrez votre commande le {{one}} à {{one}}</small>',
                pharmacie_shop_edited: 'La commande d\'achat à la pharmacie {{one}} a été modifiée.',
                pharmacie_rdv_saved: 'La demande de rendez-vous avec la pharmacie {{one}} pour {{one}} a été enregistrée avec succès à la date de {{one}}.',
                pharmacie_rdv_accepted: 'Carte d\'acceptation du rendez-vous avec la pharmacie',
                pharmacie_rdv_rejected: 'La demande de rendez-vous avec la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                pharmacie_rdv_retarted: 'Carte de retard du médecin',
                clinique_reserver_saved: 'La demande de réservation à la clinique {{one}} pour {{one}} a été enregistrée avec succès entre {{one}} et {{one}}.',
                centre_reserver_saved: 'La demande de réservation au centre {{one}} pour {{one}} a été enregistrée avec succès entre {{one}} et {{one}}.',
                labo_rdv_saved: 'La demande de rendez-vous avec le laboratoire {{one}} pour {{one}} a été enregistrée avec succès à la date de {{one}}.',
                labo_rdv_accepted: 'Carte d\'acceptation du rendez-vous avec le laboratoire',
                labo_rdv_rejected: 'La demande de rendez-vous avec le laboratoire {{one}} a été rejetée <br /> <small>""</small>',
                labo_rdv_retarted: 'Carte de retard du médecin',
                labo_rdv_pret: 'Les résultats de l\'analyse sont prêts au laboratoire {{one}}:',
                garderie_inscription_saved: 'Votre enfant {{one}} a été inscrit à la garderie : {{one}} avec succès.',
                garderie_inscription_accepted: 'Carte d\'acceptation de la garderie',
                garderie_inscription_rejected: 'La demande d\'inscription à la garderie {{one}} a été rejetée <br /> <small>""</small>',
                garderie_souscription_saved: 'Votre enfant {{one}} a été inscrit à la garderie {{one}} avec succès à la date de {{one}}.',
                garderie_souscription_accepted: 'Carte d\'acceptation de la garderie',
                garderie_souscription_rejected: 'La demande d\'inscription à la garderie {{one}} a été rejetée <br /> <small>""</small>',
                ecole_inscription_saved: 'Votre enfant {{one}} a été inscrit à la maternelle : {{one}} avec succès.',
                ecole_inscription_accepted: 'Carte d\'acceptation de l\'inscription à l\'école',
                ecole_inscription_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                ecole_souscription_saved: 'Votre enfant {{one}} a été inscrit à l\'école privée {{one}} avec succès à la date de {{one}}.',
                ecole_souscription_accepted: 'Carte d\'acceptation de l\'inscription à l\'école',
                ecole_souscription_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                lycee_inscription_saved: 'La demande d\'inscription à l\'établissement de formation privé {{one}} a été enregistrée avec succès.',
                lycee_inscription_accepted: 'Carte d\'acceptation de l\'inscription au lycée',
                lycee_inscription_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                lycee_souscription_saved: 'Votre enfant {{one}} a été inscrit à la maternelle {{one}} avec succès à la date de {{one}}.',
                lycee_souscription_accepted: 'Carte d\'acceptation de l\'inscription au lycée',
                lycee_souscription_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                universite_inscription_saved: 'La demande d\'inscription à l\'université privée {{one}} a été enregistrée avec succès.',
                universite_inscription_accepted: 'Carte d\'acceptation de l\'inscription à l\'université',
                universite_inscription_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                universite_souscription_saved: 'Votre enfant {{one}} a été inscrit à la maternelle {{one}} avec succès à la date de {{one}}.',
                universite_souscription_accepted: 'Carte d\'acceptation de l\'inscription à l\'université',
                universite_souscription_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                formation_inscription_saved: 'La demande d\'inscription à l\'établissement de formation privé {{one}} a été enregistrée avec succès.',
                formation_inscription_accepted: 'Carte d\'acceptation de l\'inscription à la formation',
                formation_inscription_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                formation_souscription_saved: 'Votre enfant {{one}} a été inscrit à la maternelle {{one}} avec succès à la date de {{one}}.',
                formation_souscription_accepted: 'Carte d\'acceptation de l\'inscription à la formation',
                formation_souscription_rejected: 'La demande d\'achat de médicaments auprès de la pharmacie {{one}} a été rejetée <br /> <small>""</small>',
                transporteur_request_saved: 'La demande de transport de marchandises ({{one}} produits) avec l\'agence de transport {{two}} de : {{three}} à : {{four}} a été enregistrée.',
                transporteur_request_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                transporteur_request_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                transporteur_request_retarted: 'Carte de retard du médecin',
                transporteur_request_livree: 'Les résultats de l\'analyse sont prêts au laboratoire {{one}}:',
                autoecole_inscrie_saved: 'L\'inscription à l\'auto-école : {{one}} pour obtenir un permis de conduire de la catégorie {{one}} a été enregistrée avec succès.',
                autoecole_inscrie_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                autoecole_inscrie_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                autoecole_inscrie_redirected: 'La demande de rendez-vous avec le médecin {{one}} a été redirigée vers le médecin {{one}} (téléphone : {{one}}, adresse : {{one}})',
                taxi_request_saved: 'La demande de réservation de taxi a été enregistrée.',
                taxi_rdv_saved: 'La demande de rendez-vous avec un taxi le {{one}} à {{one}} a été enregistrée.',
                location_request_saved: 'La demande de location de véhicule chez l\'agence de location de voitures {{one}} de {{one}} à {{one}} a été enregistrée.',
                location_request_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                location_request_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                location_request_edited: 'La demande d\'achat à la pharmacie {{one}} a été modifiée.',
                parking_reserver_saved: 'La réservation d\'une place de parking : {{one}} de {{one}} à {{one}} pour le camion ({{one}}) : {{one}} a été enregistrée.',
                parking_souscrire_saved: 'L\'abonnement au parking : {{one}} de {{one}} à {{one}} pour le camion ({{one}}) : {{one}} a été enregistré.',
                qiosque_request_saved: 'La demande d\'achat de carburant à la station-service : {{one}}. Le rendez-vous a été enregistré pour l\'heure : {{one}}.',
                qiosque_lavage_saved: 'La demande de rendez-vous pour le lavage de {{one}} chez {{one}} le {{one}} à {{one}} a été enregistrée.',
                coiffure_reserver_saved: 'La demande de réservation dans le salon de coiffure {{one}} le {{one}} à {{one}} a été enregistrée.',
                coiffure_reserver_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                coiffure_reserver_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                coiffure_reserver_retarted: 'Carte de retard du médecin',
                salon_marriage_reserver_saved: 'La demande de réservation dans la salle de mariage {{one}} pour l\'événement {{one}} a été enregistrée avec succès.',
                chef_reserver_saved: 'La demande de réservation d\'un rendez-vous avec le cuisinier {{one}} a été enregistrée avec succès.',
                orchestre_reserver_saved: 'La demande de réservation d\'un rendez-vous avec l\'orchestre {{one}} a été enregistrée avec succès.',
                photographe_reserver_saved: 'La demande de réservation d\'un rendez-vous avec le photographe {{one}} a été enregistrée avec succès.',
                fourniture_marriage_location_saved: 'La demande de location de fournitures de mariage chez {{one}} a été enregistrée avec succès.',
                magazin_commande_saved: 'La commande d\'achat de produits alimentaires auprès du magasin {{one}} a été enregistrée.',
                boulangerie_shop_saved: 'La commande d\'achat de pain chez le boulanger {{one}} a été enregistrée.',
                boucheries_shop_saved: 'La commande d\'achat de viande à la boucherie {{one}} a été enregistrée.',
                fruiterie_shop_saved: 'La commande d\'achat de fruits et légumes au magasin {{one}} a été enregistrée.',
                patisserie_shop_saved: 'La commande d\'achat de pâtisseries à la pâtisserie {{one}} a été enregistrée.',
                epicerie_shop_saved: 'La commande d\'achat à l\'épicerie {{one}} a été enregistrée.',
                quincaillerie_shop_saved: 'La commande d\'achat d\'outils chez le quincailler {{one}} a été enregistrée.',
                boutique_shop_saved: 'La commande d\'achat de vêtements au magasin {{one}} a été enregistrée.',
                bijouterie_shop_saved: 'La commande d\'achat de bijoux au magasin {{one}} a été enregistrée.',
                librairie_shop_saved: 'La commande d\'achat de fournitures scolaires à la librairie {{one}} a été enregistrée.',
                electromenager_shop_saved: 'La commande d\'achat d\'appareils électroménagers au magasin {{one}} a été enregistrée.',
                meubles_shop_saved: 'La commande d\'achat de meubles au magasin {{one}} a été enregistrée.',
                depot_commande_saved: 'La commande d\'achat au magasin {{one}} a été enregistrée.',
                avocat_souscrire_saved: 'La demande de rendez-vous avec l\'avocat {{one}} a été enregistrée avec succès à la date de {{one}}.',
                avocat_souscrire_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                avocat_souscrire_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                avocat_souscrire_retarted: 'Carte de retard du médecin',
                avocat_souscrire_redirected: 'La demande de rendez-vous avec le médecin {{one}} a été redirigée vers le médecin {{one}} (téléphone : {{one}}, adresse : {{one}})',
                avocat_souscrire_informer: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                avocat_rdv_saved: 'La demande de rendez-vous avec l\'avocat {{one}} a été enregistrée avec succès à la date de {{one}}.',
                avocat_rdv_saved_2: 'La demande de rendez-vous avec l\'avocat {{one}} a été enregistrée avec succès à la date de {{one}}.',
                gym_souscription_saved: 'La demande d\'inscription ({{one}}) à la salle de sport {{two}} a été enregistrée avec succès. ',
                gym_souscription_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                gym_souscription_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été rejetée <br /> <small>""</small>',
                pyscine_reserver_saved: 'La demande de réservation à la piscine {{one}} le {{one}} de {{one}} à {{one}} a été enregistrée avec succès.',
                pyscine_souscrire_saved: 'La demande d\'inscription ({{one}}) au stade de sport {{one}} a été enregistrée avec succès.',
                stade_reserver_saved: 'La demande de réservation au stade de sport {{one}} le {{one}} de {{one}} à {{one}} a été enregistrée avec succès.',
                stade_souscrire_saved: 'La demande d\'inscription ({{one}}) au stade de sport {{one}} a été enregistrée avec succès.',
                cinema_reserver_saved: 'La demande de réservation dans la salle de cinéma {{one}} pour regarder le film {{one}} a été enregistrée avec succès.',
                theatre_reserver_saved: 'La demande de réservation au théâtre {{one}} pour regarder la pièce de théâtre {{one}} a été enregistrée avec succès.',
                art_avis_saved: 'La demande d\'inscription {{one}} à la salle de sport {{one}} a été enregistrée avec succès. Le départ est prévu pour le {{one}}',
                musee_reserver_saved: 'La demande de réservation au musée {{one}} le {{one}} de {{one}} à {{one}} a été enregistrée avec succès.',
                securite_reserver_saved: 'La demande de réservation pour le service de sécurité ({{one}}) a été enregistrée avec succès pour l\'événement ({{one}})',
                decoration_location_saved: 'La demande de location de décorations pour l\'événement ({{one}}) a été enregistrée avec succès.',
                traiteur_reserver_saved: 'La demande de réservation de service traiteur pour l\'événement ({{one}}) a été enregistrée avec succès.',
                fleuriste_reserver_saved: 'La demande de réservation chez le fleuriste pour l\'événement ({{one}}) a été enregistrée avec succès.',
                editeur_reserver_saved: 'La demande de réservation chez l\'éditeur ({{one}}) pour l\'impression de l\'ouvrage ({{one}}) a été enregistrée avec succès.',
                hotels_reserver_saved: 'La demande de réservation {{one}} à l\'hôtel {{one}} pour la période entre {{one}} et {{one}} a été enregistrée avec succès.',
                hotels_reserver_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                hotels_reserver_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été refusée <br /> <small>"{{one}}"</small>',
                hotels_service_saved: 'La demande de réservation à l\'hôtel {{one}} pour la salle de sport {{one}} a été enregistrée avec succès. La date de début a été enregistrée pour le {{one}}.',
                hotels_service_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                hotels_service_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été refusée <br /> <small>"{{one}}"</small>',
                agence_service_saved: 'La demande de voyage vers {{one}} avec l\'agence de voyage {{one}} a été enregistrée avec succès.',
                restaurant_commande_saved: 'La commande d\'un plat du restaurant {{one}} a été enregistrée avec succès',
                restaurant_commande_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                restaurant_commande_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été refusée <br /> <small>"{{one}}"</small>',
                restaurant_reservation_saved: 'La demande de réservation au restaurant {{one}} a été enregistrée avec succès pour la date {{one}}.',
                restaurant_reservation_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                restaurant_reservation_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été refusée <br /> <small>"{{one}}"</small>',
                restaurant_reservation_edited: 'La demande de modification de la commande de la pharmacie {{one}} a été effectuée',
                cafe_commande_saved: 'La commande d\'une boisson du café {{one}} a été enregistrée avec succès',
                cafe_reservation_saved: 'La demande de réservation au café {{one}} a été enregistrée avec succès pour la date {{one}}.',
                courtier_request_saved: 'La demande {{one}} {{one}} du courtier {{one}} a été enregistrée',
                courtier_torent_saved: 'La {{one}} {{one}} avec le courtier {{one}} a été enregistrée',
                contracteur_service_saved: 'La demande de construction {{one}} par l\'entrepreneur {{one}} a été enregistrée avec succès',
                architecture_service_saved: 'La demande de conception {{one}} par l\'architecte {{one}} a été enregistrée avec succès',
                comptable_service_saved: 'La demande de {{one}} pour la période entre {{one}} et {{one}} par le comptable {{one}} a été enregistrée avec succès',
                veterinaire_rdv_saved: 'La demande de rendez-vous avec le vétérinaire {{one}} a été enregistrée avec succès pour la date {{one}}',
                veterinaire_rdv_accepted: 'Carte d\'acceptation du rendez-vous chez le médecin',
                veterinaire_rdv_rejected: 'La demande de rendez-vous avec le médecin {{one}} a été refusée <br /> <small>"{{one}}"</small>',
                veterinaire_rdv_retarted: 'Le rendez-vous chez le médecin a été retardé',
                veterinaire_rdv_redirected: 'La demande de rendez-vous avec le médecin {{one}} a été redirigée vers le médecin {{one}} (téléphone : {{one}}, adresse : {{one}})',
                fourragerie_shop_saved: 'La demande d\'achat d\'outils auprès du magasin {{one}} a été enregistrée',

            },
            suivieTitlePage: {
                docteur_rdv: 'Медицинская запись',
                infirmier_rdv: 'Запись к медсестре',
                pharmacie_shop: 'Покупка лекарств',
                pharmacie_rdv: 'Запись в аптеку',
                clinique_reserver: 'Бронирование клиники',
                centre_reserver: 'Бронирование медицинского центра',
                labo_rdv: 'Запрос анализа в лаборатории',
                garderie_inscription: 'Регистрация в детском саду',
                formation_inscription: 'Регистрация в учебном центре',
                ecole_inscription: 'Регистрация в школе',
                lycee_inscription: 'Регистрация в частной школе',
                universite_inscription: 'Регистрация в частном университете',
                transporteur_request: 'Доставка товаров',
                autoecole_inscrie: 'Получение прав',
                location_request: 'Прокат автомобилей',
                parking_reserver: 'Бронирование парковки',
                parking_souscrire: 'Подписка на парковку',
                qiosque_request: 'Запрос на заправку топлива',
                qiosque_lavage: 'Запрос на мойку автомобиля',
                magazin_commande: 'Заказ продуктов',
                boulangerie_shop: 'Покупка хлеба',
                boucheries_shop: 'Покупка мяса',
                fruiterie_shop: 'Покупка фруктов и овощей',
                patisserie_shop: 'Покупка выпечки',
                epicerie_shop: 'Покупка в бакалее',
                quincaillerie_shop: 'Покупка инструментов',
                electromenager_shop: 'Покупка бытовой техники',
                meubles_shop: 'Покупка мебели',
                boutique_shop: 'Покупка одежды',
                bijouterie_shop: 'Покупка ювелирных изделий',
                librairie_shop: 'Покупка школьных принадлежностей',
                depot_commande: 'Заказ продовольствия',
                coiffure_reserver: 'Бронирование в парикмахерской',
                salon_marriage_reserver: 'Бронирование места для свадьбы',
                chef_reserver: 'Бронирование повара',
                orchestre_reserver: 'Бронирование оркестра',
                photographe_reserver: 'Бронирование фотографа',
                fourniture_marriage_location: 'Аренда оборудования для свадьбы',
                avocat_souscrire: 'Обращение к адвокату',
                avocat_rdv: 'Встреча с адвокатом',
                gym_souscription: 'Абонемент в спортзал',
                pyscine_souscrire: 'Абонемент в бассейн',
                pyscine_reserver: 'Бронирование бассейна',
                stade_souscrire: 'Абонемент на стадион',
                stade_reserver: 'Бронирование стадиона',
                cinema_reserver: 'Бронирование кинотеатра',
                theatre_reserver: 'Бронирование театра',
                art_avis: 'Подписка в художественном зале',
                musee_reserver: 'Бронирование музея',
                restaurant_commande: 'Заказ блюда в ресторане',
                restaurant_reservation: 'Бронирование в ресторане',
                cafe_commande: 'Заказ напитка в кафе',
                cafe_reservation: 'Бронирование в кафе',
                hotels_reserver: 'Бронирование в отеле',
                hotels_service: 'Запрос услуг в отеле',
                agence_service: 'Регистрация в туристическом агентстве',
                comptable_service: 'Запрос услуг бухгалтера',
                courtier_request: 'Запрос услуг недвижимости',
                courtier_torent: 'Запрос услуг недвижимости',
                contracteur_service: 'Запрос строительных услуг',
                architecture_service: 'Запрос архитектурных услуг',
                veterinaire_rdv: 'Визит к ветеринару',
                fourragerie_shop: 'Покупка корма'
            },
            favoiteItemList: {
                admin: 'Администрация',
                sante: 'Здравоохранение',
                education: 'Образование',
                commerce: 'Торговля',
                transport: 'Транспорт',
                life: 'Досуг',
                sport: 'Спорт',
                finance: 'Финансы',
                construction: 'Строительство',
                autre: 'Прочее'
            },
            PostsTabs :{
                noPostToShow: '',
                textText: '',
                photoText: '',
                videoText: '',
            },
            toastText:{
                failOne:'',
                failTwo:'',
            },
        },
        subscribeToSystems: {
            mainTitle: 'Регистрация {{ one }} на Abyedh.',
            inscriptionUserWith: 'Владелец',
            infoGeneraleText: 'Общая информация о {{one}}',
            infoGeneraleData: {
              nomEtPrenon: 'Имя {{one}}',
              nomEtPreninPlaceholder: '',
              genreText: 'Пол {{one}}',
              genreTextPlaceholder: 'Выберите пол {{one}}',
              PhoneText: 'Телефонный номер {{one}}',
              PhoneTextPlaceholder: 'Телефонный номер {{one}}',
              positionGeoText: 'Географическое местоположение {{one}}',
              positionGeoTextPlaceholder: '',
              GouvDelegText: 'Выберите регион',
              GouvDelegTextPlaceholder: 'Выберите отдел',
              AdesssText: 'Адрес {{one}}',
              AdesssTextPlaceholder: ''
            },
            PositionGpsText: 'Позиция GPS',
            PositionGpsClickHereText: 'Нажмите здесь, чтобы установить ваше текущее местоположение по умолчанию',
            HoraireText: 'Рабочие часы',
            HoraireData: {
              alwaysOpenOne: 'Всегда открыто',
              alwaysOpenTwo: 'Когда эта функция включена, она всегда отображается.',
              weekDayes: {
                lundi: 'Понедельник',
                mardi: 'Вторник',
                mercredi: 'Среда',
                jeudi: 'Четверг',
                vendredi: 'Пятница',
                samedi: 'Суббота',
                dimanche: 'Воскресенье'
              },
              dayText: 'День',
              matinText: 'Утро',
              soirText: 'Вечер',
              editData: {
                isDayOff: '',
                ouiText: 'Да',
                nonText: 'Нет',
                editButtonText: ''
              }
            },
            InscriptionLastText: 'Регистрация',
            InscriptionLast: {
              firstCondition: 'Процесс регистрации может занять до 15 часов на проверку предоставленной информации. Вы можете отслеживать процесс на странице отслеживания.',
              secondCondition: 'Каждый абонент получает бесплатную мини-версию системы для получения запросов и общения с клиентами.',
              thirdCondition: 'Любая регистрация, содержащая неточную или вводящую в заблуждение информацию, будет автоматически отменена.',
              okayCondition: 'ОК',
              InscriptionButtonText: 'Регистрация'
            },
            successModalData: {
              operationR: 'Регистрация прошла успешно',
              cliquerPourPasser: 'Нажмите, чтобы быть перенаправленным на страницу отслеживания принятия',
              followPage: 'Страница отслеживания'
            },
            shouldLogInData: {
              shouldText: 'Сначала вам нужно иметь учетную запись на платформе Abyedh',
              detailText: 'Чтобы зарегистрироваться на {{one}}, вам сначала нужно иметь бесплатную учетную запись на платформе Abyedh. Сначала зарегистрируйтесь на платформе, а затем вернитесь на эту страницу, чтобы завершить регистрацию в системе.',
              clicLink: 'Создать аккаунт'
            }
        },
        appPages : {
            loginPage : {
                nomDuSytemList : {

                },
                versionReduit:'',
                loginText:'',
                identifiantPlaceholder :'',
                passwordPlaceholder :'',
                loginButtonText:'',
                alternativeTextOne:'',
                alternativeTextTwo:'',
                alternativeTextThree:'',
            },
            mainPage : {
                adsData: {
                    adsContentTextOne: "Вы используете бесплатную версию, хотите обновить",
                    adsContentTextTwo: "до полной версии",
                    adsContentTextThree: "системы",
                    buttonNavText: "Открыть систему"
                },
                statData: {
                    attente: "Ожидание",
                    accepter: "Принято",
                    vueText: "Просмотрено",
                    refusee: "Отклонено",
                    terminer: "Завершено"
                },
                stepsBySystem : {
                    docteur: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    pharmacie: {
                    pharmacie_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    },
                    pharmacie_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        T: "Termineé"
                    }
                    },
                    clinique: {
                    clinique_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    labo: {
                    labo_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        PR: "Pret",
                        T: "Termineé"
                    }
                    },
                    infirmier: {
                    infirmier_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    centreMD: {
                    centre_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        T: "Termineé"
                    }
                    },
                    garderie: {
                    garderie_inscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    garderie_souscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    ecole: {
                    garderie_inscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    garderie_souscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    lycee: {
                    garderie_inscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    garderie_souscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    universite: {
                    garderie_inscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    garderie_souscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    librairie: {
                    librairie_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    formation: {
                    formation_inscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    formation_souscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    impremerie: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    transporteur: {
                    transporteur_request: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    autoecole: {
                    autoecole_inscrie: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    parking: {
                    parking_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        T: "Termineé"
                    },
                    parking_souscrire: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    qiosque: {
                    qiosque_request: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        T: "Termineé"
                    },
                    qiosque_lavage: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        T: "Termineé"
                    }
                    },
                    location: {
                    location_request: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        T: "Termineé"
                    }
                    },
                    mecanicien: {
                    location_request: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        T: "Termineé"
                    }
                    },
                    magazin: {
                    magazin_commande: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    fruiterie: {
                    fruiterie_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    boucherie: {
                    boucheries_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    boulengerie: {
                    boulangerie_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    patesserie: {
                    patisserie_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    epicerie: {
                    epicerie_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    electromenager: {
                    electromenager_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    meublerie: {
                    meubles_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    bijouterie: {
                    bijouterie_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Pret",
                        T: "Termineé"
                    }
                    },
                    quincaillerie: {
                    quincaillerie_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    boutique: {
                    boutique_shop: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    depot: {
                    depot_commande: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        LV: "Livreé",
                        T: "Termineé"
                    }
                    },
                    menuisier: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    forgeron: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    cristalerie: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    marbrerie: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    electricien: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    peinture: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    plombier: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    coiffure: {
                    coiffure_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        RD: "Retardeé",
                        T: "Termineé"
                    }
                    },
                    salle_marriage: {
                    salon_marriage_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        T: "Termineé"
                    }
                    },
                    fourniture_marriage: {
                    fourniture_marriage_location: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    photographe: {
                    photographe_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        PI: "Plus d Info",
                        T: "Termineé"
                    }
                    },
                    chef: {
                    chef_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        PI: "Plus d Info",
                        T: "Termineé"
                    }
                    },
                    orchestre: {
                    orchestre_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        T: "Termineé"
                    }
                    },
                    gym: {
                    gym_souscription: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    pyscine: {
                    pyscine_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    pyscine_souscrire: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    stade: {
                    stade_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    stade_souscrire: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    cinema: {
                    cinema_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        T: "Termineé"
                    }
                    },
                    theatre: {
                    theatre_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        T: "Termineé"
                    }
                    },
                    musee: {
                    musee_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    avocat: {
                    avocat_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        PI: "Plus d Info",
                        T: "Termineé"
                    }
                    },
                    restaurant: {
                    restaurant_commande: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    restaurant_reservation: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        T: "Termineé"
                    }
                    },
                    cafe: {
                    cafe_commande: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    cafe_reservation: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        MD: "Modifier",
                        T: "Termineé"
                    }
                    },
                    hotels: {
                    hotels_reserver: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    hotels_service: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    vg_agence: {
                    agence_service: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        PI: "Plus d Info",
                        T: "Termineé"
                    }
                    },
                    comptable: {
                    comptable_service: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        PI: "Plus d Info",
                        T: "Termineé"
                    }
                    },
                    socite: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    bank: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    courtier: {
                    courtier_request: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    },
                    courtier_torent: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        T: "Termineé"
                    }
                    },
                    contracteur: {
                    contracteur_service: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        PI: "Plus d Info",
                        T: "Termineé"
                    }
                    },
                    architecture: {
                    architecture_service: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        PI: "Plus d Info",
                        T: "Termineé"
                    }
                    },
                    veterinaire: {
                    veterinaire_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    fourragerie: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    agritools: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    },
                    farm: {
                    docteur_rdv: {
                        W: "En Attent",
                        S: "Vu",
                        A: "Accepteé",
                        R: "Refuseé",
                        RT: "Retardeé",
                        RD: "Redirecteé",
                        T: "Termineé"
                    }
                    }
                },
                requestTabs : {
                    gestionDe:'Gestion des',
                    docteur: {
                    docteur_rdv: "Rendy-Vous",
                    Spesific: "Tarifs & Autres"
                    },
                    pharmacie: {
                    pharmacie_shop: "Commandes",
                    pharmacie_rdv: "Reservation",
                    Spesific: "Tarif"
                    },
                    clinique: {
                    clinique_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    labo: {
                    labo_rdv: "Rendy Vous",
                    Spesific: "Tarif & Autres"
                    },
                    infirmier: {
                    infirmier_rdv: "Rendy Vous",
                    Spesific: "Tarif"
                    },
                    centreMD: {
                    centre_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    garderie: {
                    garderie_inscription: "Inscription",
                    garderie_souscription: "Souscription",
                    Spesific: "Tarif"
                    },
                    ecole: {
                    garderie_inscription: "Inscription",
                    garderie_souscription: "Souscription",
                    Spesific: "Tarif"
                    },
                    lycee: {
                    garderie_inscription: "Inscription",
                    garderie_souscription: "Souscription",
                    Spesific: "Tarif"
                    },
                    universite: {
                    garderie_inscription: "Inscription",
                    garderie_souscription: "Souscription",
                    Spesific: "Tarif"
                    },
                    librairie: {
                    librairie_shop: "Commandes",
                    Spesific: "Articles"
                    },
                    formation: {
                    formation_inscription: "Inscription",
                    Spesific: "Tarif"
                    },
                    impremerie: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    transporteur: {
                    transporteur_request: "Commandes",
                    Spesific: "Tarif"
                    },
                    autoecole: {
                    autoecole_inscrie: "Inscription",
                    Spesific: "Tarif"
                    },
                    parking: {
                    parking_reserver: "Reservation",
                    parking_souscrire: "Souscription",
                    Spesific: "Tarif"
                    },
                    qiosque: {
                    qiosque_request: "Commandes",
                    qiosque_lavage: "Lavage",
                    Spesific: "Tarif"
                    },
                    location: {
                    location_request: "Demandes de Location",
                    Spesific: "Tarif"
                    },
                    mecanicien: {
                    location_request: "Rendy-Vous",
                    Spesific: "Tarif"
                    },
                    magazin: {
                    magazin_commande: "Commandes",
                    Spesific: "Articles"
                    },
                    fruiterie: {
                    fruiterie_shop: "Commandes",
                    Spesific: "Traifs"
                    },
                    boucherie: {
                    boucheries_shop: "Commandes",
                    Spesific: "Traifs"
                    },
                    boulengerie: {
                    boulangerie_shop: "Commandes",
                    Spesific: "Traifs"
                    },
                    patesserie: {
                    patisserie_shop: "Commandes",
                    Spesific: "Tarifs"
                    },
                    epicerie: {
                    epicerie_shop: "Commandes",
                    Spesific: "Articles"
                    },
                    electromenager: {
                    electromenager_shop: "Commandes",
                    Spesific: "Articles"
                    },
                    meublerie: {
                    meubles_shop: "Commandes",
                    Spesific: "Articles"
                    },
                    bijouterie: {
                    bijouterie_shop: "Commandes",
                    Spesific: "Articles"
                    },
                    quincaillerie: {
                    quincaillerie_shop: "Commandes",
                    Spesific: "Articles"
                    },
                    boutique: {
                    boutique_shop: "Commandes",
                    Spesific: "Articles"
                    },
                    depot: {
                    depot_commande: "Commandes",
                    Spesific: "Articles"
                    },
                    menuisier: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    forgeron: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    cristalerie: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    marbrerie: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    electricien: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    peinture: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    plombier: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    coiffure: {
                    coiffure_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    salle_marriage: {
                    salon_marriage_reserver: "Reservations",
                    Spesific: "Tarif"
                    },
                    fourniture_marriage: {
                    fourniture_marriage_location: "Reservation",
                    Spesific: "Tarif"
                    },
                    photographe: {
                    photographe_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    chef: {
                    chef_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    orchestre: {
                    orchestre_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    gym: {
                    gym_souscription: "Souscription",
                    Spesific: "Tarif"
                    },
                    pyscine: {
                    pyscine_reserver: "Reservation",
                    pyscine_souscrire: "Souscription",
                    Spesific: "Tarif"
                    },
                    stade: {
                    stade_reserver: "Reservation",
                    stade_souscrire: "Souscription",
                    Spesific: "Tarif"
                    },
                    cinema: {
                    cinema_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    theatre: {
                    theatre_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    musee: {
                    musee_reserver: "Reservation",
                    Spesific: "Tarif"
                    },
                    avocat: {
                    avocat_rdv: "Resndy Vous",
                    Spesific: "Tarif"
                    },
                    restaurant: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Menu"
                    },
                    cafe: {
                    cafe_commande: "Commandes",
                    cafe_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    hotels: {
                    hotels_reserver: "Reservation",
                    hotels_service: "Service",
                    Spesific: "Tarif"
                    },
                    vg_agence: {
                    agence_service: "Reservation",
                    Spesific: "Tarif"
                    },
                    comptable: {
                    comptable_service: "Demmandes",
                    Spesific: "Tarif"
                    },
                    socite: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    bank: {
                    restaurant_commande: "Viremment",
                    restaurant_reservation: "Retrait",
                    Spesific: "Tarif"
                    },
                    courtier: {
                    courtier_request: "Achat/Louer",
                    courtier_torent: "Vente/à Louer",
                    Spesific: "Tarif"
                    },
                    contracteur: {
                    contracteur_service: "Demmande",
                    Spesific: "Tarif"
                    },
                    architecture: {
                    architecture_service: "Demmandes",
                    Spesific: "Tarif"
                    },
                    veterinaire: {
                    veterinaire_rdv: "Rendy-Vous",
                    Spesific: "Tarifs & Autres"
                    },
                    fourragerie: {
                    restaurant_commande: "Commandes",
                    Spesific: "Tarif"
                    },
                    agritools: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    },
                    farm: {
                    restaurant_commande: "Commandes",
                    restaurant_reservation: "Reservation",
                    Spesific: "Tarif"
                    }
                },
                spesificTabs : {
                        docteur: {
                        sa: "Seances",
                        cl: "Calendrier",
                        pt: "Patient",
                        or: "Ordonance",
                        rp: "Rapport",
                        tm: "Equipe"
                        },
                        pharmacie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        clinique: {
                        sa: "Chambres",
                        cl: "Factures",
                        pt: "Calendrier",
                        or: "Urgences ",
                        rp: "Patient",
                        tm: "Docteurs"
                        },
                        labo: {
                        sa: "Analyses",
                        cl: "Calendrier",
                        pt: "Rapport",
                        or: "Todo ",
                        rp: "Patient",
                        tm: "Equipe"
                        },
                        infirmier: {
                        cl: "Calendrier",
                        pt: "Patient",
                        rp: "Todo",
                        sa: "Seances",
                        tm: "Equipe"
                        },
                        centreMD: {
                        sa: "Chambres",
                        cl: "Factures",
                        pt: "Calendrier",
                        or: "Urgences ",
                        rp: "Patient",
                        tm: "Docteurs"
                        },
                        garderie: {
                        sa: "Eleves",
                        cl: "Abonne..",
                        pt: "Classes",
                        or: "Seances ",
                        rp: "Parents",
                        tm: "Equipe"
                        },
                        ecole: {
                        sa: "Eleves",
                        cl: "Abonne..",
                        pt: "Classes",
                        or: "Seances ",
                        rp: "Parents",
                        tm: "Equipe"
                        },
                        lycee: {
                        sa: "Eleves",
                        cl: "Abonne..",
                        pt: "Classes",
                        or: "Seances ",
                        rp: "Parents",
                        tm: "Equipe"
                        },
                        universite: {
                        sa: "Etudients",
                        cl: "Abonne..",
                        pt: "Classes",
                        or: "Seances ",
                        tm: "Equipe"
                        },
                        librairie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        formation: {
                        sa: "Eleves",
                        cl: "Abonne..",
                        pt: "Classes",
                        or: "Seances ",
                        rp: "Parents",
                        tm: "Equipe"
                        },
                        impremerie: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        transporteur: {
                        sa: "Operations",
                        cl: "Camions",
                        pt: "Voyages",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        autoecole: {
                        sa: "Abonne...",
                        cl: "Seances",
                        pt: "Voitures",
                        or: "Calendrier ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        parking: {
                        sa: "Places",
                        or: "Calendrier ",
                        pt: "Factures",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        qiosque: {
                        sa: "Calendrier",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        location: {
                        sa: "Voitures",
                        cl: "Calendrier",
                        pt: "Factures",
                        or: "Position ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        mecanicien: {
                        sa: "Ateliers",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Calendrier ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        magazin: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        fruiterie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        boucherie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        boulengerie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        patesserie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        epicerie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        electromenager: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Credit",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        meublerie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Credit",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        bijouterie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        quincaillerie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        boutique: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        depot: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Camions ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        menuisier: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        forgeron: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        cristalerie: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        marbrerie: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        electricien: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        peinture: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        plombier: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        coiffure: {
                        sa: "Calendrier",
                        cl: "Stock",
                        pt: "Factures",
                        or: "Attent ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        salle_marriage: {
                        sa: "Calendrier",
                        cl: "Espaces",
                        pt: "Factures",
                        or: "Attent ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        fourniture_marriage: {
                        sa: "Stock",
                        cl: "Calendrier",
                        pt: "Factures",
                        or: "Attent ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        photographe: {
                        tm: "Media",
                        cl: "Calendrier",
                        pt: "Positions",
                        or: "Attent ",
                        rp: "Clients"
                        },
                        chef: {
                        tm: "Plats",
                        cl: "Calendrier",
                        pt: "Positions",
                        or: "Attent ",
                        rp: "Clients"
                        },
                        orchestre: {
                        tm: "Membres",
                        cl: "Calendrier",
                        pt: "Positions",
                        or: "Attent ",
                        rp: "Clients"
                        },
                        gym: {
                        sa: "Abonn...",
                        cl: "Seances",
                        pt: "Equipem...",
                        or: "Groupes ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        pyscine: {
                        sa: "Abonn...",
                        cl: "Seances",
                        pt: "Equipem...",
                        or: "Groupes ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        stade: {
                        sa: "Abonn...",
                        cl: "Seances",
                        pt: "Equipem...",
                        or: "Groupes ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        cinema: {
                        sa: "Abonn...",
                        cl: "Seances",
                        pt: "Calendrier",
                        or: "Groupes ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        theatre: {
                        sa: "Abonn...",
                        cl: "Seances",
                        pt: "Calendrier",
                        or: "Groupes ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        musee: {
                        sa: "Abonn...",
                        cl: "Seances",
                        pt: "Calendrier",
                        or: "Groupes ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        avocat: {
                        sa: "Sujet",
                        cl: "Seances",
                        pt: "Calendrier",
                        or: "Support",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        restaurant: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        cafe: {
                        sa: "Boissons",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        hotels: {
                        sa: "Chambres",
                        cl: "Restaurant",
                        pt: "Pyscine",
                        or: "Calendrier ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        vg_agence: {
                        sa: "Voyages",
                        cl: "Reservat...",
                        pt: "Billets",
                        or: "Transport ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        comptable: {
                        sa: "Archive",
                        cl: "Calendrier",
                        pt: "Auto-Con..",
                        or: "Outils ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        socite: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        bank: {
                        sa: "Operati...",
                        cl: "Comptes",
                        pt: "Liquides",
                        or: "Extrait ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        courtier: {
                        sa: "immobilier",
                        cl: "Calendrier",
                        pt: "offres",
                        or: "Intéressé ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        contracteur: {
                        sa: "Chantier",
                        cl: "Calendrier",
                        pt: "Outils",
                        or: "Partenaires ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        architecture: {
                        sa: "Plans",
                        cl: "Calendrier",
                        pt: "Projet",
                        or: "Partenaires ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        veterinaire: {
                        sa: "Seances",
                        cl: "Calendrier",
                        pt: "Patient",
                        or: "Ordonance",
                        rp: "Rapport",
                        tm: "Equipe"
                        },
                        fourragerie: {
                        sa: "Stock",
                        pt: "Factures",
                        cl: "Caisse",
                        or: "Fournisseur ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        agritools: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        },
                        farm: {
                        sa: "Plates",
                        cl: "Caisse",
                        pt: "Factures",
                        or: "Tables ",
                        rp: "Clients",
                        tm: "Equipe"
                        }
                    
                    
                },
                publicationText: 'Публикации',
                horaireText: 'Часы работы',
                imagesText: 'Изображения',
                ratingText: 'Отзывы',
                tarifText: '',
                shareText: 'Поделиться',
                messageText: '',
                parametreText: 'Настройки профиля',
                voirProfileText: 'Посмотреть профиль'
            },
            requestPage : {

            },
            requestInfoPage : {
                
            },
            profilePage : {
                voirProfileBtn:'',
                menuTabsText: {
                    modifier:'',
                    password:'',
                    horaire:'',
                    position :'',
                    images:'',
                    evaluation:'',
                    imprimer:''

                },
                modifierData :{
                    title:'',
                    nom:'',
                    phone:'',
                    location:'',
                    adresse:'',
                    genreListe:'',
                    modifierBtn:''
                },
                passwordData :{

                },
                horaireData :{

                },
                positionData :{

                },
                imagesData :{

                },
                evaluationData :{

                },
                imprimerData :{

                }
            },
            imagesPage :{
                title : 'Изображения',
                dontHaveImage : 'У вас нет изображений',
                clicForThat : 'Нажмите, чтобы загрузить изображения',
                deleteBtn : 'Удалить',
                saveBtn : 'Сохранить',
                toast : {
                    one : 'Требуется 5 изображений!',
                    two : 'Разрешено только пять изображений!',
                    three : 'Изображения сохранены!',
                    four : 'Изображение удалено!',
                    five : '',
                }
            },            
            systemInscriptionPage : {
                mainFirstTitle:'',
                systemNamesData:'',
                fullVersionHelpYouText:'',
                systemsPlusData:{
                    docteur: {
                        "1": "Ajouter et modifier de nouvelles consultations",
                        "2": "Créer des ordonnances médicales avec une liste complète de médicaments",
                        "3": "Ajouter de nouveaux patients et suivre leur état de santé",
                        "4": "Suivre l'équipe médicale",
                        "5": "Créer des rapports par type"
                    },
                    pharmacie: {
                        "1": "Ajouter et suivre les points de vente : caisses",
                        "2": "Réception des ordonnances médicales des patients",
                        "3": "Ajouter de nouveaux fournisseurs et suivre les réunions quotidiennes avec eux",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer et suivre les factures"
                    },
                    clinique: {
                        "1": "Numériser et suivre le travail des salles",
                        "2": "Suivre les réservations et les organiser",
                        "3": "Facturer toutes les opérations financières",
                        "4": "Suivre l'équipe médicale",
                        "5": "Créer des rapports par type"
                    },
                    labo: {
                        "1": "Ajouter et modifier de nouveaux tests",
                        "2": "Créer et imprimer une version papier de l'analyse",
                        "3": "Ajouter de nouveaux clients et suivre leurs rendez-vous",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer des rapports par type"
                    },
                    infirmier: {
                        "1": "Ajouter et modifier les horaires quotidiens",
                        "3": "Ajouter de nouveaux patients et suivre leur état de santé",
                        "4": "Suivre le travail de l'assistant",
                        "5": "Créer une liste de tâches par type"
                    },
                    centreMD: {
                        "1": "Numériser et suivre le travail des salles",
                        "2": "Suivre les réservations et les organiser",
                        "3": "Facturer toutes les opérations financières",
                        "4": "Suivre l'équipe médicale",
                        "5": "Créer des rapports par type"
                    },
                    garderie: {
                        "1": "Ajouter et suivre les élèves",
                        "2": "Enregistrer les sections et les niveaux scolaires",
                        "3": "Enregistrer la présence, l'absence et les dates des examens",
                        "4": "Suivre les rapports de progression des cours et le travail des éducateurs",
                        "5": "Numériser, enregistrer et suivre les cours quotidiens"
                    },
                    ecole: {
                        "1": "Ajouter et suivre les élèves",
                        "2": "Enregistrer les sections et les niveaux scolaires",
                        "3": "Enregistrer la présence, l'absence et les dates des examens",
                        "4": "Suivre les rapports de progression des cours et le travail des éducateurs",
                        "5": "Numériser, enregistrer et suivre les cours quotidiens"
                    },
                    lycee: {
                        "1": "Ajouter et suivre les élèves",
                        "2": "Enregistrer les sections et les niveaux scolaires",
                        "3": "Enregistrer la présence, l'absence et les dates des examens",
                        "4": "Suivre les rapports de progression des cours et le travail des éducateurs",
                        "5": "Numériser, enregistrer et suivre les cours quotidiens"
                    },
                    universite: {
                        "1": "Ajouter et suivre les élèves",
                        "2": "Enregistrer les sections et les niveaux scolaires",
                        "3": "Enregistrer la présence, l'absence et les dates des examens",
                        "4": "Suivre les rapports de progression des cours et le travail des éducateurs",
                        "5": "Numériser, enregistrer et suivre les cours quotidiens"
                    },
                    librairie: {
                        "1": "Ajouter et suivre les points de vente : caisses",
                        "2": "Réception des demandes des clients",
                        "3": "Ajouter de nouveaux clients et suivre la liste des dettes",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer et suivre les factures"
                    },
                    formation: {
                        "1": "Ajouter et suivre les élèves",
                        "2": "Enregistrer les sections et les niveaux scolaires",
                        "3": "Enregistrer la présence, l'absence et les dates des examens",
                        "4": "Suivre les rapports de progression des cours et le travail des éducateurs",
                        "5": "Numériser, enregistrer et suivre les cours quotidiens"
                    },
                    impremerie: {
                        "1": "Ajouter et modifier de nouvelles consultations",
                        "2": "Créer des ordonnances médicales avec une liste complète de médicaments",
                        "3": "Ajouter de nouveaux patients et suivre leur état de santé",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer des rapports par type"
                    },
                    transporteur: {
                        "1": "Ajouter et numériser les opérations de transport quotidiennes",
                        "2": "Créer et enregistrer les factures",
                        "3": "Ajouter de nouveaux clients et suivre leurs voyages",
                        "4": "Suivre l'équipe de travail"
                    },
                    autoecole: {
                        "1": "Ajouter et modifier de nouvelles consultations",
                        "2": "Suivre les dates des examens et des tests",
                        "3": "Ajouter de nouveaux candidats",
                        "4": "Suivre l'équipe de travail",
                        "5": "Suivre les voitures"
                    },
                    parking: {
                        "1": "Suivre les places de stationnement disponibles",
                        "2": "Suivre les réservations quotidiennes",
                        "3": "Facturer les réservations et archiver les factures",
                        "4": "Suivre l'équipe de travail",
                        "5": "Suivre les clients et numériser les réservations"
                    },
                    qiosque: {
                        "1": "Ajouter et suivre les points de vente : caisses",
                        "3": "Ajouter de nouveaux clients et suivre la liste des dettes",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer et suivre les factures"
                    },
                    location: {
                        "1": "Archiver et suivre les voitures",
                        "2": "Facturer les différents processus de location",
                        "3": "Suivre les opérations de location quotidiennes",
                        "4": "Suivre l'équipe de travail",
                        "5": "Suivre l'emplacement actuel des voitures"
                    },
                    mecanicien: {
                        "1": "Numériser et archiver les activités des différents ateliers",
                        "2": "Facturer les différentes opérations quotidiennes",
                        "3": "Ajouter de nouveaux clients et suivre la maintenance de leurs voitures",
                        "4": "Suivre l'équipe de travail",
                        "5": "Suivre les rendez-vous pour les différents contrôles quotidiens"
                    },
                    magazin: {
                        "1": "Ajouter et suivre les points de vente : caisses",
                        "2": "Ajouter de nouveaux clients et suivre la liste des dettes",
                        "3": "Ajouter de nouveaux fournisseurs et suivre les réunions quotidiennes avec eux",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer et suivre les factures"
                    },
                    fruiterie: {
                        "1": "Ajouter et suivre les points de vente : caisses",
                        "2": "Ajouter de nouveaux clients et suivre la liste des dettes",
                        "3": "Ajouter de nouveaux fournisseurs et suivre les réunions quotidiennes avec eux",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer et suivre les factures"
                    },
                    boucherie: {
                        "1": "Ajouter et suivre les points de vente : caisses",
                        "2": "Ajouter de nouveaux clients et suivre la liste des dettes",
                        "3": "Ajouter de nouveaux fournisseurs et suivre les réunions quotidiennes avec eux",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer et suivre les factures"
                    },
                    boulengerie: {
                        "1": "Ajouter et suivre les points de vente : caisses",
                        "2": "Ajouter de nouveaux clients et suivre la liste des dettes",
                        "3": "Ajouter de nouveaux fournisseurs et suivre les réunions quotidiennes avec eux",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer et suivre les factures"
                    },
                    patesserie: {
                        "1": "Ajouter et suivre les points de vente : caisses",
                        "2": "Ajouter de nouveaux clients et suivre la liste des dettes",
                        "3": "Ajouter de nouveaux fournisseurs et suivre les réunions quotidiennes avec eux",
                        "4": "Suivre l'équipe de travail",
                        "5": "Créer et suivre les factures"
                    },
                    epicerie: {
                    "1": "إضافة و متابعة واجهات البيع:  caisses ",
                    "2": "إضافة عملاء جدد و متابعة قائمة الديون",
                    "3": "إضافة مزودين  جدد و متابعة يومية اللقاء معهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء و متابعة الفواتير "
                    },
                    electromenager: {
                    "1": "إضافة و متابعة واجهات البيع:  caisses ",
                    "2": "إضافة عملاء جدد و متابعة قائمة الديون",
                    "3": "إضافة مزودين  جدد و متابعة يومية اللقاء معهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء و متابعة الفواتير "
                    },
                    meublerie: {
                    "1": "إضافة و متابعة واجهات البيع:  caisses ",
                    "2": "إضافة عملاء جدد و متابعة قائمة الديون",
                    "3": "إضافة مزودين  جدد و متابعة يومية اللقاء معهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء و متابعة الفواتير "
                    },
                    bijouterie: {
                    "1": "إضافة و متابعة واجهات البيع:  caisses ",
                    "2": "إضافة عملاء جدد و متابعة قائمة الديون",
                    "3": "إضافة مزودين  جدد و متابعة يومية اللقاء معهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء و متابعة الفواتير "
                    },
                    quincaillerie: {
                    "1": "إضافة و متابعة واجهات البيع:  caisses ",
                    "2": "إضافة عملاء جدد و متابعة قائمة الديون",
                    "3": "إضافة مزودين  جدد و متابعة يومية اللقاء معهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء و متابعة الفواتير "
                    },
                    boutique: {
                    "1": "إضافة و متابعة واجهات البيع:  caisses ",
                    "2": "إضافة عملاء جدد و متابعة قائمة الديون",
                    "3": "إضافة مزودين  جدد و متابعة يومية اللقاء معهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء و متابعة الفواتير "
                    },
                    depot: {
                    "1": "إضافة و متابعة واجهات البيع:  caisses ",
                    "2": "إضافة عملاء جدد و متابعة قائمة الديون",
                    "3": "إضافة مزودين  جدد و متابعة يومية اللقاء معهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء و متابعة الفواتير "
                    },
                    menuisier: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    forgeron: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    cristalerie: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    marbrerie: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    electricien: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    peinture: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    plombier: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    coiffure: {
                    "1": "فوترة عمليات الحلاقة اليومية",
                    "2": "متابعة مواعيد الحجوزات من خلال اليومية",
                    "3": "متابعة قاعة الأنتضار و تقليص مدة الانتضار ",
                    "4": "متابعة فريق العمل"
                    },
                    salle_marriage: {
                    "1": "فوترة عمليات الحجز اليومية",
                    "2": "متابعة مواعيد الحجوزات من خلال اليومية",
                    "3": "متابعة قائمة الأنتضار ",
                    "4": "متابعة فريق العمل"
                    },
                    fourniture_marriage: {
                    "1": "متابعة و التحكم في المخزون ",
                    "2": "متابعة مواعيد الحجوزات من خلال اليومية",
                    "3": "متابعة قائمة الأنتضار لتفادي النسيان و التأخير",
                    "4": "متابعة فريق العمل"
                    },
                    photographe: {
                    "1": "متابعة المواعيد و الحجوزات من خلال اليومية",
                    "2": "تقديم و عرض الأعمال السابقة",
                    "3": "مناقشة أراء المتابعين و الرد علي مقترحاتهم",
                    "4": "أرشفة و تسجيل قائمة العملاء مع بيانات الإتصال بهم"
                    },
                    chef: {
                    "1": "متابعة المواعيد و الحجوزات من خلال اليومية",
                    "2": "تقديم و عرض الأعمال السابقة",
                    "3": "مناقشة أراء المتابعين و الرد علي مقترحاتهم",
                    "4": "أرشفة و تسجيل قائمة العملاء مع بيانات الإتصال بهم"
                    },
                    orchestre: {
                    "1": "متابعة المواعيد و الحجوزات من خلال اليومية",
                    "2": "تقديم و عرض الأعمال السابقة",
                    "3": "مناقشة أراء المتابعين و الرد علي مقترحاتهم",
                    "4": "أرشفة و تسجيل قائمة العملاء مع بيانات الإتصال بهم"
                    },
                    gym: {
                    "1": "رقمنة و أرشفة الأشتراكات",
                    "2": "متابعة تسجيل الحصص في قاعة البيانات",
                    "3": "متابعة المدربين و فريق العمل"
                    },
                    pyscine: {
                    "1": "رقمنة و أرشفة الأشتراكات",
                    "2": "متابعة تسجيل الحصص في قاعة البيانات",
                    "3": "متابعة المدربين و فريق العمل"
                    },
                    stade: {
                    "1": "رقمنة و أرشفة الأشتراكات",
                    "2": "متابعة تسجيل الحصص في قاعة البيانات",
                    "3": "متابعة المدربين و فريق العمل"
                    },
                    cinema: {
                    "1": "رقمنة و أرشفة الأشتراكات",
                    "2": "متابعة تسجيل الحصص في قاعة البيانات",
                    "3": "متابعة فريق العمل"
                    },
                    theatre: {
                    "1": "رقمنة و أرشفة الأشتراكات",
                    "2": "متابعة تسجيل الحصص في قاعة البيانات",
                    "3": "متابعة فريق العمل"
                    },
                    musee: {
                    "1": "رقمنة و أرشفة الأشتراكات",
                    "2": "متابعة تسجيل الحصص في قاعة البيانات",
                    "3": "متابعة فريق العمل"
                    },
                    avocat: {
                    "1": "أرشفة و رقمنة القضايا الموكولة",
                    "2": "تسجيل و متابعة الحصص و الأستشارات ",
                    "3": "إنشاء قاعدة بيانات الموكلين و بيانات التواصل معهم ",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    restaurant: {
                    "1": "أرشفة ومتابعة قائمة الأطباق",
                    "2": "إنشاء طاولة جديدة و متابعة الطلبات حسب الطاولة",
                    "3": "متابعة فريق العمل "
                    },
                    cafe: {
                    "1": "أرشفة ومتابعة قائمة المشروبات",
                    "2": "إنشاء طاولة جديدة و متابعة الطلبات حسب الطاولة",
                    "3": "متابعة فريق العمل "
                    },
                    hotels: {
                    "1": "إضافة و رقمنة الغرف حسب الصنف ",
                    "2": "متابعة طلبات و فواتير المطعم ",
                    "3": "إستقبال طلبات الزوار من الغرف آليا ",
                    "4": "متابعة فريق العمل"
                    },
                    vg_agence: {
                    "1": " متابعة مواعيد حجز تذاكر الطيران",
                    "2": "متابعة مواعيد حجز الإقامة",
                    "3": "ترتيب الرحلات السياحية",
                    "4": "متابعة فريق العمل"
                    },
                    comptable: {
                    "1": "أرشفة ذكية لكل ملفاتك الرقمية",
                    "2": "متابعة مواعيد تدقيق المحاسبة من خلال اليومية",
                    "3": "الربط المباشر بأنظمة العملاء",
                    "4": "الحصول علي أدوات المحاسبة السهلة"
                    },
                    socite: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    bank: {
                    "1": "رقمنة الحسابات البنكية",
                    "2": "أرشفة العمليات اليومية و حمايتها",
                    "3": "إضافة عملاء جدد و متابعة حساباتهم البنكية",
                    "4": "متابعة فريق العمل"
                    },
                    courtier: {
                    "1": "أرشفة و رقمنة العقارات التي تعمل عليها",
                    "2": "متابعة مواعيد الزيارات من خلال اليومية",
                    "3": "أنشاء قاعدة عملاء و مهتمين بالعروض العقارية",
                    "4": "متابعة فريق العمل"
                    },
                    contracteur: {
                    "1": "رقمنة و أرشفة مناطق البناء من خلال الخريطة",
                    "2": "متابعة تقدم الإشغال بإستعمال اليومية التفاعلية",
                    "3": "إنشاء قاعدة بيانات الشركات و قنوات الإتصال بهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    architecture: {
                    "1": "رقمنة و أرشفة التصاميم العقارية ",
                    "2": "متابعة تقدم المشاريع بإستعمال اليومية التفاعلية",
                    "3": "إنشاء قاعدة بيانات الشركات و قنوات الإتصال بهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    veterinaire: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    fourragerie: {
                    "1": "إضافة و متابعة واجهات البيع:  caisses ",
                    "2": "إضافة عملاء جدد و متابعة قائمة الديون",
                    "3": "إضافة مزودين  جدد و متابعة يومية اللقاء معهم",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء و متابعة الفواتير "
                    },
                    agritools: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    },
                    farm: {
                    "1": "إضافة و تعديل حصص جديدة ",
                    "2": "إنشاء وصفات طبية مع قائمة كاملة للأدوية",
                    "3": "إضافة مرضي جدد و متابعة حالتهم الصحية",
                    "4": "متابعة فريق العمل",
                    "5": "إنشاء تقارير حسب النوع"
                    }
                },
                startCard : {
                    subscribeNow:'',
                    systemNeedTwoCon :'',
                    conditionOne:'',
                    conditionTwo :'',
                    saveSubsRequestButton:'',
                    priceText:''
                },
                waitingCard : {
                    subsIsRunning :'',
                    suppText:'',
                },
                SuccessCard :{

                }

            },
            
        },
        toolsApps : {
            toolsPage:{
                mainTitle:'Découvrez la gamme d\'outils créatifs Abiyad qui vous dispensent de nombreux autres outils.',
                subtitle:'Une gamme complète d\'outils numériques et d\'applications tunisiennes pour vous aider à trouver les informations appropriées',
                transportTitle : 'Applications de transport',
                educationTitle : 'Applications éducatives',
                financeTite : 'Applications financières et commerciales',
                constructionTitle :'Applications dans le domaine de la construction',
                otherTitle :'Autres applications',
                appNames: {
                    taxiApp : 'Taxi',
                    louageApp : 'Louage',
                    transportApp : 'Transport',
                    automobileApp : 'Automobile',
                    etudeApp : 'Cours particuliers',
                    programAnuueliApp : 'Programme annuel',
                    livreScolaireApp : 'Manuels scolaires',
                    examainApp : 'Examens',
                    findworkApp : 'Trouve-moi un travail',
                    salaireApp : 'Salaire',
                    locationApp : 'Location',
                    newsApp : 'Actualités',
                    forumApp : 'Forum',
                    statApp : 'Statistiques',
                    adminBlogApp : 'Encyclopédie administrative',
                    catalogueApp : 'Catalogue',
                    sportApp : 'Sports',
                    cultureApp : 'Culture',
                    tourizmeApp : 'Découvrir la France',
                    
                }
            },
            taxiApp:{
                mainAdsTitle : 'Vous prenez un taxi ? Vous voulez réserver un taxi ? Vous voulez connaître le prix d\'une course en taxi ? Vous voulez gagner du temps et ne pas attendre longtemps ? Vous êtes au bon endroit : la plateforme Abiyad pour les taxis',
                taxiSimpleText :'Taxi individuel',
                taxiSimpleTextAds :'Recherchez un taxi individuel',
                taxiCollectiffText :'Taxi collectif',
                taxiCollectiffTextAds :'Recherchez un taxi collectif',
                systemCardTitle : 'Êtes-vous un chauffeur de taxi et souhaitez-vous vous inscrire sur la plateforme ?',
                systemTitleContent : 'Découvrez la version réduite du système de gestion des taxis qui vous permet de vous présenter et de recevoir des demandes de vos clients.',
                inscriptionBtnText : 'Inscrire au système',
                systemBtnText : 'Accéder au système',
                addSimple: {

                },
                addCollectif : {

                }
            },
            LouageApp:{

            },
        },
        aboutPage:{
            mainTitle : 'Платформа Абияд',
            subtitle : 'Приложение для всего',
            numbers: {
                research: 'Результаты поиска',
                visitor : 'Посетители в месяц',
                usersNum: 'Пользователи',
                clientNum : 'Клиенты',
            },
            platformeObjectText:'Цель платформы Абияд - цифровизация как можно большего числа деятельностей для тунисских пользователей с целью создания всеохватывающей цифровой среды. Она делает это через:',
            directoryContent : {
                title: 'Интерактивный каталог:',
                text:'Абияд предоставляет интерактивный каталог и поисковый движок, содержащий более 21 000 различных предприятий, таких как врачи, адвокаты, рестораны, кафе, школы, отели, администрации и т. д. Помимо этого, она предоставляет всю информацию, необходимую для принятия правильного решения для вашей повседневной деятельности, такую как время путешествия, транспортные средства и административные ситуации в Тунисе, что значительно упрощает жизнь тунисским гражданам... Вы также можете взаимодействовать напрямую с результатами поиска.',
                buttonText:'Узнать больше'
            },
            systemsContent : {
                title: 'Системный движок:',
                text:'Абияд предлагает множество систем мониторинга, управления и учета для фрилансеров, малых предприятий и других: частные школы и университеты, врачи, адвокаты, владельцы кафе, рестораны, различные точки продажи, компании и т. д. а также множество других систем, направленных на улучшение и развитие бизнеса и повышение качества услуг и продукции с использованием различных технологий и цифровой инфраструктуры.',
                buttonText:'Узнать больше'
            },
            toolsContent : {
                title: 'Набор творческих инструментов:',
                text:'Одной из основных целей Абияд является предоставление подходящей информации и руководства для тунисских пользователей, чтобы облегчить выполнение их повседневных задач и устранить путаницу и сложности, связанные с тунисской администрацией. Это делается через энциклопедию, объясняющую пользователям, как выполнять различные административные и правовые действия, такие как получение паспорта, удостоверения личности, создание компаний, получение медицинской книжки и т. д., и другие вопросы, которые пользователь может столкнуться в своей повседневной жизни.',
                buttonText:'Узнать больше'
            },
            profileContent : {
                title: 'Легкое управление всеми вашими повседневными активностями:',
                text:'Обычный пользователь также может управлять своими повседневными действиями через свой аккаунт на Абияд, используя цифровой интерфейс, объединяющий все его повседневные действия, а также организованный и напоминающий календарь. Этот аккаунт также может быть связан с банковским счетом и цифровым административным аккаунтом Туниса для упрощения управления вашими повседневными действиями и экономии времени и усилий.',
                buttonText:'Узнать больше'
            },
        },


    },
}
export default RussiaTrans