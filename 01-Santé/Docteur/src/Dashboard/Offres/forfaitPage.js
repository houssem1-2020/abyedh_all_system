         ...
         .then(function (response) {
            ...
            if (!response.data) {
               ...
               response.data.map( (getData) => articleListContainer.push([
                  ...
                  _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/of/info/${getData.Tarif_ID}`)}><span className='d-none d-lg-inline'> Info </span> </Button>),
               ],))
                  ...
                  <div className='col-12 col-lg-4  '>
                     <div className="sticky-top" style={{top:'70px'}}>
                        <div className='card card-body border-div shadow-sm mb-4'>
                             <h3 className='mb-1'>Ajouter Tarif </h3> 
 
                              <h5 className='mb-1 mt-0'>Nom: </h5>
                              <Input icon='star' iconPosition='left' placeholder='Nom' className='w-100 border-0 shadow-sm rounded mb-3' onKeyPress={event => OnKeyPressFunc(event)} value={tarifData.Tarif_Name}  onChange={(e) => setTarifData({...tarifData, Tarif_Name: e.target.value })}/>
                              
                              <h5 className='mb-1 mt-0'>Description: </h5>
                              <Form>
                                 <TextArea rows={2} placeholder='Description' onKeyPress={event => OnKeyPressFunc(event)} value={tarifData.Tarif_Description} onChange={(e) => setTarifData({...tarifData, Tarif_Description: e.target.value })}/>
                              </Form>

                              <h5 className='mb-1'>Tarif: </h5>
                              <Input icon='dollar' iconPosition='left' type='number' placeholder='Tarif' value={tarifData.Tarif}   className='w-100 border-0 shadow-sm rounded mb-3' onChange={(e) => setTarifData({...tarifData, Tarif: e.target.value })}/> 
                                 
                              <div className='text-end mb-2'>
                                 <Button onClick={SaveArticle}  className='text-end rounded-pill bg-system-btn'  disabled={saveBtnState} positive>  <Icon name='save outline' /> Enregistrer <Loader inverted active={loaderState} inline size='tiny' className='ms-2 text-danger'/></Button>
                              </div>
