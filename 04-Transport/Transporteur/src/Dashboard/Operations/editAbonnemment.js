...
function AjouterFacture() {
          ...
          .then(function (response) {
                if(!response.data[0]) {
                   ...
                   // SelectClientFunction(response.data[0].OR_Patient)
                    setFactureD({ OR_Patient: response.data[0].OR_Patient , OR_Date: response.data[0].OR_Date , OP_De:JSON.parse(response.data[0].OP_De) , OP_Vers:JSON.parse(response.data[0].OP_Vers) ,  OP_Articles:JSON.parse(response.data[0].OP_Articles)})
                    setLoading(true)
    ...
    const EditOrdonance = () =>{
            if (!factureD.OR_Date ) {toast.error("Date est Invalide !", GConf.TostErrorGonf)}
            else if (!factureD.OR_Patient) {toast.error("Patient De est Invalide !", GConf.TostErrorGonf)}
