        ...
        .then(function (response) {
          if (!response.data) {
              ...
              response.data.map( (getData) => classesListeContainer.push([
                _(<TableImage image={'camion.jpg'} forStock onClick={() => openEditModal(getData,true)}/>),
                getData.CM_ID,
                getData.CM_Name,
                getData.CM_Matricule,
                getData.CM_Genre,
                _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/cm/info/${getData.CM_ID}`)}><span className='d-none d-lg-inline'> Info </span><Icon  name='angle right' /></Button>),
              ],))
    ...
    return (<>
              <div className='row'>
                  ...
                  <div className='col-12 col-lg-4 text-end align-self-center'>
                      <MainSubNavCard text='Interface Conduit' link='../C' icon='window-dock' />  
                  </div>
              </div>
