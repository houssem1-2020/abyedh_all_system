        ...
        if (!loginD.Log) {toast.error("Entrer Un identifiant !", GConf.TostErrorGonf)}
            ...
            axios.post(`${GConf.ApiCamionLink}/LogIn`, {
                ...
                if(response.data.length != 0 ) {
                        ...
                        if (!Offline) {
                            localStorage.setItem(`Camion_Offline`, JSON.stringify({stock: [], facture: [],  client:[],  factureToSave:[], clientToSave:[], depensesToSave:[]}));
                        }
                        localStorage.setItem(`Camion_LocalD`, JSON.stringify(response.data));
